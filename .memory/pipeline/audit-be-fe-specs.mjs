...
const beReportPath = path.join(auditDir, "2026-08-29-be-ambiguity-report.md");
const feReportPath = path.join(auditDir, "2026-08-29-fe-ambiguity-report.md");
...
function localIndexLinks(indexPath) {
  ...
  return [...new Set([...source.matchAll(/\]\(([^)#?]+\.md)(?:#[^)]+)?\)/g)]
    .map((match) => match[1])
...
function reconcileLayer(directory) {
  const filesystem = markdownFiles(directory).filter((file) => file !== "index.md");
  const listed = localIndexLinks(path.join(directory, "index.md")).filter((file) => file !== "index.md");
  return {
    filesystem,
    listed,
...
function routeRegistry(source) {
  const lines = source.split(/\r?\n/);
  const candidates = [];
  for (let index = 0; index < lines.length - 2; index += 1) {
    if (!/^\|/.test(lines[index]) || !/^\|\s*:?-/.test(lines[index + 1])) continue;
    const header = parseRow(lines[index]).map((cell) => cell.replace(/[\x60*]/g, "").toLowerCase());
    if (!header.some((cell) => /method|path|route/.test(cell))) continue;
    const rows = [];
    for (let rowIndex = index + 2; rowIndex < lines.length && /^\|/.test(lines[rowIndex]); rowIndex += 1) {
      const cells = parseRow(lines[rowIndex]);
      const raw = cells.join(" ");
      if (/\b(GET|POST|PUT|PATCH|DELETE|WS|WEBSOCKET|SSE)\b/i.test(raw) && /\/[A-Za-z]/.test(raw)) {
        rows.push({ line: rowIndex + 1, raw, cells });
      }
    }
...
function hasUnresolvedChoice(source) {
  ...
  const openIsClosed = !openQuestions || /^(?:None\.?|[-*]\s*None\.?|No unresolved\b)/i.test(openQuestions);
  const marker = source.match(/\b(?:TODO|TBD|FIXME)\b|\{\{[^}]+\}\}|implementation decides|left to implementation/i);
  return !openIsClosed || Boolean(marker);
}

function applicationErrorCodes(source) {
  return [...new Set([...source.matchAll(/\b([A-Z][A-Z0-9_]{3,})\b/g)]
    .map((match) => match[1])
    .filter((value) => /INVALID|VALIDATION|UNAUTH|FORBIDDEN|NOT_FOUND|CONFLICT|STALE|RATE|DEPENDENCY|TIMEOUT|EXPIRED|REQUIRED|UNAVAILABLE|FAILED|DENIED|MISMATCH|BLOCKED|CANCELLED|REVOKED|DUPLICATE/.test(value)))];
}
...
function scoreBeDocument(file) {
  ...
  const hasBe00 = file === "00-infrastructure.md" || /\bBE00\b|00-infrastructure\.md/i.test(source);
  const iaSource = /(?:\.\.\/|specs\/)ia\/\d{2}-[^)\s]+\.md/i.test(source);
  const integrationSection = /^#{2,4}\s+.*(?:External|Integration).*Seam/im.test(source);
  const listRoutes = routes.filter(({ raw }) => {
    if (!/\bGET\b/.test(raw)) return false;
    return /list|search|feed|discover|collection|catalog|history|report|statement|export|browse/i.test(raw)
      || !/\/\{[^}]+\}(?:[?\x60|\s]|$)/.test(raw);
  });
  const checks = [
    iaSource && /interaction|feature|source map|referenced[- ]material/i.test(source) && routes.length > 0,
    hasBe00 && /Zod 4|strict Zod/i.test(source) && /request/i.test(source)
      && /success|response/i.test(source) && routes.length > 0,
    hasBe00 && /ApiError|global error/i.test(source) && /retry|non-retryable|terminal/i.test(source)
      && applicationErrorCodes(source).length > 0,
    /Database Schema|Persistence|durable authority/i.test(source) && /uuid|text|integer|boolean|timestamptz|jsonb/i.test(source)
      && /constraint|nullab|not null|unique/i.test(source) && /index|query pattern/i.test(source),
    hasBe00 && /middleware|CORS/i.test(source) && /\brate\b|rate[- /]?limit/i.test(source) && /validat/i.test(source)
      && /auth|capability/i.test(source),
    /state|lifecycle/i.test(source) && /transition|terminal|allowed|blocked|reject/i.test(source),
    /idempoten/i.test(source) && /concurr|lock|version|atomic|CAS|serializ/i.test(source),
    listRoutes.length === 0 || (/pagination|cursor|collection bounds|fixed-read|page size/i.test(source)
      && /default|max(?:imum)?|limit/i.test(source) && /sort|order/i.test(source) && /filter|query/i.test(source)),
    !integrationSection || (/timeout|deadline/i.test(source) && /retry/i.test(source) && /circuit/i.test(source)),
    hasBe00 && /auth|capability|RLS/i.test(source) && /sanitiz|validat/i.test(source)
      && /disclosure|redact|output filter|response allowlist|ownership|RLS/i.test(source),
    hasBe00 && /ApiError|global error envelope|Error Architecture/i.test(source)
      && /requestId/i.test(source) && /details/i.test(source),
  ];
  const evidence = [
    evidenceLine(lines, [/IA source/i, /IA Source Map/i, /Referenced[- ]Material/i, /\.\.\/ia\//i]),
    evidenceLine(lines, [/Zod 4/i, /Request.*Response/i, /BE00 inheritance/i, /Platform contract/i]),
    evidenceLine(lines, [/ApiError/i, /Error Matrix/i, /retry guidance/i]),
    evidenceLine(lines, [/Database Schema/i, /Persistence/i, /durable authority/i]),
    ...
    evidenceLine(lines, [/pagination/i, /collection bounds/i, /cursor/i, /page size/i]),
    evidenceLine(lines, [/External Seams/i, /Integration Seams/i, /timeout/i, /circuit/i]),
    evidenceLine(lines, [/Access Control/i, /authorization/i, /capability/i, /RLS/i]),
    evidenceLine(lines, [/Error Architecture/i, /global error envelope/i, /ApiError/i, /BE00/i]),
  ];
...
function renderScope({ status, beFiles, feFiles, reportLines = [], gapsFixed = "pending" }) {
  ...
  return [
    ...
    "## Documents",
    "",
    ...documents.map((filePath, index) => `${index + 1}. ${tick}${path.relative(root, filePath)}${tick}`),
    "",
    "## Rubric Files",
    "",
...
function renderBeReport(results, reconciliation, feSummary) {
  const failures = results.flatMap((result) => result.checks.flatMap((ok, index) => ok ? [] : [{ file: result.file, dimension: beDimensions[index] }]));
  const total = results.length * beDimensions.length;
  const routeCount = results.reduce((sum, result) => sum + result.routes.length, 0);
  const corpusHash = hashCorpus([path.join(beDir, "index.md"), ...results.map((result) => path.join(beDir, result.file))]);
  const report = [
    "# BE Ambiguity Audit — Fresh Combined Run (2026-08-29)",
    "",
    "## Verdict",
    "",
    failures.length ? `**FAIL — ${failures.length}/${total} ambiguous checkpoints.**` : `**PASS — ${total}/${total} checkpoints; 0/${total} ambiguity (0.00%).**`,
    "",
    "## Scope and Freshness",
    "",
    ...
    ...results.map((result, index) => {
      ...
      const evidence = result.evidence.map((line, dimension) => `D${dimension + 1} ${line ? `L${line}` : "N/A"}`).join("; ");
    ...
    "## Punch List",
    "",
    ...(failures.length ? failures.map((failure) => `- ${tick}${failure.file}${tick}: ${failure.dimension}.`) : ["- None."]),
    "",
    "## Gaps Fixed",
    "",
...
if (beReconciliation.missing.length || beReconciliation.unlisted.length || feReconciliation.missing.length || feReconciliation.unlisted.length) {
  throw new Error(`Enumeration gate failed: ${JSON.stringify({ beReconciliation, feReconciliation })}`);
}

const beFiles = beReconciliation.filesystem;
const feFiles = feReconciliation.filesystem;
fs.writeFileSync(scopePath, renderScope({ status: "in progress — 0/201 processed", beFiles, feFiles }), "utf8");

const feRun = spawnSync(process.execPath, [path.join(root, ".memory/pipeline/audit-fe-specs.mjs")], {
  cwd: root,
  encoding: "utf8",
  maxBuffer: 64 * 1024 * 1024,
});
const feJsonMatch = feRun.stdout.match(/<<<FE_AUDIT_JSON>>>\n([^\n]+)/);
const feReportMatch = feRun.stdout.match(/<<<FE_AUDIT_REPORT>>>\n([\s\S]*?)<<<FE_AUDIT_END>>>/);
if (!feJsonMatch || !feReportMatch) throw new Error(`FE audit helper output was invalid: ${feRun.stderr}`);
...
fs.writeFileSync(beReportPath, beReport.content.replace(/\n+$/, "\n"), "utf8");
fs.writeFileSync(feReportPath, feReportMatch[1], "utf8");
const failed = beReport.failures.length || feSummary.failed || feSummary.crossFailures;
fs.writeFileSync(scopePath, renderScope({
  status: failed
  ...
  feFiles,
  reportLines: [
    `- **BE report:** ${tick}.memory/wiki/specs/audits/2026-08-29-be-ambiguity-report.md${tick}`,
    `- **FE report:** ${tick}.memory/wiki/specs/audits/2026-08-29-fe-ambiguity-report.md${tick}`,
  ],
  gapsFixed: failed ? "See report punch lists; remediation required before a fresh rerun." : "None — the fresh run found no ambiguity gaps and changed no scoped spec.",
}), "utf8");

process.stdout.write(`${JSON.stringify({
  documents: beFiles.length + feFiles.length + 2,
  be: { documents: beFiles.length, routes: beReport.routeCount, total: beReport.total, failed: beReport.failures.length },
  fe: feSummary,
