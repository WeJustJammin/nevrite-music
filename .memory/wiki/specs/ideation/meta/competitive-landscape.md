...
bet is that consolidating them wins the user, and that being **in the workflow** — hosting the hire,
the project and the delivery, so the record is confirmed at the first sharing moment rather than
reconstructed years later — creates a record no point solution can replicate. The credits databases
(Jaxsta, Muso.AI, Sound Credit, Discogs) exist *because* credits are broken, and all fail the same
way: they **reconstruct** credits after the fact from whoever remembers. None were anywhere near
the work.

> **v1 wording, restated 2026-07-22 (D-70).** The differentiation stated on this page is
> *capture-vs-reconstruct*, and that still holds for v1 — but the capture point in v1 is the **first
> sharing moment** (the review link `07.05.02` plus the session-close prompt `07.06.02`), not inside
> the DAW. The DAW bridge (`07.09`) is not phased into v1 and no locally-installed client surface is
> authorised. "Present at the session / capture at source" is the **direction**, not the v1 claim.
> Domain 07 D-06 makes this an obligation: an overclaim discovered later by a professional user is
> an unrecoverable trust breach. See Q-04 below — whether the restated claim is still competitively
> differentiating is a market judgement no source in this tree makes.
...
| **Sound Credit / Jaxsta / Muso.AI** | Credits databases | **Credits (02)** | **The direct wedge competitor — and the one we beat by construction.** They reconstruct after release; **in v1 we capture at the first sharing moment** — days after the take, while everyone is still reachable (D-70). Capture-at-source is the direction, not the v1 claim. Detailed below. |
...
the day costs one signature. **A platform that hosts the work — the hire, the project, the delivery
— is the only kind of system that is present early enough to capture provenance while it is still
uncontested.** That is not a feature competitors can copy: an incumbent that only sees the finished
release has already missed the moment.

> **v1 boundary (D-70)**: "early enough" in v1 means the **first sharing moment** — the review link
> and the session-close prompt, days after the take rather than years. It does not mean inside the
> DAW at the instant the take exists; that requires `07.09`, which is unphased and gated. The
> asymmetry against the credits databases survives the restatement (they capture nothing until
> release), but the wording must not claim source capture.

## Unique Differentiators

1. **Split-at-first-sharing** — ownership captured while the work is fresh and the parties are still
   in contact, not in reconstruction years later. Structurally unrepeatable by any competitor who
   was not in the workflow when the work was made. *(D-70: the stronger "split-at-creation, inside
   the DAW" form is the direction; `07.09` is unphased and gated, so v1 does not claim it.)*
...
was captured here while it was still uncontested, and cannot be reconstructed elsewhere. A user who
leaves doesn't lose access to *data they could export*; they lose the *provenance* that only exists
because the platform was in the workflow when the work happened. Network effects compound it: each
verified collaboration strengthens the graph for everyone in it. This is defensible in a way a
marketplace or a DAW alone is not — those compete on price and features; provenance competes on
...
| Q-04 | **Is the restated v1 claim still competitively differentiating?** D-70 restated the v1 position from "capture at source / present at the moment of creation" to "capture at the first sharing moment". Whether that still wins against Sound Credit / Jaxsta / Muso.AI on positioning is a market judgement no source in this tree makes — this page must be re-read against the new wording before v1 positioning copy is finalised. Canonical entry: `vision.md` Q-08. | User | `/create-prd` |
