...
| Q-03 | **Can an artist export their fan list in full?** If no, the lock-in is hostile, not earned — this directly answers `meta/problem-statement.md` Q-02. If yes, the owned-audience promise is credible but the moat is behavioural. Collides with [20.01.05](./20.01-fan-graph-owned-audience/20.01.05-fan-list-ownership-transfer.md) DT-03: unlimited export makes ownership rules decorative. | User | `/ideate-validate` |
| Q-04 | **How far does the Operator's stake extend?** Venues demonstrably run mailing lists (20.01), broadcast (20.03) and sell merch (20.04). Memberships (20.05) are marked `None` but grassroots supporter schemes are real. Either domain 20 serves venues too, or domain 16 reimplements this domain. | User | `/ideate-validate` |
| Q-05 | **Is the Operator-side demand product bigger than the artist-side one?** [20.07](./20.07-fan-demand-show-requests.md) DT-03 — "which artist's audience is already outside my door" has no incumbent and hits the Operator's stated core pain (a perishable calendar) directly. | User | `/ideate-validate` |
| Q-06 | **Does the fan-side product need a mobile surface?** [20.03.02](./20.03-broadcast-fan-messaging/20.03.02-channel-routing-bulk-delivery.md) DT-02 and [20.06.02](./20.06-fan-experience-discovery/20.06.02-gig-alerts-near-me.md) DT-01 both land here: gig alerts are a time-critical push product, and iOS web push effectively requires a home-screen install nobody does. D-13 predicted this; domain 20 is the concrete evidence. Reopens `meta/constraints.md` Project Surfaces. | User | `/ideate-validate` |
| Q-07 | **No rights record on a D2F product: hard-block or 100%-to-lister?** [20.04.04](./20.04-direct-to-fan-storefront/20.04.04-d2f-revenue-split-payout.md) DT-02/Q-02. Hard-block is thesis-pure and empties the launch catalogue (every track predates the platform). 100%-with-acknowledgement fills it and makes splits optional. **The most consequential open question in the domain.** | User | `/ideate-validate` |
