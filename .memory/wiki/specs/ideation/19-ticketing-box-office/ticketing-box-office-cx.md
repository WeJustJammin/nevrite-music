...
| CX-04 | [19.04 Door Scanning](./19.04-door-scanning-access-control/) | [19.05 Counts & Drops](./19.05-box-office-counts-drops/) | **Hard gate**: the box office cannot close while any scanning device is unreconciled — the gate `scanned` count would be short by an unknown number, and the derived total with it. The reconciled, deduplicated gate count is **one component** of the number the fire officer and settlement read: with the window `admitted` counter it composes `admissions_total`, whose paid-only projection `admissions_paid` is settlement's verified draw | Operator, Musician | High | 19.04.02 D-04, 19.05.05 D-02, **19.05.01 D-09**; intra 19.04.02→19.05.05, 19.04.02→19.05.01, 19.04.01→19.05.01 |
| CX-05 | [19.04 Door Scanning](./19.04-door-scanning-access-control/) | [19.07 External Integration](./19.07-external-ticketing-integration/) | **The domain's most consequential internal link**: the gate-observed `scanned` is the only count WeJammin independently observes — the derived `admissions_total` is not, because it contains WeJammin's own window sales — so reconciling a foreign count is only verification (not transcription) if WeJammin runs the door | Musician, Operator | High | 19.07.03 DT-01 and D-07; the door and reconciliation stand or fall together |
...
| CX-14 | [19.04 Door Scanning](./19.04-door-scanning-access-control/) | [19.06 Refunds & Reschedule](./19.06-refunds-cancellations-rescheduling/) | Refunds and transfers landing after the door replica's last sync are the main source of staleness *with money attached* — they set the stakes for the amber staleness threshold (19.04.02 D-11) and can produce admitted-then-refunded anomalies at the lane. Staleness warns; it never blocks and never refuses | Operator, Fan, Musician | High | 19.04.02→19.06.01 (state-race); admitted-vs-refunded is a settlement discrepancy |
...
**Relationship**: A comp draws a unit from the artist hold (or a comp-designated block for reserved seating, to preserve sellable adjacency, D-06) at a bound price level, decrementing the manifest's sellable count and incrementing comp — without touching paid. The separate-counter model — paid, comp, and admissions split into the gate-observed `scanned` plus the window `admitted`, with `admissions_total` derived from both (19.05.01 D-09) — exists precisely because this decrement must not read as a sale.
...
**Relationship**: The certified statement (19.05.05) is this domain's output and settlement's input. It cannot be produced while any scanning device still holds unpushed scans — the gate `scanned` count would be short by an unknown quantity, and "unknown" is what a certification exists to eliminate.

**Corrected by owner decision DQ-17.1 / DQ-17.2 (2026-07-22)**: this entry previously read *"the reconciled, deduplicated scanned count is simultaneously the fire officer's capacity number and settlement's admitted number."* That is now **false as written**. `scanned` is **gate-observed admissions only**; a walk-up buyer is admitted at birth by the selling device and never scans (19.05.04 D-01), and walk-up is 20–50% of the gate at the beachhead venues. The fire officer's capacity number and settlement's admitted number are the **derived `admissions_total`** — reconciled gate `scanned` (net of reversals) plus window `admitted` (net of un-admissions) — and settlement's draw is its paid-only projection `admissions_paid` (19.05.01 D-09). The hard gate is unchanged: an unreconciled device leaves both the gate count and the derived total short, and the derived total additionally inherits the stalest input's freshness (19.05.01 D-10).
...
**Synthesis questions answered**:
1. **Shared state conflict**: The gate `scanned` counter. Devices write asynchronously and are the authority during the show (19.04.02 D-02); the close needs it final. The scanned counter reads net of reversals (accepts − reversals per D-07), not raw accepts. The window's `admitted` counter is a separate store with a separate writer, so the two never contend — they compose into the derived total (19.05.01 D-09).
...
**Relationship**: Door-time comps (including after-close over-allocations) mutate the live count while the box office is being closed and settlement computed from it. Paid, comp, the gate-observed `scanned`, the window `admitted` and the derived `admissions_total` are each certified lines, plus a facility-fee line (19.05.05, 19.05.01 D-09); a comp issued after close forces re-certification with all parties notified.
...
**Relationship**: Refunds and transfers landing after the door replica's last sync are the main source of staleness *with money attached*. They set the stakes for the amber staleness threshold (19.04.02 D-11, which authors the rule for the whole sub-domain): a fan refunded online after the replica synced still scans as valid at a stale door, and an already-admitted-then-refunded ticket is a real settlement discrepancy (paid drops, gate `scanned` unchanged — the exit-scoped rule at 19.05.01 D-11).
...
**Synthesis questions answered**:
1. **Shared state conflict**: The ticket's validity state. The refund writes online; the door reads a stale replica. The replica's freshness gate (amber at 60 min, per-show override, 19.04.02 D-11) **surfaces** the exposure; it does not bound it — under DQ-16.3 the amber is a warning only, and no source defines an escalation from an ignored one (19.04.02 Q-06).
2. **Trigger chain**: Refund/transfer online → replica staleness grows → the door **admits** on a stale replica (admit-and-reconcile) and raises an amber warning. It never refuses on age, however stale, and readiness is never withheld on age (19.04.02 D-11; 19.04.01 D-06, DT-04). Refund provenance (paid down, gate `scanned` unchanged if already admitted) is a discrepancy settlement must carry (19.05.01 D-11, 19.05.04).
3. **Permission intersection**: No.
