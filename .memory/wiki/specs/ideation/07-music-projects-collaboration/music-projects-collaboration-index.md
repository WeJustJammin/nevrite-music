...
| Q-02 | **[OWNER]** ⚠️ **PARTIALLY RESOLVED 2026-07-22 → D-09.** *(Four features independently argue for a phone.)* **Answered**: `07.06.02` Q-04/Q-09 (the close prompt fires while packing up) and `07.09.03` Q-03 (push may beat a desktop notification) — both are closed by D-09, which assigns PWA web push + in-app as the v1 delivery using the capability `meta/constraints.md`'s Mobile (PWA) row and `ideation-index.md` D-28 already lock into v1. The mobile-surface question is therefore **not** reopened: v1 = web + PWA, native mobile stays phase 2, classification stays `single-surface`. **Still open**: `07.02.01` DT-02 (idea capture vs a lock-screen voice memo) and `07.02.03` DT-01 (charts on a music stand) — neither is a notification-delivery question and neither is answered by D-09. **Re-pointed 2026-07-23**: `/ideate-validate` has run and did not take this decision. The *surface* axis is in fact already settled — [`ideation-index.md`](../ideation-index.md) **D-28** (User, 2026-07-18): *"**Mobile** = native app is **phase 2** (v1 = web + PWA; classification stays `single-surface`, native tracked as a future surface…)"* — so no native surface exists in the v1 window under any reading. What is left is an owner **scope** call that no later stage takes on its own: do these two features ship knowingly weak on web + PWA in v1, or wait for the phase-2 native surface? Both source rows ([`07.02.01`](./07.02-songwriting-composition-workspace/07.02.01-idea-capture-inbox.md) Q-01, [`07.02.03`](./07.02-songwriting-composition-workspace/07.02.03-chord-arrangement-chart-workspace.md) Q-01) are already `[OWNER]` and re-pointed to `/create-prd` on the same reasoning. | User | `/create-prd` |
| Q-03 | ✅ **RESOLVED 2026-07-23.** *(Was: does readiness (`07.08.03`) ever hard-block? Candidates for a legitimate block: DSP release (domain 12) and remix publication (`07.08` CX-05).)* **Answer**: no — the score never blocks anything, and both candidate gates turn out to be owned elsewhere, so D-04 holds without exception. [`07.08` index Q-01](./07.08-delivery-readiness-qc/07.08-delivery-readiness-qc-index.md): *"**Resolved: no — the score itself never blocks, and both candidate gates are owned elsewhere.** … `07.08.03` D-08: '07 does not inherit 12's authority to hold dispatch — 12 may gate, 07 may not', and `07.08.05` D-04 blocks a **public remix publication** on source-declaration verification. The sub-domain's only hard stops are therefore asset-integrity failure (`07.08.01` D-04) and undeclared sources before publication (`07.08.05` D-04) — neither is the score. The debt is acted on because the user wants to ship (`07.08.03` DT-02), never because it bars them."* Restated at feature level in [`07.08.03`](./07.08-delivery-readiness-qc/07.08.03-metadata-completeness-readiness-score.md) Q-01. | — | Closed |
| Q-04 | ✅ **RESOLVED 2026-07-23.** *(Was: **work vs recording** (`07.01.01` Q-01) — is a Song one entity with typed versions, or are `work` (ISWC) and `recording` (ISRC) both first-class?)* **Answer**: both are first-class, they live in domain 09, and the Song is neither. [`07.01` index Q-01](./07.01-song-release-production-board/07.01-song-release-production-board-index.md): *"**Resolved — each fixation is its own recording of one work; the Song models neither.** … Also closes 07.01.01 Q-01 and domain Q-04."* It rests on [`09.01.01`](../09-rights-ownership/09.01-rights-registry/09.01.01-work-recording-duality.md) **D-04** (*"**Two link classes**, not one overloaded type: Recording→Work (typed, weighted) and Recording→Recording lineage (kinded)"*) and **D-09** (*"A recording exists here only by **explicit assertion**, never by promotion of a session artifact"*), with [`07.01.01`](./07.01-song-release-production-board/07.01.01-song-record-work-entity.md) **D-04** (*"**The Song is not a rights object.** It owns pointers, never percentages, links, weights or ledgers"*) and `07.04.01` **D-14** carrying each lineage edge's `same-recording` / `new-recording` character. **Residual**: the storage representation only (does a recording get its own row, or is it a computed cluster over lineage edges) — tracked as `07.04.01` Q-02, already pointed at `/create-prd-architecture`. | — | Closed (storage representation → `07.04.01` Q-02) |
| Q-05 | ✅ **RESOLVED behaviorally 2026-07-23.** *(Was: is a session the same entity as a studio booking (16) and a service order (05) — one entity with three lenses, or three with a shared key?)* **Answer**: three, never reconciled. [`07.06.01`](./07.06-sessions-documentation-recall/07.06.01-session-record-attendance.md) **D-07**: *"A session, a **booking** (16.06) and a **service order** (05.03) are **three distinct facts** linked `0..1 : 0..N`, never reconciled … Cardinality, lifecycle, parties and visibility all contradict a merge."* Its DT-04 rejects the one-entity reading on six independent grounds (most sessions have neither; most bookings are not sessions; one booking holds two sessions; a booking can be cancelled and a session cannot be un-happened; the parties differ; the Operator's visibility is irreconcilable with R-01). Ratified at sub-domain level as [`07.06` index Q-01](./07.06-sessions-documentation-recall/07.06-sessions-documentation-recall-index.md) ✅. **Still open**: the storage representation of the link only, which that decision itself routes onward. | User | `/create-prd-architecture` (link representation only) |
| Q-06 | ✅ **RESOLVED 2026-07-23.** *(Was: is the beachhead in this domain — `07.05.02` private review links, or `07.05.01` timestamped review?)* **Answer**: yes, 07 is inside the v1 wedge, and the named v1 capture moment is `07.05.02`. [`ideation-index.md`](../ideation-index.md) **D-31** (User, 2026-07-18): *"**v1 = session spine** (01, 02, 05, 07, 09-capture) ~45 Musts — the wedge, shipped first and fast"*; **D-70(d)**: *"v1 captures at the **first sharing moment** (review link `07.05.02` + close prompt); capture-at-source is the **direction**, not the current claim"* — restated locally as D-11. The two candidates are not either/or: both survived MoSCoW as **Must** ([moscow-ledger.md](../moscow-ledger.md) § MUST — `07.05.02` *"sending a mix for review is the MOST FREQUENT thing a producer does with a finished bounce"*; `07.05.01` *"the door, the daily hook of D-18's consolidation half"*). Precedent for reading D-31 as the beachhead answer: `04.02.03` Q-01 (*"the beachhead question closed on the session spine"*) and `21` Q-09 (*"the beachhead is decided and is not this domain"*). | — | Closed |
| Q-07 | ✅ **RESOLVED 2026-07-23.** *(Was: is the provenance graph exportable (`07.08.01` Q-02)? If credits, splits, attestations and lineage are not exportable in an open form, the lock-in is hostile.)* **Answer**: yes — always, in full, and owned outside this domain, so the lock-in is **earned**. [`07.08.01` Q-02](./07.08-delivery-readiness-qc/07.08.01-handoff-package-builder-recipient-spec.md): *"**Resolved: yes, and it is owned outside this feature.** `02.08 Credit Export & DDEX RIN Emission` is exactly the missing half"*, citing [`02.08`](../02-credits-attribution/02.08-credit-export-ddex-rin.md) **D-01** — *"Full, frictionless portability export — always … Any friction here converts earned lock-in into hostile"* — and **D-06** (*"portability export (`D-01`) always includes everything"*), with DDEX RIN as the open emission form. This domain originates the `archive` spec's **asset half only** (`07.08.01` D-06); the provenance-graph half is `02.08`'s. | — | Closed |
...
| Q-09 | **[OWNER]** **Do specialist engineers need their own persona?** `07.07.03` Q-02 (mastering) and `07.07.05` Q-03 (Atmos) both argue that distinct professionals are folded into "Producer". Two features raising it independently echoes `meta/personas.md` Q-01's dealer question. **Re-pointed 2026-07-23**: `/ideate-validate` has run and did not take this decision. The four-persona set is owner-ratified ([`meta/personas.md`](../meta/personas.md): *"4 primary personas confirmed by owner 2026-07-16 (D-19)"*) and **only the owner can add a fifth** — no later stage resolves it on its own. Both source rows (`07.07.03` Q-02, `07.07.05` Q-03) and `meta/personas.md` Q-01/Q-05 are already `[OWNER]` and re-pointed to `/create-prd` on the same reasoning; this row joins them. | User | `/create-prd` |
| Q-10 | ✅ **RESOLVED 2026-07-23.** *(Was: `07.07.04` DT-01 and `07.07.05` DT-03 both argue themselves out of existence; both proposed `Won't`.)* **Answer**: the MoSCoW pass ran and ratified both proposals. [moscow-ledger.md](../moscow-ledger.md) § **WONT (53)** carries `07.07.04` — *"does a DDP capture a fact at the moment of creation that would otherwise be reconstructed from memory? No — nobody litigates a DDP. Tiny population"* — and `07.07.05` — *"**Wont for the FEATURE**, but DT-01's parallel-lineage DATA MODEL is a Must regardless — it is cheap now and expensive to retrofit, while the feature is expensive now and cheap later."* The row's wider signal is confirmed by the ledger too: the three surviving `07.07` features (`07.07.01`, `07.07.02`, `07.07.03`) all sit in **COULD**, so the sub-domain carries **no Must at all**. **Carried forward**: the Atmos parallel-lineage data model survives its feature's rejection and must not be dropped with it. | — | Closed |
