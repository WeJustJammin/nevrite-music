...
| Q-06 | ~~**Does money move through the platform on local pickup?**~~ (13.11 Q-01) **RESOLVED by [13.11](./13.11-local-pickup-meetup-safety.md) D-04 (P-09) — the seller chooses per listing**, platform-settled or off-platform, never a single global rule. A global platform-settled rule would impose marketplace-facilitator sales-tax and 1099-K obligations on every cash handshake; a global off-platform rule would strand the escrow, evidence and ownership-chain machinery `13.06.02`, `13.02.04`, `13.06.05` and `13.08.01` already specify for the settled branch. **The chain follows the money**: a settled pickup gets the normal `13.06.05` transfer write, an off-platform one has no settlement event to hang a transfer on and falls back to the manual handshake `15.01.03` D-01 names — so the hole at the category's most common transaction shape is bounded to one disclosed branch rather than global. Residuals ride on their own rows, not here: presentation of the off-platform provenance gap ([13.11](./13.11-local-pickup-meetup-safety.md) Q-04), settled-branch mechanics — inspection window, pickup confirmation, tax situs ([13.11](./13.11-local-pickup-meetup-safety.md) Q-05), the cash-settled evidence tier ([13.06.05](./13.06-cart-checkout-orders/13.06.05-ownership-transfer-on-settlement.md) Q-07), and whether a pickup line may share a cart with a shipped one ([13.06.01](./13.06-cart-checkout-orders/13.06.01-cart-multivendor-checkout.md) Q-04). | Owner | ✅ `/ideate-validate`, 2026-07-21 |
