...
| CX-24 | [14.03 Delivery & Library](./14.03-delivery-versioning-library/) | [14.04 Sound Content](./14.04-sound-content-catalogs/) | Audio must **carry** embedded tempo/key/loop metadata (ACIDized-WAV / Apple-Loops) — a DAW auto-stretches only if the file says its tempo. **Not two writers on one artifact**: the metadata write moved to ingest (`14.08.03`, into the stored master); the per-buyer watermark is delivery's only write, and only where enabled. Unmarked artifacts are served byte-identical to the master. Pack revision is append-and-correct for existing buyers; a metadata correction re-writes the master and its hash. | Musician, Producer | High | 14.04.01 DT-03 / D-06 · 14.03.05 · 14.03.02 D-05 · **re-scoped by `14.03.01` D-12 (2026-07-30)**. |
...
### CX-24: Delivery & Library ↔ Sound Content (Who Writes the Artifact, and When)

**Relationship**: Audio must **carry** embedded tempo/key/loop metadata (ACIDized-WAV / Apple-Loops convention) — a DAW auto-stretches a loop only if the file says its tempo (14.04.01 DT-03). ~~This write lands on the *same* delivery step as per-buyer forensic watermarking: **two writers on one artifact**.~~ **Re-scoped 2026-07-30 per `14.03.01` D-12** (owner ratification of run-3 finding `14-digital-goods-marketplace#001`): it is **not two writers on one artifact**, and modelling it as one relationship was the error that let a blocking contradiction survive. The two writes are unlike things. Metadata is a property of the **product** — "this loop is 142 BPM" is true for every buyer — so it is written **once at ingest**, by `14.08.03` audio QC, into the stored master (14.04.01 D-06 as amended). The watermark is a property of the **buyer** and is the only thing delivery writes, where enabled. This cross-cut is therefore a **catalog↔ingest** relationship on the metadata side and a **delivery↔integrity** one on the mark side, and its consequence is the contract in `14.03.01` D-12: an unmarked artifact is served byte-identical to its master, so the published SHA-256 is correct for it; only a marked artifact carries a second, per-grant hash. Pack revision is append-and-correct for existing buyers, versioned for new ones — a revision never removes a held asset (14.03 CX-06) — and a **metadata correction is now a master re-write with a new hash** (`14.03.02` D-05).
...
**Synthesis questions answered**:
1. **Shared state**: The **stored master's** bytes, plus — for marked products only — the per-grant bytes derived from them. **One writer per stage, never two on one step** (`14.03.01` D-12): `14.08.03` writes metadata into the master at ingest; `14.03.05` stamps the mark at delivery. There is no composition-ordering problem left to solve and no requirement that the mark "survive the metadata write", because the metadata is already in the master the stamper reads. The mark must still survive time-stretch and the other operations sample use consists of — that is `14.03.05` Q-01, untouched by this and still open.
2. **Trigger chain**: **Ingest**: content submitted → QC analyses and reconciles → tempo/key/loop metadata embedded into the master → master hashed and published. **Delivery**: grant issued → apply per-buyer watermark (**if the vendor enabled it**) → serve. Where marking is off, delivery is `grant issued → serve` and the bytes are byte-identical to the master. Ordering still matters within the mark: it is deterministic per grant (`14.03.01` DT-03) so resume can realign, and that determinism is also what makes a per-grant hash computable.
3. **Permission intersection**: The metadata write is a **vendor-side** action inside the submission gate (`14.08.01`/`14.08.03`) and needs no buyer at all. Only the mark happens under an entitled grant, and it is not a separate permission from delivery's.
...
| Edge/CDN strategy for multi-GB assets under per-buyer watermarking | `/create-prd-architecture` | CX-24: a **marked** artifact is per-buyer and therefore uncacheable, colliding with the locked Cloudflare constraint on the largest files. Narrowed by `14.03.01` D-12 — the metadata write is no longer on this path, so the uncacheable set is exactly the watermark-enabled inventory, and `14.03.05` Q-03 (per-product opt-in) decides how large that set is. |
| Per-grant SHA-256 over a stamped multi-GB stream (hash-and-trailer) | `/create-prd-architecture` | **New, from `14.03.01` D-12.** A watermarked artifact's delivered bytes have their own hash, which the client verifies. On a 100 GB library those bytes cannot be materialised twice, so the digest must be computed while serving and delivered at the end of the stream — and must survive resume, since a grant's ranges are not one continuous read. Bounded to marked products; unmarked ones need none of it. |
