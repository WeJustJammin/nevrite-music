# Career finance and business operations: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [41a-income-tax-receivables.md](../be/41a-income-tax-receivables.md), [41b-deals-recoupment-pl.md](../be/41b-deals-recoupment-pl.md)
> **IA Source**: [41-career-finance.md](../ia/41-career-finance.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [41-career-finance.md](../ia/41-career-finance.md) in full.
- **BE sources**: [41a-income-tax-receivables.md](../be/41a-income-tax-receivables.md), [41b-deals-recoupment-pl.md](../be/41b-deals-recoupment-pl.md).
...
|---|---|---|
| Classification and scope | `41-career-finance.md`; BE index | Shard boundary and completed BE split group |
...
### `CareerFinanceRoute` (Astro server route)

```ts
interface CareerFinanceRouteProps {
  children?: never;
...
### `IncomeTaxReceivablesWorkbench` (bounded React island)

**BE owner**: `41a-income-tax-receivables.md`

```ts
interface IncomeTaxReceivablesWorkbenchProps {
  contractFields: IncomeTaxReceivablesWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly IncomeTaxReceivablesRecord[]>;
  actorId: string;
...
interface IncomeTaxReceivablesRecord {
  id: string;
  version: string;
  state: string;
...
### `DealsRecoupmentPlWorkbench` (bounded React island)

**BE owner**: `41b-deals-recoupment-pl.md`

```ts
interface DealsRecoupmentPlWorkbenchProps {
  contractFields: DealsRecoupmentPlWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly DealsRecoupmentPlRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface DealsRecoupmentPlRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/career-finance` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/career-finance/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `41-career-finance.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `41-career-finance.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `41-career-finance.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `41-career-finance.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `41-career-finance.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `41-career-finance.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `41-career-finance.md` Accessibility |
...
|---|---|---|---|---|
| `CareerFinanceRoute` | `CareerFinanceRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `41-career-finance.md` user flows/accessibility; design-system page archetypes |
| `IncomeTaxReceivablesWorkbench` | `IncomeTaxReceivablesWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `41a-income-tax-receivables.md` request/response fields; `41-career-finance.md` interactions/access rules |
| `DealsRecoupmentPlWorkbench` | `DealsRecoupmentPlWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `41b-deals-recoupment-pl.md` request/response fields; `41-career-finance.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `41.01` | Ingest income event | `CareerFinanceRoute` | Trusted source event/idempotency key | Immutable verified/observed row appends | Late reversal parks; duplicate proposes reconciliation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.02` | Import off-platform income | `CareerFinanceRoute` | Holder and supported file/manual entry | Declared rows preview then commit | Overlap stays visible until holder decision | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.03` | View FX projection | `CareerFinanceRoute` | Native row and selected reporting currency/date policy | Estimated/actual conversion shows provenance | Missing rate yields unresolved, blocks issued statement | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.04` | Issue income statement | `CareerFinanceRoute` | Holder/delegate, clean period and complete FX | Immutable snapshot and verification page publish | Duplicate/FX blocker refuses issuance | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.05` | Link income to work | `CareerFinanceRoute` | Earner permission and candidate work/credit | Read projection attributes without moving facts | Credit revoke degrades attribution, never deletes income | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.06` | Capture expense/receipt | `CareerFinanceRoute` | Holder and purchase/upload | Evidence row and confirmed fields append | Extraction failure retains receipt for manual completion | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.07` | Build tax pack | `CareerFinanceRoute` | Supported jurisdiction/tax year and reconciled records | Immutable evidence pack plus exceptions issues | Unsupported rules/blockers refuse with no generic advice | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.08` | Track withholding/reclaim | `CareerFinanceRoute` | Income deduction, territory and confirmed deadline | Preparation checklist/alert saves | No filing/submission or legal eligibility claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.09` | Send/revise quote | `CareerFinanceRoute` | Authorized supplier entity | Immutable sent version and public recipient link | Revision creates successor; expiry only warns | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.10` | Issue/correct invoice | `CareerFinanceRoute` | Accepted terms/customer fields and sequence lock | Numbered immutable invoice or linked credit note issues | Missing required fiscal field blocks issuance | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.11` | Run dunning ladder | `CareerFinanceRoute` | Opt-in invoice, unpaid fact and no exclusion | Neutral reminder sends and appends evidence | Payment/unknown status halts; failed send stays explicit | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.12` | Store contract instrument | `CareerFinanceRoute` | Authorized party and document evidence | Immutable instrument links into deal chain | Delegation scoped per document; replacement never overwrites | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.13` | Confirm key term | `CareerFinanceRoute` | Cited extraction/manual entry and authorized reviewer | Confirmed term version activates dependent alerts | Ambiguous/unconfirmed term remains non-authoritative | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.14` | Reconcile deal to facts | `CareerFinanceRoute` | Confirmed term and current rights/financial projection | Cited observation/absence/conflict renders | No auto-correction or legal conclusion | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.15` | Prepare advance referral | `CareerFinanceRoute` | Holder opt-in and sufficient verified statement | Lender-facing consented package/cost comparison prepares | Declared income excluded; platform never approves loan | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.16` | Calculate commission/recoupment | `CareerFinanceRoute` | Confirmed terms and ordered income events | Clause-cited calculation/reversals append | Ambiguous scope/sequence holds row | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.17` | View runway | `CareerFinanceRoute` | Holder opt-in and confidence-sufficient net cashflow | Range and gap lead time render with action routes | Low confidence withholds reassurance | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.18` | View/close P&L | `CareerFinanceRoute` | Authorized project/tour/band and canonical rows | Provisional view or immutable closing version | Untagged/unreconciled rows remain visible/block per policy | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `41.19` | Record band allocation | `CareerFinanceRoute` | Confirmed governance rule, close version and member debts | Record-only allocation evidence appends | No transfer; missing rule blocks distribution calculation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/career-finance` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fcareer-finance` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `CareerFinanceRoute` variant `appPage` | `Career finance and business operations | WeJammin` | `Work with career finance and business operations using current authority, record state, and provenance.` |
| `/app/career-finance/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `CareerFinanceRoute` with the matching workbench detail variant | `Record | Career finance and business operations | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `CareerFinanceRoute` variant `publicPage` | `Career finance and business operations | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `CareerFinanceRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `CareerFinanceRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `IncomeTaxReceivablesWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `DealsRecoupmentPlWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `41a-income-tax-receivables.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `41b-deals-recoupment-pl.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `41a-income-tax-receivables.md` | `41A-INCOME-TAX-RECEIVABLES-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `IncomeTaxReceivablesWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `41b-deals-recoupment-pl.md` | `41B-DEALS-RECOUPMENT-PL-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `DealsRecoupmentPlWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `41a-income-tax-receivables.md` | Named source schemas | `FxProjectionParams`, `FxProjectionResult`, `FxProjectionV1` | `IncomeTaxReceivablesWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `41b-deals-recoupment-pl.md` | Named source schemas | `RunwayQuery`, `actionRouteCodes` | `DealsRecoupmentPlWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type IncomeTaxReceivablesContractField =
  | 'FxProjectionParams'
  | 'FxProjectionResult'
  | 'FxProjectionV1';
interface IncomeTaxReceivablesWorkbenchContractFields {
  source: '41a-income-tax-receivables.md';
  fields: Readonly<Record<IncomeTaxReceivablesContractField, unknown>>;
}
```

```ts
type DealsRecoupmentPlContractField =
  | 'RunwayQuery'
  | 'actionRouteCodes';
interface DealsRecoupmentPlWorkbenchContractFields {
  source: '41b-deals-recoupment-pl.md';
  fields: Readonly<Record<DealsRecoupmentPlContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `41a-income-tax-receivables.md` | `IncomeTaxReceivablesWorkbenchContractFields.fields` and `IncomeTaxReceivablesWorkbenchProps.contractFields` | `CONFLICT`, `CREDIT_REVOKED`, `DEADLINE_SOURCE_REQUIRED`, `DEDUCTION_MISMATCH`, `DELIVERY_UNAVAILABLE`, `DEPENDENCY_UNAVAILABLE`, `EVIDENCE_DIGEST_MISMATCH`, `EXTRACTION_UNAVAILABLE`, `FISCAL_FIELD_REQUIRED`, `FORBIDDEN`, `FX_RATE_UNRESOLVED`, `IDEMPOTENCY_MISMATCH`, `IMPORT_HASH_MISMATCH`, `IMPORT_PREVIEW_EXPIRED`, `INVALID_REQUEST`, `INVALID_TRANSITION`, `NOT_FOUND`, `OVERLAP_DECISION_REQUIRED`, `PROJECTION_INPUT_INVALIDATED`, `RATE_LIMITED`, `RECIPIENT_REQUIRED`, `RECONCILIATION_BLOCKED`, `SOURCE_EVENT_CONFLICT`, `STEP_UP_REQUIRED`, `TAX_INPUT_BLOCKED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_MISMATCH` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `41b-deals-recoupment-pl.md` | `DealsRecoupmentPlWorkbenchContractFields.fields` and `DealsRecoupmentPlWorkbenchProps.contractFields` | `ALLOCATION_TOTAL_MISMATCH`, `CITATION_INVALID`, `CLOSING_DIGEST_MISMATCH`, `CONFLICT`, `CONSENT_SCOPE_MISMATCH`, `DEPENDENCY_UNAVAILABLE`, `DOCUMENT_DIGEST_MISMATCH`, `FACT_DEPENDENCY_UNAVAILABLE`, `FINAL_CLOSE_REQUIRED`, `FORBIDDEN`, `FX_RATE_UNRESOLVED`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `INVALID_TRANSITION`, `NOT_FOUND`, `PREDECESSOR_REQUIRED`, `PROVIDER_DISCLOSURE_UNAVAILABLE`, `RATE_LIMITED`, `REVIEW_AUTHORITY_REQUIRED`, `STEP_UP_REQUIRED`, `TERM_VALUE_KIND_MISMATCH`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERIFIED_SNAPSHOT_REQUIRED`, `VERSION_MISMATCH` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
