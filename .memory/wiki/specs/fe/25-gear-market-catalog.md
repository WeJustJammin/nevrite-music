# Gear catalog, listings and market data: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [25a-gear-catalog-authority-matching.md](../be/25a-gear-catalog-authority-matching.md), [25b-gear-listing-disclosure-lifecycle.md](../be/25b-gear-listing-disclosure-lifecycle.md), [25c-gear-inventory-bulk-channels.md](../be/25c-gear-inventory-bulk-channels.md), [25d-gear-market-guides-storefront-policies.md](../be/25d-gear-market-guides-storefront-policies.md)
> **IA Source**: [25-gear-market-catalog.md](../ia/25-gear-market-catalog.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [25-gear-market-catalog.md](../ia/25-gear-market-catalog.md) in full.
- **BE sources**: [25a-gear-catalog-authority-matching.md](../be/25a-gear-catalog-authority-matching.md), [25b-gear-listing-disclosure-lifecycle.md](../be/25b-gear-listing-disclosure-lifecycle.md), [25c-gear-inventory-bulk-channels.md](../be/25c-gear-inventory-bulk-channels.md), [25d-gear-market-guides-storefront-policies.md](../be/25d-gear-market-guides-storefront-policies.md).
...
|---|---|---|
| Classification and scope | `25-gear-market-catalog.md`; BE index | Shard boundary and completed BE split group |
...
### `GearMarketCatalogRoute` (Astro server route)

```ts
interface GearMarketCatalogRouteProps {
  children?: never;
...
### `GearCatalogAuthorityMatchingWorkbench` (bounded React island)

**BE owner**: `25a-gear-catalog-authority-matching.md`

```ts
interface GearCatalogAuthorityMatchingWorkbenchProps {
  contractFields: GearCatalogAuthorityMatchingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearCatalogAuthorityMatchingRecord[]>;
  actorId: string;
...
interface GearCatalogAuthorityMatchingRecord {
  id: string;
  version: string;
  state: string;
...
### `GearListingDisclosureLifecycleWorkbench` (bounded React island)

**BE owner**: `25b-gear-listing-disclosure-lifecycle.md`

```ts
interface GearListingDisclosureLifecycleWorkbenchProps {
  contractFields: GearListingDisclosureLifecycleWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearListingDisclosureLifecycleRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearListingDisclosureLifecycleRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `GearInventoryBulkChannelsWorkbench` (bounded React island)

**BE owner**: `25c-gear-inventory-bulk-channels.md`

```ts
interface GearInventoryBulkChannelsWorkbenchProps {
  contractFields: GearInventoryBulkChannelsWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearInventoryBulkChannelsRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearInventoryBulkChannelsRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `GearMarketGuidesStorefrontPoliciesWorkbench` (bounded React island)

**BE owner**: `25d-gear-market-guides-storefront-policies.md`

```ts
interface GearMarketGuidesStorefrontPoliciesWorkbenchProps {
  contractFields: GearMarketGuidesStorefrontPoliciesWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearMarketGuidesStorefrontPoliciesRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearMarketGuidesStorefrontPoliciesRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/gear-market-catalog` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/gear-market-catalog/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `25-gear-market-catalog.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `25-gear-market-catalog.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `25-gear-market-catalog.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `25-gear-market-catalog.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `25-gear-market-catalog.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `25-gear-market-catalog.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `25-gear-market-catalog.md` Accessibility |
...
|---|---|---|---|---|
| `GearMarketCatalogRoute` | `GearMarketCatalogRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `25-gear-market-catalog.md` user flows/accessibility; design-system page archetypes |
| `GearCatalogAuthorityMatchingWorkbench` | `GearCatalogAuthorityMatchingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `25a-gear-catalog-authority-matching.md` request/response fields; `25-gear-market-catalog.md` interactions/access rules |
| `GearListingDisclosureLifecycleWorkbench` | `GearListingDisclosureLifecycleWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `25b-gear-listing-disclosure-lifecycle.md` request/response fields; `25-gear-market-catalog.md` interactions/access rules |
| `GearInventoryBulkChannelsWorkbench` | `GearInventoryBulkChannelsWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `25c-gear-inventory-bulk-channels.md` request/response fields; `25-gear-market-catalog.md` interactions/access rules |
| `GearMarketGuidesStorefrontPoliciesWorkbench` | `GearMarketGuidesStorefrontPoliciesWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `25d-gear-market-guides-storefront-policies.md` request/response fields; `25-gear-market-catalog.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `25.01` | Search/browse catalog | `GearMarketCatalogRoute` | Public request with bounded filters | Versioned models, aliases and active facets render | Search failure offers exact retry; no fabricated matches | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.02` | Create provisional model inline | `GearMarketCatalogRoute` | Seller has a real listing/record context | Provisional model publishes with assertion provenance | Duplicate candidate routes to reversible resolution; listing may remain unmatched | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.03` | Contribute catalog fact | `GearMarketCatalogRoute` | Authenticated contributor; category schema permits field | Attributed assertion appends and may corroborate current value | Unsupported/safety claim remains unverified and cannot suppress warnings | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.04` | Moderate catalog change | `GearMarketCatalogRoute` | Category standing, no commercial conflict, blast-radius quorum met | Versioned accept/reject/merge/split with reversible history | Recusal or quorum failure leaves contribution pending | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.05` | Match listing to model | `GearMarketCatalogRoute` | Draft/live listing and candidate graph exist | Human-confirmed or high-confidence/high-margin reversible bind | Ambiguity/unavailability degrades to unmatched; publication continues | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.06` | Decode serial/date | `GearMarketCatalogRoute` | Supported versioned decoder available | Hypotheses render with source, confidence and alternatives | Unsupported/ambiguous returns unknown; no identity mutation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.07` | Evaluate fitment/voltage | `GearMarketCatalogRoute` | Model/unit/region facts available | Advisory warning cites facts and unknown coverage | Fitment no-result suppresses a warning; voltage no-result says it could not verify; neither blocks | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.08` | Grade and disclose flaws | `GearMarketCatalogRoute` | Seller controls unit/listing and schema version loads | Required grade, structured flaws and evidence form consistent version | Contradiction blocks the edit; validator outage publishes flagged for async review | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.09` | Declare originality | `GearMarketCatalogRoute` | Category component schema loads | Original/replaced/unknown vector and deterministic nominal aggregate persist | Schema outage uses explicit partial/unknown fallback; no false original claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.10` | Create listing/media | `GearMarketCatalogRoute` | Seller facet activates; payout support checked at draft start | Idempotent draft stores versioned unit media, demo and seller assertions | Unsupported payout region blocks publish, not drafting/export | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.11` | Publish listing | `GearMarketCatalogRoute` | Payout ready; material flaws evidenced; policy snapshot valid | Listing activates, emits unit hint, starts matching/screening and public projection | Registry/matcher outage degrades; missing material evidence blocks and a confirmed theft hit holds | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.12` | Amend live listing | `GearMarketCatalogRoute` | Seller controls active listing; no incompatible claim state | New version publishes; offer/claim consequences apply by field type | Price edit with live offer rejects; material post-claim change routes to order amendment | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.13` | End/pause/relist | `GearMarketCatalogRoute` | Seller controls listing and transition is valid | Append-only lifecycle event; relist inherits unit history | Published listing never deletes; reserved listing exits only through order cancellation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.14` | Claim inventory | `GearMarketCatalogRoute` | Buyer eligible; price/listing version current | One qty-one claimant wins or counted stock decrements atomically | Loser gets designed alternatives; stale price rejects, never silently reprices | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.15` | Create bundle/parts/B-stock | `GearMarketCatalogRoute` | Seller owns each constituent or counted line | Atomic bundle references constituents; each used unit keeps its own grade | One unavailable constituent fails whole claim; no automatic split | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.16` | Bulk import/list | `GearMarketCatalogRoute` | Authorized organisation listing role | Reviewable partial-success batch preserves per-unit obligations | Invalid rows isolate; bulk defaults remain flagged and comp-weighted down | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.17` | Sync external availability | `GearMarketCatalogRoute` | Post-consumer adapter is admitted and connected | Best-effort per-channel state with explicit freshness/error | Failures surface per channel/unit; policies and disclosures never sync implicitly | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.18` | Display provenance/theft status | `GearMarketCatalogRoute` | Current owner consents; source visibility/status permits | Verified session links and scoped screening facts render without hidden counts | Ownership transfer resets owner-end publication consent; absent history renders nothing | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.19` | View price guide/comps | `GearMarketCatalogRoute` | Public request resolves a model/condition/originality bucket | Distribution/sample/period or raw examples render according to confidence | Confidence failure returns no number; unpriceable unit declines with reason | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.20` | Request price suggestion/repricing | `GearMarketCatalogRoute` | Seller controls listing; guide confidence passes | Evidence-backed range; optional bounded repricing policy may propose/act | No confidence means no suggestion; seller deviation carries no penalty | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `25.21` | Manage storefront policies | `GearMarketCatalogRoute` | Seller/principal has field authority | Versioned structured defaults inherit to future listings; order pins claim-time terms | Statutory-invalid terms do not render/apply; existing obligations survive away mode | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/gear-market-catalog` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fgear-market-catalog` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `GearMarketCatalogRoute` variant `appPage` | `Gear catalog, listings and market data | WeJammin` | `Work with gear catalog, listings and market data using current authority, record state, and provenance.` |
| `/app/gear-market-catalog/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `GearMarketCatalogRoute` with the matching workbench detail variant | `Record | Gear catalog, listings and market data | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `GearMarketCatalogRoute` variant `publicPage` | `Gear catalog, listings and market data | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `GearMarketCatalogRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |
...
| `GearCatalogAuthorityMatchingWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearListingDisclosureLifecycleWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearInventoryBulkChannelsWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearMarketGuidesStorefrontPoliciesWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
| `GearCatalogAuthorityMatchingWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `GearListingDisclosureLifecycleWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `GearInventoryBulkChannelsWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `GearMarketGuidesStorefrontPoliciesWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `25a-gear-catalog-authority-matching.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `25b-gear-listing-disclosure-lifecycle.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
| `25d-gear-market-guides-storefront-policies.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `25a-gear-catalog-authority-matching.md` | `25A-GEAR-CATALOG-AUTHORITY-MATCHING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearCatalogAuthorityMatchingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `25b-gear-listing-disclosure-lifecycle.md` | `25B-GEAR-LISTING-DISCLOSURE-LIFECYCLE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearListingDisclosureLifecycleWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `25c-gear-inventory-bulk-channels.md` | `25C-GEAR-INVENTORY-BULK-CHANNELS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearInventoryBulkChannelsWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `25d-gear-market-guides-storefront-policies.md` | `25D-GEAR-MARKET-GUIDES-STOREFRONT-POLICIES-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearMarketGuidesStorefrontPoliciesWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `25a-gear-catalog-authority-matching.md` | Named source schemas | `categoryId`, `makerId`, `query`, `alias`, `includeUnmatched`, `schemaVersion`, `nextCursor` | `GearCatalogAuthorityMatchingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `25b-gear-listing-disclosure-lifecycle.md` | Named source schemas | `transactionRef` | `GearListingDisclosureLifecycleWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `25c-gear-inventory-bulk-channels.md` | Named source schemas | `aggregateId`, `bodyHash` | `GearInventoryBulkChannelsWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `25d-gear-market-guides-storefront-policies.md` | Named source schemas | `listingId`, `includeEvidenceSummary`, `includeCustodyBoundary`, `provenanceSources`, `categoryKey`, `listingModelBindId`, `conditionClass`, `originalityAggregate`, `region`, `currency`, `includeExamples`, `nextCursor`, `aggregateId`, `bodyHash` | `GearMarketGuidesStorefrontPoliciesWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type GearCatalogAuthorityMatchingContractField =
  | 'alias'
  | 'categoryId'
  | 'includeUnmatched'
  | 'makerId'
  ...
  | 'query'
  | 'schemaVersion';
interface GearCatalogAuthorityMatchingWorkbenchContractFields {
  source: '25a-gear-catalog-authority-matching.md';
  fields: Readonly<Record<GearCatalogAuthorityMatchingContractField, unknown>>;
}
```

```ts
type GearListingDisclosureLifecycleContractField =
  | 'transactionRef';
interface GearListingDisclosureLifecycleWorkbenchContractFields {
  source: '25b-gear-listing-disclosure-lifecycle.md';
  fields: Readonly<Record<GearListingDisclosureLifecycleContractField, unknown>>;
}
```

```ts
type GearInventoryBulkChannelsContractField =
  | 'aggregateId'
  | 'bodyHash';
interface GearInventoryBulkChannelsWorkbenchContractFields {
  source: '25c-gear-inventory-bulk-channels.md';
  fields: Readonly<Record<GearInventoryBulkChannelsContractField, unknown>>;
}
```

```ts
type GearMarketGuidesStorefrontPoliciesContractField =
  | 'aggregateId'
  | 'bodyHash'
  | 'categoryKey'
  | 'conditionClass'
  | 'currency'
  | 'includeCustodyBoundary'
  | 'includeEvidenceSummary'
  | 'includeExamples'
  | 'listingId'
  | 'listingModelBindId'
  | 'nextCursor'
  | 'originalityAggregate'
  | 'provenanceSources'
  | 'region';
interface GearMarketGuidesStorefrontPoliciesWorkbenchContractFields {
  source: '25d-gear-market-guides-storefront-policies.md';
  fields: Readonly<Record<GearMarketGuidesStorefrontPoliciesContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `25a-gear-catalog-authority-matching.md` | `GearCatalogAuthorityMatchingWorkbenchContractFields.fields` and `GearCatalogAuthorityMatchingWorkbenchProps.contractFields` | `CANDIDATE_INVALID`, `CATALOG_UNAVAILABLE`, `DEPENDENCY_UNAVAILABLE`, `DUPLICATE_REVIEW`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `MODERATOR_STANDING_REQUIRED`, `NOT_FOUND`, `RATE_LIMITED`, `VALIDATION_FAILED`, `VERSION_MISMATCH` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `25b-gear-listing-disclosure-lifecycle.md` | `GearListingDisclosureLifecycleWorkbenchContractFields.fields` and `GearListingDisclosureLifecycleWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `INVALID_INPUT`, `LISTING_NOT_FOUND`, `LISTING_STATE_FORBIDDEN`, `LISTING_WRITE_FORBIDDEN`, `PUBLICATION_PRECONDITION_FAILED`, `RATE_LIMITED`, `STATE_CONFLICT`, `STOCK_UNAVAILABLE`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `25c-gear-inventory-bulk-channels.md` | `GearInventoryBulkChannelsWorkbenchContractFields.fields` and `GearInventoryBulkChannelsWorkbenchProps.contractFields` | `BUNDLE_COMPONENT_CONFLICT`, `CHANNEL_RESPONSE_INVALID`, `CHANNEL_TARGET_NOT_FOUND`, `CLAIM_EXPIRED`, `DEPENDENCY_UNAVAILABLE`, `IMPORT_ROW_INVALID`, `INVALID_INPUT`, `INVENTORY_CLAIM_FORBIDDEN`, `INVENTORY_CONFLICT`, `INVENTORY_NOT_FOUND`, `RATE_LIMITED`, `STALE_PRICE` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `25d-gear-market-guides-storefront-policies.md` | `GearMarketGuidesStorefrontPoliciesWorkbenchContractFields.fields` and `GearMarketGuidesStorefrontPoliciesWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `GUIDE_FILTER_FORBIDDEN`, `GUIDE_NOT_FOUND`, `INVALID_INPUT`, `LISTING_NOT_FOUND`, `POLICY_VERSION_CONFLICT`, `POLICY_WRITE_FORBIDDEN`, `PROVENANCE_FORBIDDEN`, `PROVENANCE_NOT_FOUND`, `RATE_LIMITED`, `STOREFRONT_NOT_FOUND`, `SUGGESTION_FORBIDDEN`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
