# Tour routing, logistics, finance and reporting: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [34a-tour-container-routing-book.md](../be/34a-tour-container-routing-book.md), [34b-tour-travel-rooming-ground-perdiem.md](../be/34b-tour-travel-rooming-ground-perdiem.md), [34c-tour-budgets-actuals-expenses.md](../be/34c-tour-budgets-actuals-expenses.md), [34d-tour-border-merch-carbon.md](../be/34d-tour-border-merch-carbon.md)
> **IA Source**: [34-touring-operations.md](../ia/34-touring-operations.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [34-touring-operations.md](../ia/34-touring-operations.md) in full.
- **BE sources**: [34a-tour-container-routing-book.md](../be/34a-tour-container-routing-book.md), [34b-tour-travel-rooming-ground-perdiem.md](../be/34b-tour-travel-rooming-ground-perdiem.md), [34c-tour-budgets-actuals-expenses.md](../be/34c-tour-budgets-actuals-expenses.md), [34d-tour-border-merch-carbon.md](../be/34d-tour-border-merch-carbon.md).
...
|---|---|---|
| Classification and scope | `34-touring-operations.md`; BE index | Shard boundary and completed BE split group |
...
### `TouringOperationsRoute` (Astro server route)

```ts
interface TouringOperationsRouteProps {
  children?: never;
...
### `TourContainerRoutingBookWorkbench` (bounded React island)

**BE owner**: `34a-tour-container-routing-book.md`

```ts
interface TourContainerRoutingBookWorkbenchProps {
  contractFields: TourContainerRoutingBookWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TourContainerRoutingBookRecord[]>;
  actorId: string;
...
interface TourContainerRoutingBookRecord {
  id: string;
  version: string;
  state: string;
...
### `TourTravelRoomingGroundPerdiemWorkbench` (bounded React island)

**BE owner**: `34b-tour-travel-rooming-ground-perdiem.md`

```ts
interface TourTravelRoomingGroundPerdiemWorkbenchProps {
  contractFields: TourTravelRoomingGroundPerdiemWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TourTravelRoomingGroundPerdiemRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TourTravelRoomingGroundPerdiemRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TourBudgetsActualsExpensesWorkbench` (bounded React island)

**BE owner**: `34c-tour-budgets-actuals-expenses.md`

```ts
interface TourBudgetsActualsExpensesWorkbenchProps {
  contractFields: TourBudgetsActualsExpensesWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TourBudgetsActualsExpensesRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TourBudgetsActualsExpensesRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TourBorderMerchCarbonWorkbench` (bounded React island)

**BE owner**: `34d-tour-border-merch-carbon.md`

```ts
interface TourBorderMerchCarbonWorkbenchProps {
  contractFields: TourBorderMerchCarbonWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TourBorderMerchCarbonRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TourBorderMerchCarbonRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/touring-operations` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/touring-operations/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `34-touring-operations.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `34-touring-operations.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `34-touring-operations.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `34-touring-operations.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `34-touring-operations.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `34-touring-operations.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `34-touring-operations.md` Accessibility |
...
|---|---|---|---|---|
| `TouringOperationsRoute` | `TouringOperationsRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `34-touring-operations.md` user flows/accessibility; design-system page archetypes |
| `TourContainerRoutingBookWorkbench` | `TourContainerRoutingBookWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `34a-tour-container-routing-book.md` request/response fields; `34-touring-operations.md` interactions/access rules |
| `TourTravelRoomingGroundPerdiemWorkbench` | `TourTravelRoomingGroundPerdiemWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `34b-tour-travel-rooming-ground-perdiem.md` request/response fields; `34-touring-operations.md` interactions/access rules |
| `TourBudgetsActualsExpensesWorkbench` | `TourBudgetsActualsExpensesWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `34c-tour-budgets-actuals-expenses.md` request/response fields; `34-touring-operations.md` interactions/access rules |
| `TourBorderMerchCarbonWorkbench` | `TourBorderMerchCarbonWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `34d-tour-border-merch-carbon.md` request/response fields; `34-touring-operations.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `34.01` | Create/link tour | `TouringOperationsRoute` | Authorized primary act; optional dates | Tour/container and participant/date links append | One-off remains standalone | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.02` | Add show/hold/non-show day | `TouringOperationsRoute` | Tour/date authority valid | Ordered date with type, constraints and cost scope adds | Conflicting primary ownership rejects | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.03` | Allocate shared co-headline cost | `TouringOperationsRoute` | Linked tours/date and binding allocation supplied | Cost share/version pins to both budgets | Missing approval leaves cost unallocated | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.04` | Evaluate route leg | `TouringOperationsRoute` | Adjacent dates, load times, transport and rest inputs | Drive range/rest/humane-risk and profile confidence render | Unknown rule profile returns facts only | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.05` | Render tour book | `TouringOperationsRoute` | Authorized recipient/current sources | Versioned accessible artifact/live link/offline bundle issues | Gaps explicit; old version announces supersession | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.06` | Record travel/accommodation | `TouringOperationsRoute` | Authorized source confirmation | Segment/stay/reference/cost/cancellation facts append | No booking or silent email ingestion | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.07` | Build rooming list | `TouringOperationsRoute` | Roster/person constraints and inventory available | Minimal room assignments/grid version issue | Sensitive reason withheld; conflicts named | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.08` | Plan ground transport | `TouringOperationsRoute` | Vehicle/driver/leg/rest facts available | Feasibility and call impacts append | No legality claim without authoritative profile | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.09` | Version per diem/float | `TouringOperationsRoute` | Roster/day eligibility/rates/current float known | Derived obligations and cash assertions append | No custody/advance implication | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.10` | Version tour budget | `TouringOperationsRoute` | Dates/categories/currencies/rates defined | Per-date/tour planned totals and permissions save | Unsupported free-form total rejects | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.11` | Accrue actual | `TouringOperationsRoute` | Settlement/travel/expense/merch fact arrives | Source-linked per-date actual and variance append | Ambiguous source remains pending | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.12` | Capture expense | `TouringOperationsRoute` | Actor authorized; offline draft allowed | Receipt/explanation/source/currency/date append | OCR mismatch requires confirmation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.13` | Track visa/work permit | `TouringOperationsRoute` | Person/border/date requirement source known | Requirement, lead time, document validity and alert save | No advice or inferred eligibility | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.14` | Generate/reconcile carnet | `TouringOperationsRoute` | Date manifest and required gear attributes available | Crossing list, discrepancies and re-entry state append | Missing serial/value/origin/weight blocks readiness only | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.15` | Track withholding readiness | `TouringOperationsRoute` | Cross-border deal/party and specialist refs available | Warning, deadline, forms/certificate and actual fact track | No computed rate/advice | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.16` | Count tour merch | `TouringOperationsRoute` | Authorized stock/event; offline count | Load-in/sales/comp/damage/return and per-show balance append | Conflict requires count reconciliation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `34.17` | Generate carbon estimate | `TouringOperationsRoute` | Sufficient logistics facts and factor set | Distance/mode/fuel/accommodation estimate with coverage/exclusions | Insufficient inputs return partial/unknown | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/touring-operations` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Ftouring-operations` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `TouringOperationsRoute` variant `appPage` | `Tour routing, logistics, finance and reporting | WeJammin` | `Work with tour routing, logistics, finance and reporting using current authority, record state, and provenance.` |
| `/app/touring-operations/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `TouringOperationsRoute` with the matching workbench detail variant | `Record | Tour routing, logistics, finance and reporting | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `TouringOperationsRoute` variant `publicPage` | `Tour routing, logistics, finance and reporting | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `TouringOperationsRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `TouringOperationsRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
...
| `TourTravelRoomingGroundPerdiemWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TourBudgetsActualsExpensesWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TourBorderMerchCarbonWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
| `TourTravelRoomingGroundPerdiemWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TourBudgetsActualsExpensesWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TourBorderMerchCarbonWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `34a-tour-container-routing-book.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `34b-tour-travel-rooming-ground-perdiem.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `34c-tour-budgets-actuals-expenses.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `34d-tour-border-merch-carbon.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `34a-tour-container-routing-book.md` | `34A-TOUR-CONTAINER-ROUTING-BOOK-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TourContainerRoutingBookWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `34b-tour-travel-rooming-ground-perdiem.md` | `34B-TOUR-TRAVEL-ROOMING-GROUND-PERDIEM-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TourTravelRoomingGroundPerdiemWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `34c-tour-budgets-actuals-expenses.md` | `34C-TOUR-BUDGETS-ACTUALS-EXPENSES-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TourBudgetsActualsExpensesWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `34d-tour-border-merch-carbon.md` | `34D-TOUR-BORDER-MERCH-CARBON-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TourBorderMerchCarbonWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `34a-tour-container-routing-book.md` | Named source schemas | `Tour`, `RouteLeg`, `TourBookVersion`, `CreateTour`, `tourId`, `AddTourDate`, `AllocateSharedCost`, `EvaluateRoute`, `RenderTourBook`, `superseded`, `tour_create_total`, `tour_date_append_total`, `tour_allocation_total`, `route_evaluation_total`, `tour_book_job_total` | `TourContainerRoutingBookWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `34b-tour-travel-rooming-ground-perdiem.md` | Named source schemas | `TravelRecord`, `RoomingList`, `PerDiemFloatVersion`, `tourId`, `TravelRecordRequest`, `RoomingListRequest`, `GroundPlanRequest`, `PerDiemFloatRequest`, `rooming_lists`, `ground_plans`, `tour_id`, `anon`, `legal`, `travel_record_total`, `rooming_list_total`, `ground_plan_total`, `per_diem_float_total` | `TourTravelRoomingGroundPerdiemWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `34c-tour-budgets-actuals-expenses.md` | Named source schemas | `TourBudgetVersion`, `TourActual`, `TourExpense`, `tourId`, `BudgetVersionRequest`, `ActualRequest`, `ExpenseRequest`, `anon`, `spentAt`, `tour_budget_version_total`, `tour_actual_total`, `tour_expense_total` | `TourBudgetsActualsExpensesWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `34d-tour-border-merch-carbon.md` | Named source schemas | `BorderReadiness`, `CarnetReconciliation`, `TourMerchMovement`, `CarbonEstimate`, `tourId`, `CarnetRequest`, `MerchMovementRequest`, `CarbonEstimateRequest`, `ready`, `estimate_unverified`, `anon`, `border_readiness_total`, `carnet_reconciliation_total`, `BorderReadinessV1`, `withholding_readiness_total`, `tour_merch_movement_total`, `externally_verified`, `carbon_estimate_total` | `TourBorderMerchCarbonWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type TourContainerRoutingBookContractField =
  | 'AddTourDate'
  | 'AllocateSharedCost'
  | 'CreateTour'
  | 'EvaluateRoute'
  ...
  | 'Tour'
  | 'TourBookVersion'
  | 'route_evaluation_total'
  | 'superseded'
  | 'tourId'
  | 'tour_allocation_total'
  | 'tour_book_job_total'
  | 'tour_create_total'
  | 'tour_date_append_total';
interface TourContainerRoutingBookWorkbenchContractFields {
  source: '34a-tour-container-routing-book.md';
  fields: Readonly<Record<TourContainerRoutingBookContractField, unknown>>;
}
```

```ts
type TourTravelRoomingGroundPerdiemContractField =
  | 'GroundPlanRequest'
  | 'PerDiemFloatRequest'
  | 'PerDiemFloatVersion'
  | 'RoomingList'
  ...
  | 'ground_plans'
  | 'legal'
  | 'per_diem_float_total'
  | 'rooming_list_total'
  | 'rooming_lists'
  | 'tourId'
  | 'tour_id'
  | 'travel_record_total';
interface TourTravelRoomingGroundPerdiemWorkbenchContractFields {
  source: '34b-tour-travel-rooming-ground-perdiem.md';
  fields: Readonly<Record<TourTravelRoomingGroundPerdiemContractField, unknown>>;
}
```

```ts
type TourBudgetsActualsExpensesContractField =
  | 'ActualRequest'
  ...
  | 'tourId'
  | 'tour_actual_total'
  | 'tour_budget_version_total'
  | 'tour_expense_total';
interface TourBudgetsActualsExpensesWorkbenchContractFields {
  source: '34c-tour-budgets-actuals-expenses.md';
  fields: Readonly<Record<TourBudgetsActualsExpensesContractField, unknown>>;
}
```

```ts
type TourBorderMerchCarbonContractField =
  | 'BorderReadiness'
  | 'BorderReadinessV1'
  | 'CarbonEstimate'
  | 'CarbonEstimateRequest'
  | 'CarnetReconciliation'
  | 'CarnetRequest'
  | 'MerchMovementRequest'
  | 'TourMerchMovement'
  | 'anon'
  | 'border_readiness_total'
  | 'carbon_estimate_total'
  | 'carnet_reconciliation_total'
  | 'estimate_unverified'
  | 'externally_verified'
  | 'ready'
  | 'tourId'
  | 'tour_merch_movement_total'
  | 'withholding_readiness_total';
interface TourBorderMerchCarbonWorkbenchContractFields {
  source: '34d-tour-border-merch-carbon.md';
  fields: Readonly<Record<TourBorderMerchCarbonContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `34a-tour-container-routing-book.md` | `TourContainerRoutingBookWorkbenchContractFields.fields` and `TourContainerRoutingBookWorkbenchProps.contractFields` | `APPROVAL_REQUIRED`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `RENDER_UNAVAILABLE`, `ROUTE_INPUT_INVALID`, `ROUTING_SOURCE_UNAVAILABLE`, `SOURCE_AUTHORITY_UNAVAILABLE`, `SOURCE_OWNERSHIP_CONFLICT`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `34b-tour-travel-rooming-ground-perdiem.md` | `TourTravelRoomingGroundPerdiemWorkbenchContractFields.fields` and `TourTravelRoomingGroundPerdiemWorkbenchProps.contractFields` | `ASSIGNMENT_CONFLICT`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `MANDATE_INVALID`, `NOT_FOUND`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `34c-tour-budgets-actuals-expenses.md` | `TourBudgetsActualsExpensesWorkbenchContractFields.fields` and `TourBudgetsActualsExpensesWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `EVIDENCE_REQUIRED`, `FORBIDDEN`, `FX_UNAVAILABLE`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `34d-tour-border-merch-carbon.md` | `TourBorderMerchCarbonWorkbenchContractFields.fields` and `TourBorderMerchCarbonWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `SOURCE_SEQUENCE_CONFLICT`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
