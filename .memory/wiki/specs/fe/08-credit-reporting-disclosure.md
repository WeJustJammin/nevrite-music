...
> **BE Source**: [08a-portability-ddex-emission.md](../be/08a-portability-ddex-emission.md), [08b-union-session-reporting.md](../be/08b-union-session-reporting.md), [08c-gear-credit-linkage.md](../be/08c-gear-credit-linkage.md), [08d-ai-contribution-disclosure.md](../be/08d-ai-contribution-disclosure.md)
> **IA Source**: [08-credit-reporting-disclosure.md](../ia/08-credit-reporting-disclosure.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [08-credit-reporting-disclosure.md](../ia/08-credit-reporting-disclosure.md) in full.
- **BE sources**: [08a-portability-ddex-emission.md](../be/08a-portability-ddex-emission.md), [08b-union-session-reporting.md](../be/08b-union-session-reporting.md), [08c-gear-credit-linkage.md](../be/08c-gear-credit-linkage.md), [08d-ai-contribution-disclosure.md](../be/08d-ai-contribution-disclosure.md).
...
|---|---|---|
| Classification and scope | `08-credit-reporting-disclosure.md`; BE index | Shard boundary and completed BE split group |
...
### `CreditReportingDisclosureRoute` (Astro server route)

```ts
interface CreditReportingDisclosureRouteProps {
  children?: never;
...
### `PortabilityDdexEmissionWorkbench` (bounded React island)

**BE owner**: `08a-portability-ddex-emission.md`

```ts
interface PortabilityDdexEmissionWorkbenchProps {
  contractFields: PortabilityDdexEmissionWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly PortabilityDdexEmissionRecord[]>;
  actorId: string;
...
interface PortabilityDdexEmissionRecord {
  id: string;
  version: string;
  state: string;
...
### `UnionSessionReportingWorkbench` (bounded React island)

**BE owner**: `08b-union-session-reporting.md`

```ts
interface UnionSessionReportingWorkbenchProps {
  contractFields: UnionSessionReportingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly UnionSessionReportingRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface UnionSessionReportingRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `GearCreditLinkageWorkbench` (bounded React island)

**BE owner**: `08c-gear-credit-linkage.md`

```ts
interface GearCreditLinkageWorkbenchProps {
  contractFields: GearCreditLinkageWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearCreditLinkageRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearCreditLinkageRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `AiContributionDisclosureWorkbench` (bounded React island)

**BE owner**: `08d-ai-contribution-disclosure.md`

```ts
interface AiContributionDisclosureWorkbenchProps {
  contractFields: AiContributionDisclosureWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly AiContributionDisclosureRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface AiContributionDisclosureRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/credit-reporting-disclosure` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/credit-reporting-disclosure/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|---|
| `CXR-01` Export own credit history | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-02` Preflight RIN package | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-03` Generate RIN package | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-04` Review stale emission | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-05` Generate portability receipt | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-06` Prepare union session report | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-07` Certify union report | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-08` Link gear to contribution | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-09` Publish gear discography line | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-10` Transfer registered gear | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-11` Add AI involvement entry | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-12` Amend/retract AI disclosure | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-13` Evaluate destination requirement | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CXR-14` View disclosure | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `08-credit-reporting-disclosure.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `08-credit-reporting-disclosure.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `08-credit-reporting-disclosure.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `08-credit-reporting-disclosure.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `08-credit-reporting-disclosure.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `08-credit-reporting-disclosure.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `08-credit-reporting-disclosure.md` Accessibility |
...
|---|---|---|---|---|
| `CreditReportingDisclosureRoute` | `CreditReportingDisclosureRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `08-credit-reporting-disclosure.md` user flows/accessibility; design-system page archetypes |
| `PortabilityDdexEmissionWorkbench` | `PortabilityDdexEmissionWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `08a-portability-ddex-emission.md` request/response fields; `08-credit-reporting-disclosure.md` interactions/access rules |
| `UnionSessionReportingWorkbench` | `UnionSessionReportingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `08b-union-session-reporting.md` request/response fields; `08-credit-reporting-disclosure.md` interactions/access rules |
| `GearCreditLinkageWorkbench` | `GearCreditLinkageWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `08c-gear-credit-linkage.md` request/response fields; `08-credit-reporting-disclosure.md` interactions/access rules |
| `AiContributionDisclosureWorkbench` | `AiContributionDisclosureWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `08d-ai-contribution-disclosure.md` request/response fields; `08-credit-reporting-disclosure.md` interactions/access rules |
...
|---|---|---|---|
| `CXR-01` Export own credit history | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-01` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-02` Preflight RIN package | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-02` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-03` Generate RIN package | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-03` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-04` Review stale emission | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-04` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-05` Generate portability receipt | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-05` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-06` Prepare union session report | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-06` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-07` Certify union report | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-07` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-08` Link gear to contribution | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-08` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-09` Publish gear discography line | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-09` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-10` Transfer registered gear | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-10` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-11` Add AI involvement entry | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-11` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-12` Amend/retract AI disclosure | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-12` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-13` Evaluate destination requirement | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-13` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CXR-14` View disclosure | `CreditReportingDisclosureRoute` orchestrates; `PortabilityDdexEmissionWorkbench`, `UnionSessionReportingWorkbench`, `GearCreditLinkageWorkbench`, `AiContributionDisclosureWorkbench` renders the relevant BE response and command state | `08-credit-reporting-disclosure.md` Interactions row `CXR-14` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
...
|---|---|---|---|---|
| `/app/credit-reporting-disclosure` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fcredit-reporting-disclosure` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `CreditReportingDisclosureRoute` variant `appPage` | `Credit reporting, exchange and disclosure | WeJammin` | `Work with credit reporting, exchange and disclosure using current authority, record state, and provenance.` |
| `/app/credit-reporting-disclosure/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `CreditReportingDisclosureRoute` with the matching workbench detail variant | `Record | Credit reporting, exchange and disclosure | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `CreditReportingDisclosureRoute` variant `publicPage` | `Credit reporting, exchange and disclosure | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `CreditReportingDisclosureRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `CreditReportingDisclosureRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `PortabilityDdexEmissionWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `UnionSessionReportingWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearCreditLinkageWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `AiContributionDisclosureWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `08a-portability-ddex-emission.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `08b-union-session-reporting.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `08c-gear-credit-linkage.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `08d-ai-contribution-disclosure.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `08a-portability-ddex-emission.md` | `08A-PORTABILITY-DDEX-EMISSION-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `PortabilityDdexEmissionWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `08b-union-session-reporting.md` | `08B-UNION-SESSION-REPORTING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `UnionSessionReportingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `08c-gear-credit-linkage.md` | `08C-GEAR-CREDIT-LINKAGE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearCreditLinkageWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `08d-ai-contribution-disclosure.md` | `08D-AI-CONTRIBUTION-DISCLOSURE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `AiContributionDisclosureWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `08a-portability-ddex-emission.md` | Named source schemas | All named response fields | `PortabilityDdexEmissionWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `08b-union-session-reporting.md` | Named source schemas | All named response fields | `UnionSessionReportingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `08c-gear-credit-linkage.md` | Named source schemas | All named response fields | `GearCreditLinkageWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `08d-ai-contribution-disclosure.md` | Named source schemas | All named response fields | `AiContributionDisclosureWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type PortabilityDdexEmissionContractField =
  never;
interface PortabilityDdexEmissionWorkbenchContractFields {
  source: '08a-portability-ddex-emission.md';
  fields: Readonly<Record<PortabilityDdexEmissionContractField, unknown>>;
}
```

```ts
type UnionSessionReportingContractField =
  never;
interface UnionSessionReportingWorkbenchContractFields {
  source: '08b-union-session-reporting.md';
  fields: Readonly<Record<UnionSessionReportingContractField, unknown>>;
}
```

```ts
type GearCreditLinkageContractField =
  never;
interface GearCreditLinkageWorkbenchContractFields {
  source: '08c-gear-credit-linkage.md';
  fields: Readonly<Record<GearCreditLinkageContractField, unknown>>;
}
```

```ts
type AiContributionDisclosureContractField =
  never;
interface AiContributionDisclosureWorkbenchContractFields {
  source: '08d-ai-contribution-disclosure.md';
  fields: Readonly<Record<AiContributionDisclosureContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `08a-portability-ddex-emission.md` | `PortabilityDdexEmissionWorkbenchContractFields.fields` and `PortabilityDdexEmissionWorkbenchProps.contractFields` | `ACCESS_REVOKED`, `ARTIFACT_GENERATION_FAILED`, `ARTIFACT_NOT_FOUND`, `DELIVERY_ADAPTER_UNAVAILABLE`, `EMISSION_NOT_FOUND`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_OUTPUT_KIND`, `INVALID_REQUEST`, `OVERRIDE_INVALID`, `PROFILE_UNAVAILABLE`, `REQUEST_NOT_FOUND`, `RIN_INPUT_INVALID`, `SCOPE_UNAUTHORIZED`, `SOURCE_NOT_FOUND`, `SOURCE_STALE`, `SOURCE_UNAVAILABLE`, `STORAGE_UNAVAILABLE`, `UNAUTHENTICATED`, `URL_EXPIRED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `08b-union-session-reporting.md` | `UnionSessionReportingWorkbenchContractFields.fields` and `UnionSessionReportingWorkbenchProps.contractFields` | `ACCESS_REVOKED`, `ARTIFACT_UNAVAILABLE`, `CONSEQUENCE_ACK_REQUIRED`, `DECLARATION_INVALID`, `FORBIDDEN`, `HUMAN_DECLARATION_REQUIRED`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `PROFILE_UNAVAILABLE`, `REPORT_NOT_FOUND`, `SESSION_OR_PROFILE_NOT_FOUND`, `SOURCE_STALE`, `SOURCE_UNAVAILABLE`, `UNAUTHENTICATED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `08c-gear-credit-linkage.md` | `GearCreditLinkageWorkbenchContractFields.fields` and `GearCreditLinkageWorkbenchProps.contractFields` | `ACCESS_REVOKED`, `AUTHOR_REQUIRED`, `CREDIT_OR_ITEM_NOT_FOUND`, `EMBARGOED_NOT_FOUND`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `ITEM_NOT_FOUND`, `PROJECTION_UNAVAILABLE`, `SESSION_LINK_FORBIDDEN`, `SOURCE_UNAVAILABLE`, `UNAUTHENTICATED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `08d-ai-contribution-disclosure.md` | `AiContributionDisclosureWorkbenchContractFields.fields` and `AiContributionDisclosureWorkbenchProps.contractFields` | `ACTING_CONTEXT_STALE`, `CONTRIBUTION_NOT_FOUND`, `CONTRIBUTION_OR_POLICY_NOT_FOUND`, `CREDIT_NOT_FOUND`, `DEPENDENCY_UNAVAILABLE`, `DISCLOSURE_NOT_FOUND`, `FIELD_REQUIRED`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `PROFILE_UNAVAILABLE`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
