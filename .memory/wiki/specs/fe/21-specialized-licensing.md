# Specialized clearances and licensing: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [21a-sample-interpolation-remix-clearance.md](../be/21a-sample-interpolation-remix-clearance.md), [21b-creator-microlicensing-content-id.md](../be/21b-creator-microlicensing-content-id.md), [21c-ai-training-corpus-compensation.md](../be/21c-ai-training-corpus-compensation.md), [21d-cover-print-grand-right-routing.md](../be/21d-cover-print-grand-right-routing.md)
> **IA Source**: [21-specialized-licensing.md](../ia/21-specialized-licensing.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [21-specialized-licensing.md](../ia/21-specialized-licensing.md) in full.
- **BE sources**: [21a-sample-interpolation-remix-clearance.md](../be/21a-sample-interpolation-remix-clearance.md), [21b-creator-microlicensing-content-id.md](../be/21b-creator-microlicensing-content-id.md), [21c-ai-training-corpus-compensation.md](../be/21c-ai-training-corpus-compensation.md), [21d-cover-print-grand-right-routing.md](../be/21d-cover-print-grand-right-routing.md).
...
|---|---|---|
| Classification and scope | `21-specialized-licensing.md`; BE index | Shard boundary and completed BE split group |
...
### `SpecializedLicensingRoute` (Astro server route)

```ts
interface SpecializedLicensingRouteProps {
  children?: never;
...
### `SampleInterpolationRemixClearanceWorkbench` (bounded React island)

**BE owner**: `21a-sample-interpolation-remix-clearance.md`

```ts
interface SampleInterpolationRemixClearanceWorkbenchProps {
  contractFields: SampleInterpolationRemixClearanceWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly SampleInterpolationRemixClearanceRecord[]>;
  actorId: string;
...
interface SampleInterpolationRemixClearanceRecord {
  id: string;
  version: string;
  state: string;
...
### `CreatorMicrolicensingContentIdWorkbench` (bounded React island)

**BE owner**: `21b-creator-microlicensing-content-id.md`

```ts
interface CreatorMicrolicensingContentIdWorkbenchProps {
  contractFields: CreatorMicrolicensingContentIdWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly CreatorMicrolicensingContentIdRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface CreatorMicrolicensingContentIdRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `AiTrainingCorpusCompensationWorkbench` (bounded React island)

**BE owner**: `21c-ai-training-corpus-compensation.md`

```ts
interface AiTrainingCorpusCompensationWorkbenchProps {
  contractFields: AiTrainingCorpusCompensationWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly AiTrainingCorpusCompensationRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface AiTrainingCorpusCompensationRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `CoverPrintGrandRightRoutingWorkbench` (bounded React island)

**BE owner**: `21d-cover-print-grand-right-routing.md`

```ts
interface CoverPrintGrandRightRoutingWorkbenchProps {
  contractFields: CoverPrintGrandRightRoutingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly CoverPrintGrandRightRoutingRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface CoverPrintGrandRightRoutingRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/specialized-licensing` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/specialized-licensing/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|---|
| `SPL-01` Contributor declares sampled material | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-02` System suggests sample identity | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-03` Owner requests instant sample clearance | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-04` Owners negotiate sample terms | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-05` Creator declares replay/interpolation | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-06` Rights holder grants remix/stem use | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-07` Owner opts work into creator catalogue | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-08` Creator buys micro-licence | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-09` Creator reports Content ID claim | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-10` Subscriber cancels | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-11` Owner sets AI-training decision | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-12` Dataset buyer assembles corpus | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-13` Owner withdraws AI consent | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-14` System records training compensation | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPL-15` User clears cover/print/grand-right use | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `21-specialized-licensing.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `21-specialized-licensing.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `21-specialized-licensing.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `21-specialized-licensing.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `21-specialized-licensing.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `21-specialized-licensing.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `21-specialized-licensing.md` Accessibility |
...
|---|---|---|---|---|
| `SpecializedLicensingRoute` | `SpecializedLicensingRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `21-specialized-licensing.md` user flows/accessibility; design-system page archetypes |
| `SampleInterpolationRemixClearanceWorkbench` | `SampleInterpolationRemixClearanceWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `21a-sample-interpolation-remix-clearance.md` request/response fields; `21-specialized-licensing.md` interactions/access rules |
| `CreatorMicrolicensingContentIdWorkbench` | `CreatorMicrolicensingContentIdWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `21b-creator-microlicensing-content-id.md` request/response fields; `21-specialized-licensing.md` interactions/access rules |
| `AiTrainingCorpusCompensationWorkbench` | `AiTrainingCorpusCompensationWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `21c-ai-training-corpus-compensation.md` request/response fields; `21-specialized-licensing.md` interactions/access rules |
| `CoverPrintGrandRightRoutingWorkbench` | `CoverPrintGrandRightRoutingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `21d-cover-print-grand-right-routing.md` request/response fields; `21-specialized-licensing.md` interactions/access rules |
...
|---|---|---|---|
| `SPL-01` Contributor declares sampled material | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-01` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-02` System suggests sample identity | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-02` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-03` Owner requests instant sample clearance | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-03` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-04` Owners negotiate sample terms | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-04` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-05` Creator declares replay/interpolation | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-05` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-06` Rights holder grants remix/stem use | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-06` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-07` Owner opts work into creator catalogue | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-07` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-08` Creator buys micro-licence | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-08` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-09` Creator reports Content ID claim | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-09` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-10` Subscriber cancels | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-10` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-11` Owner sets AI-training decision | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-11` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-12` Dataset buyer assembles corpus | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-12` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-13` Owner withdraws AI consent | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-13` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-14` System records training compensation | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-14` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPL-15` User clears cover/print/grand-right use | `SpecializedLicensingRoute` orchestrates; `SampleInterpolationRemixClearanceWorkbench`, `CreatorMicrolicensingContentIdWorkbench`, `AiTrainingCorpusCompensationWorkbench`, `CoverPrintGrandRightRoutingWorkbench` renders the relevant BE response and command state | `21-specialized-licensing.md` Interactions row `SPL-15` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
...
|---|---|---|---|---|
| `/app/specialized-licensing` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fspecialized-licensing` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `SpecializedLicensingRoute` variant `appPage` | `Specialized clearances and licensing | WeJammin` | `Work with specialized clearances and licensing using current authority, record state, and provenance.` |
| `/app/specialized-licensing/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `SpecializedLicensingRoute` with the matching workbench detail variant | `Record | Specialized clearances and licensing | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `SpecializedLicensingRoute` variant `publicPage` | `Specialized clearances and licensing | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `SpecializedLicensingRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `SpecializedLicensingRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `SampleInterpolationRemixClearanceWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `CreatorMicrolicensingContentIdWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `AiTrainingCorpusCompensationWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `CoverPrintGrandRightRoutingWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `21a-sample-interpolation-remix-clearance.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `21b-creator-microlicensing-content-id.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `21c-ai-training-corpus-compensation.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `21d-cover-print-grand-right-routing.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `21a-sample-interpolation-remix-clearance.md` | `21A-SAMPLE-INTERPOLATION-REMIX-CLEARANCE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `SampleInterpolationRemixClearanceWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `21b-creator-microlicensing-content-id.md` | `21B-CREATOR-MICROLICENSING-CONTENT-ID-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `CreatorMicrolicensingContentIdWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `21c-ai-training-corpus-compensation.md` | `21C-AI-TRAINING-CORPUS-COMPENSATION-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `AiTrainingCorpusCompensationWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `21d-cover-print-grand-right-routing.md` | `21D-COVER-PRINT-GRAND-RIGHT-ROUTING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `CoverPrintGrandRightRoutingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `21a-sample-interpolation-remix-clearance.md` | Named source schemas | `DeclareSample`, `SuggestSampleIdentity`, `ClearSample`, `GrantDerivativeUse`, `sample_declaration`, `sample_source_side`, `sample_identity_suggestion`, `sample_clearance_request`, `sample_terms`, `derivative_asset_grant`, `declared`, `unidentified`, `suggested_match`, `identified`, `superseded`, `retracted`, `Origin`, `DeclareSampleRequest`, `DeclareSampleResult`, `SuggestSampleIdentityRequest`, `SuggestSampleIdentityResult`, `InstantClearSampleRequest`, `ClearSampleResult`, `NegotiateSampleRequest`, `DeclareInterpolationRequest`, `GrantDerivativeUseRequest`, `GrantDerivativeUseResult`, `licensing_private`, `contribution_id`, `source_work_id`, `source_asset_id`, `candidate_work_id`, `candidate_asset_id`, `instrument_id`, `fingerprint_artifact_ref`, `evidence_ref`, `source_asset_ids`, `source_owner_party_ids`, `migration_role`, `search_path`, `sample_declaration_total`, `sample_suggestion_total`, `sample_instant_clear_total`, `sample_negotiation_total`, `interpolation_declaration_total`, `derivative_grant_total` | `SampleInterpolationRemixClearanceWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `21b-creator-microlicensing-content-id.md` | Named source schemas | `PurchaseCreatorLicence`, `ReleaseContentClaim`, `creator_licence_listing`, `channel_proof`, `whitelist_operation`, `content_claim_case`, `subscription_grant_history`, `WhitelistState`, `not_requested`, `confirmed`, `revoked_in_error`, `reconciling`, `CreatorListingRequest`, `CreatorListingResult`, `PurchaseCreatorLicenceRequest`, `PurchaseCreatorLicenceResult`, `ReleaseContentClaimRequest`, `ReleaseContentClaimResult`, `CancelSubscriptionGrantRequest`, `CancelSubscriptionGrantResult`, `licensing_private`, `work_id`, `owner_share_id`, `template_id`, `instrument_id`, `subscription_id`, `oauth_grant_ref`, `channel_opaque_ref`, `content_ref`, `payment_authorization_ref`, `provider_receipt_ref`, `provider_receipt_refs`, `claim_provider_ref`, `review_case_id`, `creator_licence_purchase`, `migration_role`, `search_path`, `creator_listing_total`, `creator_purchase_total`, `content_claim_release_total`, `subscription_cancel_total` | `CreatorMicrolicensingContentIdWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `21c-ai-training-corpus-compensation.md` | Named source schemas | `SetAITrainingConsent`, `ShipCorpus`, `AllocateTrainingProceeds`, `ai_training_consent`, `corpus_manifest`, `corpus_manifest_item`, `shipped_model_use`, `training_compensation_allocation`, `AIConsentState`, `CorpusState`, `refused`, `opted_in`, `withdrawn`, `superseded`, `draft`, `clearance_blocked`, `ready`, `shipped`, `withdrawn_pre_ship`, `SetAITrainingConsentRequest`, `SetAITrainingConsentResult`, `ShipCorpusRequest`, `ShipCorpusResult`, `WithdrawAITrainingConsentRequest`, `WithdrawAITrainingConsentResult`, `AllocateTrainingProceedsRequest`, `AllocateTrainingProceedsResult`, `licensing_private`, `work_id`, `share_id`, `instrument_id`, `corpus_id`, `dataset_id`, `dataset_object_ref`, `delivery_target_ref`, `deal_id`, `proceeds_entry_ref`, `owner_notice_receipt_ref`, `delivery_receipt_ref`, `model_ref`, `ship_receipt_ref`, `platform_wallet_ref`, `migration_role`, `search_path`, `ai_consent_total`, `corpus_ship_total`, `ai_withdrawal_total`, `training_allocation_total` | `AiTrainingCorpusCompensationWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `21d-cover-print-grand-right-routing.md` | Named source schemas | `ClassifySpecialRoute`, `cover_mechanical_case`, `print_lyric_request`, `caseId`, `ClassifySpecialRouteRequest`, `ClassifySpecialRouteResult`, `licensing_private`, `work_id`, `release_id`, `text_artifact_ref`, `instrument_id`, `special_route_case`, `migration_role`, `search_path`, `special_route_classification_total` | `CoverPrintGrandRightRoutingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type SampleInterpolationRemixClearanceContractField =
  | 'ClearSample'
  | 'ClearSampleResult'
  | 'DeclareInterpolationRequest'
  | 'DeclareSample'
  | 'DeclareSampleRequest'
  | 'DeclareSampleResult'
  | 'GrantDerivativeUse'
  | 'GrantDerivativeUseRequest'
  | 'GrantDerivativeUseResult'
  | 'InstantClearSampleRequest'
  | 'NegotiateSampleRequest'
  | 'Origin'
  | 'SuggestSampleIdentity'
  | 'SuggestSampleIdentityRequest'
  | 'SuggestSampleIdentityResult'
  | 'candidate_asset_id'
  | 'candidate_work_id'
  | 'contribution_id'
  | 'declared'
  | 'derivative_asset_grant'
  | 'derivative_grant_total'
  | 'evidence_ref'
  | 'fingerprint_artifact_ref'
  | 'identified'
  | 'instrument_id'
  | 'interpolation_declaration_total'
  | 'licensing_private'
  | 'migration_role'
  | 'retracted'
  | 'sample_clearance_request'
  | 'sample_declaration'
  | 'sample_declaration_total'
  | 'sample_identity_suggestion'
  | 'sample_instant_clear_total'
  | 'sample_negotiation_total'
  | 'sample_source_side'
  | 'sample_suggestion_total'
  | 'sample_terms'
  | 'search_path'
  | 'source_asset_id'
  | 'source_asset_ids'
  | 'source_owner_party_ids'
  | 'source_work_id'
  | 'suggested_match'
  | 'superseded'
  | 'unidentified';
interface SampleInterpolationRemixClearanceWorkbenchContractFields {
  source: '21a-sample-interpolation-remix-clearance.md';
  fields: Readonly<Record<SampleInterpolationRemixClearanceContractField, unknown>>;
}
```

```ts
type CreatorMicrolicensingContentIdContractField =
  | 'CancelSubscriptionGrantRequest'
  | 'CancelSubscriptionGrantResult'
  | 'CreatorListingRequest'
  | 'CreatorListingResult'
  | 'PurchaseCreatorLicence'
  | 'PurchaseCreatorLicenceRequest'
  | 'PurchaseCreatorLicenceResult'
  | 'ReleaseContentClaim'
  | 'ReleaseContentClaimRequest'
  | 'ReleaseContentClaimResult'
  | 'WhitelistState'
  | 'channel_opaque_ref'
  | 'channel_proof'
  ...
  | 'confirmed'
  | 'content_claim_case'
  | 'content_claim_release_total'
  | 'content_ref'
  | 'creator_licence_listing'
  | 'creator_licence_purchase'
  | 'creator_listing_total'
  | 'creator_purchase_total'
  | 'instrument_id'
  | 'licensing_private'
  | 'migration_role'
  | 'not_requested'
  ...
  | 'revoked_in_error'
  | 'search_path'
  | 'subscription_cancel_total'
  | 'subscription_grant_history'
  | 'subscription_id'
  | 'template_id'
  | 'whitelist_operation'
  | 'work_id';
interface CreatorMicrolicensingContentIdWorkbenchContractFields {
  source: '21b-creator-microlicensing-content-id.md';
  fields: Readonly<Record<CreatorMicrolicensingContentIdContractField, unknown>>;
}
```

```ts
type AiTrainingCorpusCompensationContractField =
  | 'AIConsentState'
  | 'AllocateTrainingProceeds'
  | 'AllocateTrainingProceedsRequest'
  | 'AllocateTrainingProceedsResult'
  | 'CorpusState'
  | 'SetAITrainingConsent'
  | 'SetAITrainingConsentRequest'
  | 'SetAITrainingConsentResult'
  | 'ShipCorpus'
  | 'ShipCorpusRequest'
  | 'ShipCorpusResult'
  | 'WithdrawAITrainingConsentRequest'
  | 'WithdrawAITrainingConsentResult'
  | 'ai_consent_total'
  | 'ai_training_consent'
  | 'ai_withdrawal_total'
  | 'clearance_blocked'
  ...
  | 'shipped_model_use'
  | 'superseded'
  | 'training_allocation_total'
  | 'training_compensation_allocation'
  | 'withdrawn'
  | 'withdrawn_pre_ship'
  | 'work_id';
interface AiTrainingCorpusCompensationWorkbenchContractFields {
  source: '21c-ai-training-corpus-compensation.md';
  fields: Readonly<Record<AiTrainingCorpusCompensationContractField, unknown>>;
}
```

```ts
type CoverPrintGrandRightRoutingContractField =
  | 'ClassifySpecialRoute'
  | 'ClassifySpecialRouteRequest'
  | 'ClassifySpecialRouteResult'
  | 'caseId'
  | 'cover_mechanical_case'
  | 'instrument_id'
  | 'licensing_private'
  | 'migration_role'
  | 'print_lyric_request'
  ...
  | 'search_path'
  | 'special_route_case'
  | 'special_route_classification_total'
  | 'text_artifact_ref'
  | 'work_id';
interface CoverPrintGrandRightRoutingWorkbenchContractFields {
  source: '21d-cover-print-grand-right-routing.md';
  fields: Readonly<Record<CoverPrintGrandRightRoutingContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `21a-sample-interpolation-remix-clearance.md` | `SampleInterpolationRemixClearanceWorkbenchContractFields.fields` and `SampleInterpolationRemixClearanceWorkbenchProps.contractFields` | `CANDIDATE_INVALID`, `CLEARANCE_REQUIRED`, `DECLARATION_REQUIRED`, `FORBIDDEN`, `JOB_SCOPE_INVALID`, `NOT_FOUND`, `OBLIGATION_STACK_INVALID`, `PROVIDER_UNAVAILABLE`, `SCOPE_INVALID`, `STEM_AUTHORITY_REQUIRED`, `UNANIMOUS_CONSENT_REQUIRED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `21b-creator-microlicensing-content-id.md` | `CreatorMicrolicensingContentIdWorkbenchContractFields.fields` and `CreatorMicrolicensingContentIdWorkbenchProps.contractFields` | `CHANNEL_PROOF_REQUIRED`, `CLAIM_REFERENCE_INVALID`, `CLEARANCE_REQUIRED`, `FORBIDDEN`, `NOT_FOUND`, `PROVIDER_UNAVAILABLE`, `TEMPLATE_SCOPE_MISMATCH`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `21c-ai-training-corpus-compensation.md` | `AiTrainingCorpusCompensationWorkbenchContractFields.fields` and `AiTrainingCorpusCompensationWorkbenchProps.contractFields` | `AI_OPT_IN_REQUIRED`, `AI_SCOPE_INVALID`, `ALLOCATION_RULE_INVALID`, `CONSENT_GATE_FAILED`, `DISCLOSURE_REQUIRED`, `FORBIDDEN`, `JOB_SCOPE_INVALID`, `OWNER_NOTICE_REQUIRED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `21d-cover-print-grand-right-routing.md` | `CoverPrintGrandRightRoutingWorkbenchContractFields.fields` and `CoverPrintGrandRightRoutingWorkbenchProps.contractFields` | `FORBIDDEN`, `NOT_FOUND`, `VALIDATION_FAILED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
