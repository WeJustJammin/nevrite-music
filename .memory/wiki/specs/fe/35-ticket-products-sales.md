# Ticket products, sales, access packages and delivery: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [35a-ticket-inventory-onsale-presale.md](../be/35a-ticket-inventory-onsale-presale.md), [35b-ticket-carts-orders-waitlists.md](../be/35b-ticket-carts-orders-waitlists.md), [35c-ticket-guest-allocations-door.md](../be/35c-ticket-guest-allocations-door.md), [35d-ticket-vip-rsvp-conversion.md](../be/35d-ticket-vip-rsvp-conversion.md), [35e-ticket-delivery-transfer-claim.md](../be/35e-ticket-delivery-transfer-claim.md)
> **IA Source**: [35-ticket-products-sales.md](../ia/35-ticket-products-sales.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [35-ticket-products-sales.md](../ia/35-ticket-products-sales.md) in full.
- **BE sources**: [35a-ticket-inventory-onsale-presale.md](../be/35a-ticket-inventory-onsale-presale.md), [35b-ticket-carts-orders-waitlists.md](../be/35b-ticket-carts-orders-waitlists.md), [35c-ticket-guest-allocations-door.md](../be/35c-ticket-guest-allocations-door.md), [35d-ticket-vip-rsvp-conversion.md](../be/35d-ticket-vip-rsvp-conversion.md), [35e-ticket-delivery-transfer-claim.md](../be/35e-ticket-delivery-transfer-claim.md).
...
|---|---|---|
| Classification and scope | `35-ticket-products-sales.md`; BE index | Shard boundary and completed BE split group |
...
### `TicketProductsSalesRoute` (Astro server route)

```ts
interface TicketProductsSalesRouteProps {
  children?: never;
...
### `TicketInventoryOnsalePresaleWorkbench` (bounded React island)

**BE owner**: `35a-ticket-inventory-onsale-presale.md`

```ts
interface TicketInventoryOnsalePresaleWorkbenchProps {
  contractFields: TicketInventoryOnsalePresaleWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketInventoryOnsalePresaleRecord[]>;
  actorId: string;
...
interface TicketInventoryOnsalePresaleRecord {
  id: string;
  version: string;
  state: string;
...
### `TicketCartsOrdersWaitlistsWorkbench` (bounded React island)

**BE owner**: `35b-ticket-carts-orders-waitlists.md`

```ts
interface TicketCartsOrdersWaitlistsWorkbenchProps {
  contractFields: TicketCartsOrdersWaitlistsWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketCartsOrdersWaitlistsRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TicketCartsOrdersWaitlistsRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TicketGuestAllocationsDoorWorkbench` (bounded React island)

**BE owner**: `35c-ticket-guest-allocations-door.md`

```ts
interface TicketGuestAllocationsDoorWorkbenchProps {
  contractFields: TicketGuestAllocationsDoorWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketGuestAllocationsDoorRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TicketGuestAllocationsDoorRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TicketVipRsvpConversionWorkbench` (bounded React island)

**BE owner**: `35d-ticket-vip-rsvp-conversion.md`

```ts
interface TicketVipRsvpConversionWorkbenchProps {
  contractFields: TicketVipRsvpConversionWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketVipRsvpConversionRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TicketVipRsvpConversionRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TicketDeliveryTransferClaimWorkbench` (bounded React island)

**BE owner**: `35e-ticket-delivery-transfer-claim.md`

```ts
interface TicketDeliveryTransferClaimWorkbenchProps {
  contractFields: TicketDeliveryTransferClaimWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketDeliveryTransferClaimRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TicketDeliveryTransferClaimRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/ticket-products-sales` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/ticket-products-sales/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `35-ticket-products-sales.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `35-ticket-products-sales.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `35-ticket-products-sales.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `35-ticket-products-sales.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `35-ticket-products-sales.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `35-ticket-products-sales.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `35-ticket-products-sales.md` Accessibility |
...
|---|---|---|---|---|
| `TicketProductsSalesRoute` | `TicketProductsSalesRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `35-ticket-products-sales.md` user flows/accessibility; design-system page archetypes |
| `TicketInventoryOnsalePresaleWorkbench` | `TicketInventoryOnsalePresaleWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `35a-ticket-inventory-onsale-presale.md` request/response fields; `35-ticket-products-sales.md` interactions/access rules |
| `TicketCartsOrdersWaitlistsWorkbench` | `TicketCartsOrdersWaitlistsWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `35b-ticket-carts-orders-waitlists.md` request/response fields; `35-ticket-products-sales.md` interactions/access rules |
| `TicketGuestAllocationsDoorWorkbench` | `TicketGuestAllocationsDoorWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `35c-ticket-guest-allocations-door.md` request/response fields; `35-ticket-products-sales.md` interactions/access rules |
| `TicketVipRsvpConversionWorkbench` | `TicketVipRsvpConversionWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `35d-ticket-vip-rsvp-conversion.md` request/response fields; `35-ticket-products-sales.md` interactions/access rules |
| `TicketDeliveryTransferClaimWorkbench` | `TicketDeliveryTransferClaimWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `35e-ticket-delivery-transfer-claim.md` request/response fields; `35-ticket-products-sales.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `35.01` | Configure scaling | `TicketProductsSalesRoute` | Show/room config/currency and authority valid | Ticket types, shared variants, tiers and limits version | Capacity/currency/fee inconsistency rejects | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.02` | Lock manifest | `TicketProductsSalesRoute` | Scaling, blocks, holds/kills and accessible stock valid | Locked sellable/held/protected inventory version | On-sale cannot schedule partially locked manifest | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.03` | Configure fees | `TicketProductsSalesRoute` | Settlement authority; all fee lines defined | Exact all-in calculation/version publishes | Mid-sale increase blocked; parity violation rejects | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.04` | Register and schedule announce/on-sale | `TicketProductsSalesRoute` | Locked manifest and accepted-deal reference; Shard-30 boundary available | Versioned venue-local `OnSaleSchedule` is recorded in Shard 35, Shard 30 binds authorization to the exact schedule, and only then is a durable timer eligible | DST gap, stale edit, missing binding or authorization failure leaves fan-invisible `Embargoed` and arms no announce timer | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.05` | Configure presale | `TicketProductsSalesRoute` | Window, allocation, eligibility/code policy valid | Real source-pool allocation reserves | Overlapping inventory rejects | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.06` | Issue/redeem code | `TicketProductsSalesRoute` | Fan/window/code state and pool current | Eligibility/use count and cart source atomically reserve | Invalid code and exhausted allocation remain distinct | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.07` | Enter queue | `TicketProductsSalesRoute` | Queue-enabled sale and fan eligible | Randomized neutral position with reconnect token | No role priority or duplicate positions | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.08` | Create cart | `TicketProductsSalesRoute` | Inventory/source pool and price version current | Units and exact price hold atomically | Tier may advance; cart price stays fixed | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.09` | Complete checkout | `TicketProductsSalesRoute` | Hold, fan/order/payee and payment valid | Payment/order/tickets commit or compensate idempotently | Ambiguous payment keeps bounded reconcile state; no oversell | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.10` | Join waitlist | `TicketProductsSalesRoute` | Sold-out/exhausted pool; demand terms supplied | Private entry with all/partial preference and consent | Position never disclosed | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.11` | Claim returned inventory | `TicketProductsSalesRoute` | Offer/code active and units returned | Requested permitted quantity enters cart | Expiry/lapse advances batch without public release | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.12` | Create/raise guest allocation | `TicketProductsSalesRoute` | Entitled party/funder, deadline and manifest headroom | Matching held units reserve atomically | Cannot lower below spend | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.13` | Submit guest/+1 | `TicketProductsSalesRoute` | Allocation and deadline valid | One ticket/barcode per head moves held→comp | Partial +1 spend disallowed; duplicate warns | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.14` | Add door guest | `TicketProductsSalesRoute` | Door role or online Operator; reserve/headroom known | Reserve-backed comp or attributed over-allocation line issues | Offline beyond reserve refuses safely | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.15` | Compose VIP access package | `TicketProductsSalesRoute` | Artist approval and component inventories current | Per-variant availability/binding constraint saves | Missing physical fulfillment keeps component disabled | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.16` | Purchase VIP package | `TicketProductsSalesRoute` | Every component/slot available | All components reserve/issue atomically | Any component failure releases all | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.17` | Redeem M&G/VIP | `TicketProductsSalesRoute` | Valid package, assigned slot/current roster | Fulfillment states update in one fan package view | Miss/cancel uses component fault policy | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.18` | RSVP/free admit | `TicketProductsSalesRoute` | Event cap/owner and verified fan valid | One admission/head issues without payment surfaces | Duplicate is idempotent; self-release returns unit | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.19` | Convert free event to paid | `TicketProductsSalesRoute` | RSVP/manifest and explicit conversion approved | Existing RSVPs become comp allocation; remaining units sellable | Cannot strand or charge existing holder silently | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.20` | Deliver/update ticket | `TicketProductsSalesRoute` | Issuance/change and delivery routes available | Live wallet projection plus fallbacks update | Delivery failure alerts Operator | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.21` | Claim transferred ticket | `TicketProductsSalesRoute` | Signed single-use link and ticket state valid | Recipient receives same deterministic ticket identity | Account creation optional; replay fails | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `35.22` | Execute scheduled announce | `TicketProductsSalesRoute` | Timer due; exact current schedule, authorization and lifecycle versions | Shard-30 `CommitScheduledAnnouncement` performs the canonical exactly-once transition; Shard 35 updates its derived schedule projection | Missing/stale/revoked/superseded/unknown versions fail closed; duplicate callback is idempotent | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/ticket-products-sales` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fticket-products-sales` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `TicketProductsSalesRoute` variant `appPage` | `Ticket products, sales, access packages and delivery | WeJammin` | `Work with ticket products, sales, access packages and delivery using current authority, record state, and provenance.` |
| `/app/ticket-products-sales/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `TicketProductsSalesRoute` with the matching workbench detail variant | `Record | Ticket products, sales, access packages and delivery | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `TicketProductsSalesRoute` variant `publicPage` | `Ticket products, sales, access packages and delivery | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `TicketProductsSalesRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `TicketProductsSalesRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
...
| `TicketCartsOrdersWaitlistsWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TicketGuestAllocationsDoorWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TicketVipRsvpConversionWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TicketDeliveryTransferClaimWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
| `TicketCartsOrdersWaitlistsWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TicketGuestAllocationsDoorWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TicketVipRsvpConversionWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TicketDeliveryTransferClaimWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `35a-ticket-inventory-onsale-presale.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `35b-ticket-carts-orders-waitlists.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `35c-ticket-guest-allocations-door.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `35d-ticket-vip-rsvp-conversion.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `35e-ticket-delivery-transfer-claim.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `35a-ticket-inventory-onsale-presale.md` | `35A-TICKET-INVENTORY-ONSALE-PRESALE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketInventoryOnsalePresaleWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `35b-ticket-carts-orders-waitlists.md` | `35B-TICKET-CARTS-ORDERS-WAITLISTS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketCartsOrdersWaitlistsWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `35c-ticket-guest-allocations-door.md` | `35C-TICKET-GUEST-ALLOCATIONS-DOOR-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketGuestAllocationsDoorWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `35d-ticket-vip-rsvp-conversion.md` | `35D-TICKET-VIP-RSVP-CONVERSION-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketVipRsvpConversionWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `35e-ticket-delivery-transfer-claim.md` | `35E-TICKET-DELIVERY-TRANSFER-CLAIM-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketDeliveryTransferClaimWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `35a-ticket-inventory-onsale-presale.md` | Named source schemas | `TicketScalingVersion`, `Manifest`, `InventoryBlock`, `FeeProfileVersion`, `OnSaleSchedule`, `AccessCode`, `eventId`, `ScalingRequest`, `ManifestRequest`, `FeeProfileRequest`, `ScheduleRequest`, `PresaleRequest`, `CodeRequest`, `scheduleId`, `ExecuteSchedule`, `anon`, `dealVersion`, `ticket_scaling_total`, `ticket_manifest_total`, `fee_profile_total`, `onsale_schedule_total`, `presale_policy_total`, `access_code_batch_total`, `schedule_execution_total` | `TicketInventoryOnsalePresaleWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `35b-ticket-carts-orders-waitlists.md` | Named source schemas | `QueuePosition`, `TicketCart`, `WaitlistEntry`, `TicketOrder`, `eventId`, `QueueRequest`, `CartRequest`, `cartId`, `CheckoutRequest`, `WaitlistRequest`, `offerId`, `ClaimOfferRequest`, `queue_entry_total`, `ticket_cart_total`, `ticket_checkout_total`, `waitlist_entry_total`, `waitlist_claim_total` | `TicketCartsOrdersWaitlistsWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `35c-ticket-guest-allocations-door.md` | Named source schemas | `GuestAllocation`, `eventId`, `AllocationRequest`, `allocationId`, `GuestEntryRequest`, `DoorGuestRequest`, `pending_approval`, `door_guest_action`, `guest_allocation_total`, `guest_entry_total`, `door_guest_total` | `TicketGuestAllocationsDoorWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `35d-ticket-vip-rsvp-conversion.md` | Named source schemas | `AccessPackage`, `RSVPAdmission`, `eventId`, `PackageRequest`, `packageId`, `PurchaseRequest`, `RedemptionRequest`, `RSVPRequest`, `ConversionRequest`, `event_id`, `product_ref`, `order_id`, `holder_identity_id`, `anon`, `access_package_total`, `access_purchase_total`, `benefit_redemption_total`, `rsvp_admission_total`, `paid_conversion_total` | `TicketVipRsvpConversionWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `35e-ticket-delivery-transfer-claim.md` | Named source schemas | `Ticket`, `TicketPassProjection`, `orderId`, `DeliveryRequest`, `transferId`, `ClaimTransferRequest`, `anon`, `transfer_pending`, `ticket_delivery_total`, `ticket_transfer_claim_total` | `TicketDeliveryTransferClaimWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type TicketInventoryOnsalePresaleContractField =
  | 'AccessCode'
  | 'CodeRequest'
  | 'ExecuteSchedule'
  | 'FeeProfileRequest'
  ...
  | 'ScalingRequest'
  | 'ScheduleRequest'
  | 'TicketScalingVersion'
  | 'access_code_batch_total'
  | 'anon'
  | 'dealVersion'
  | 'eventId'
  | 'fee_profile_total'
  | 'onsale_schedule_total'
  | 'presale_policy_total'
  | 'scheduleId'
  | 'schedule_execution_total'
  | 'ticket_manifest_total'
  | 'ticket_scaling_total';
interface TicketInventoryOnsalePresaleWorkbenchContractFields {
  source: '35a-ticket-inventory-onsale-presale.md';
  fields: Readonly<Record<TicketInventoryOnsalePresaleContractField, unknown>>;
}
```

```ts
type TicketCartsOrdersWaitlistsContractField =
  | 'CartRequest'
  | 'CheckoutRequest'
  | 'ClaimOfferRequest'
  | 'QueuePosition'
  ...
  | 'queue_entry_total'
  | 'ticket_cart_total'
  | 'ticket_checkout_total'
  | 'waitlist_claim_total'
  | 'waitlist_entry_total';
interface TicketCartsOrdersWaitlistsWorkbenchContractFields {
  source: '35b-ticket-carts-orders-waitlists.md';
  fields: Readonly<Record<TicketCartsOrdersWaitlistsContractField, unknown>>;
}
```

```ts
type TicketGuestAllocationsDoorContractField =
  | 'AllocationRequest'
  | 'DoorGuestRequest'
  | 'GuestAllocation'
  | 'GuestEntryRequest'
  ...
  | 'door_guest_total'
  | 'eventId'
  | 'guest_allocation_total'
  | 'guest_entry_total'
  | 'pending_approval';
interface TicketGuestAllocationsDoorWorkbenchContractFields {
  source: '35c-ticket-guest-allocations-door.md';
  fields: Readonly<Record<TicketGuestAllocationsDoorContractField, unknown>>;
}
```

```ts
type TicketVipRsvpConversionContractField =
  | 'AccessPackage'
  | 'ConversionRequest'
  | 'PackageRequest'
  | 'PurchaseRequest'
  ...
  | 'RSVPRequest'
  | 'RedemptionRequest'
  | 'access_package_total'
  | 'access_purchase_total'
  | 'anon'
  | 'benefit_redemption_total'
  | 'eventId'
  | 'event_id'
  | 'holder_identity_id'
  | 'order_id'
  | 'packageId'
  | 'paid_conversion_total'
  | 'product_ref'
  | 'rsvp_admission_total';
interface TicketVipRsvpConversionWorkbenchContractFields {
  source: '35d-ticket-vip-rsvp-conversion.md';
  fields: Readonly<Record<TicketVipRsvpConversionContractField, unknown>>;
}
```

```ts
type TicketDeliveryTransferClaimContractField =
  | 'ClaimTransferRequest'
  | 'DeliveryRequest'
  | 'Ticket'
  | 'TicketPassProjection'
  | 'anon'
  | 'orderId'
  | 'ticket_delivery_total'
  | 'ticket_transfer_claim_total'
  | 'transferId'
  | 'transfer_pending';
interface TicketDeliveryTransferClaimWorkbenchContractFields {
  source: '35e-ticket-delivery-transfer-claim.md';
  fields: Readonly<Record<TicketDeliveryTransferClaimContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `35a-ticket-inventory-onsale-presale.md` | `TicketInventoryOnsalePresaleWorkbenchContractFields.fields` and `TicketInventoryOnsalePresaleWorkbenchProps.contractFields` | `CAPACITY_CONFLICT`, `CODE_EXPIRED`, `DEPENDENCY_UNAVAILABLE`, `FEE_POLICY_INVALID`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `SCHEDULE_INVALID`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `35b-ticket-carts-orders-waitlists.md` | `TicketCartsOrdersWaitlistsWorkbenchContractFields.fields` and `TicketCartsOrdersWaitlistsWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `INVENTORY_CONFLICT`, `NOT_FOUND`, `OFFER_EXPIRED`, `PAYMENT_FAILED`, `QUEUE_OR_CART_EXPIRED`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `35c-ticket-guest-allocations-door.md` | `TicketGuestAllocationsDoorWorkbenchContractFields.fields` and `TicketGuestAllocationsDoorWorkbenchProps.contractFields` | `APPROVAL_REQUIRED`, `CREDENTIAL_OR_RESTRICTION_FAILED`, `DEPENDENCY_UNAVAILABLE`, `DUPLICATE_GUEST`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `35d-ticket-vip-rsvp-conversion.md` | `TicketVipRsvpConversionWorkbenchContractFields.fields` and `TicketVipRsvpConversionWorkbenchProps.contractFields` | `CAPACITY_CONFLICT`, `CONVERSION_POLICY_INVALID`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `35e-ticket-delivery-transfer-claim.md` | `TicketDeliveryTransferClaimWorkbenchContractFields.fields` and `TicketDeliveryTransferClaimWorkbenchProps.contractFields` | `DELIVERY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `ORDER_QUANTITY_CONFLICT`, `RATE_LIMITED`, `RECIPIENT_OR_POLICY_INVALID`, `SIGNING_UNAVAILABLE`, `TICKET_STATE_CONFLICT`, `TRANSFER_EXPIRED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
