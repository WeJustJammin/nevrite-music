# Analytics ingestion, matching and reporting: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [39-analytics-ingestion-reporting.md](../be/39-analytics-ingestion-reporting.md)
> **IA Source**: [39-analytics-ingestion-reporting.md](../ia/39-analytics-ingestion-reporting.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [39-analytics-ingestion-reporting.md](../ia/39-analytics-ingestion-reporting.md) in full.
- **BE sources**: [39-analytics-ingestion-reporting.md](../be/39-analytics-ingestion-reporting.md).
...
|---|---|---|
| Classification and scope | `39-analytics-ingestion-reporting.md`; BE index | Shard boundary and completed BE split group |
...
### `AnalyticsIngestionReportingRoute` (Astro server route)

```ts
interface AnalyticsIngestionReportingRouteProps {
  children?: never;
...
### `AnalyticsIngestionReportingWorkbench` (bounded React island)

**BE owner**: `39-analytics-ingestion-reporting.md`

```ts
interface AnalyticsIngestionReportingWorkbenchProps {
  contractFields: AnalyticsIngestionReportingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly AnalyticsIngestionReportingRecord[]>;
  actorId: string;
...
interface AnalyticsIngestionReportingRecord {
  id: string;
  version: string;
  state: string;
...
|---|---|---|---|
| `/app/analytics-ingestion-reporting` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/analytics-ingestion-reporting/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `39-analytics-ingestion-reporting.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `39-analytics-ingestion-reporting.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `39-analytics-ingestion-reporting.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `39-analytics-ingestion-reporting.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `39-analytics-ingestion-reporting.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `39-analytics-ingestion-reporting.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `39-analytics-ingestion-reporting.md` Accessibility |
...
|---|---|---|---|---|
| `AnalyticsIngestionReportingRoute` | `AnalyticsIngestionReportingRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `39-analytics-ingestion-reporting.md` user flows/accessibility; design-system page archetypes |
| `AnalyticsIngestionReportingWorkbench` | `AnalyticsIngestionReportingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `39-analytics-ingestion-reporting.md` request/response fields; `39-analytics-ingestion-reporting.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `39.01` | Connect source | `AnalyticsIngestionReportingRoute` | Entity authority and provider grant | Canonical connection/delegated reader activates without claiming match | Unsupported scope/provider remains disconnected/manual-import only | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.02` | Sync source | `AnalyticsIngestionReportingRoute` | Active capability and scheduled cursor | Immutable observations/restatements ingest idempotently | Transient retry quiet; terminal/actionable state notifies | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.03` | Disconnect source | `AnalyticsIngestionReportingRoute` | Owner selects retain/purge and terms allow | Credentials revoke; history retains or purges by policy | Unknown provider outcome reconciles; no silent deletion | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.04` | Import metric file | `AnalyticsIngestionReportingRoute` | Supported parser and preview accepted | Claimed observations commit once with row/source provenance | Overlap/schema/metric conflict blocks or quarantines rows | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.05` | Inspect metric | `AnalyticsIngestionReportingRoute` | Authorized dashboard context | Value renders with source, coverage, gap, freshness and restatement | Insufficient coverage returns bounded/unknown, never estimate | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.06` | Match external profile | `AnalyticsIngestionReportingRoute` | Candidate evidence and entity authority | Confirmed unique subject binding appends | Collision/merge/split quarantines; contested binding blocked | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.07` | Match catalog | `AnalyticsIngestionReportingRoute` | External recording and corroborating evidence | Contributor-owned many-to-many binding appends | Fuzzy/ISRC-only candidate requires confirmation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.08` | Resolve conflict | `AnalyticsIngestionReportingRoute` | Cleanup steward or scoped dispute route | Correct binding/alias or explicit unresolved remedy records | No response never silently awards claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.09` | View dashboard | `AnalyticsIngestionReportingRoute` | Person and authorized contexts | Set-union series render with per-series integrity labels | Partial source affects only dependent metric | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.10` | Configure alert/digest | `AnalyticsIngestionReportingRoute` | Recipient and permitted metric | Rule consumes domain noise budget and saves | Closed gate/sparse/private data cannot be scheduled | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.11` | Render digest | `AnalyticsIngestionReportingRoute` | Due rules and complete permission-scoped render | Answer plus provenance/freshness sends once | Any required render failure sends nothing | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.12` | Share/export report | `AnalyticsIngestionReportingRoute` | Step-up, field policy and snapshot/live choice | Expiring report carries all integrity labels | Provider terms redact/deny fields explicitly; cohorts excluded | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.13` | View cohort comparison | `AnalyticsIngestionReportingRoute` | B2 gate open and query meets approved floor | Pull-only anonymized stage/shape comparison renders | Below floor suppresses, never widens | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.14` | View contribution catalog | `AnalyticsIngestionReportingRoute` | Counter/self-claimed credits and source series | All records list; coverage precedes non-estimated aggregate | No-data remains visible unknown | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.15` | Compare role slices | `AnalyticsIngestionReportingRoute` | Normalized credit roles | Overlapping role totals and visible unroled bucket render | Slices state non-additivity; no inferred role | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `39.16` | Publish performance proof | `AnalyticsIngestionReportingRoute` | Counter-attested role credit, sufficient allowed coverage and provider terms | Role-scoped total proof version publishes | Import/self-claim/terms restriction blocks; later invalidation degrades | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/analytics-ingestion-reporting` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fanalytics-ingestion-reporting` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `AnalyticsIngestionReportingRoute` variant `appPage` | `Analytics ingestion, matching and reporting | WeJammin` | `Work with analytics ingestion, matching and reporting using current authority, record state, and provenance.` |
| `/app/analytics-ingestion-reporting/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `AnalyticsIngestionReportingRoute` with the matching workbench detail variant | `Record | Analytics ingestion, matching and reporting | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `AnalyticsIngestionReportingRoute` variant `publicPage` | `Analytics ingestion, matching and reporting | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `AnalyticsIngestionReportingRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `AnalyticsIngestionReportingRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `AnalyticsIngestionReportingWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `39-analytics-ingestion-reporting.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `39-analytics-ingestion-reporting.md` | `39-ANALYTICS-INGESTION-REPORTING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `AnalyticsIngestionReportingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `39-analytics-ingestion-reporting.md` | Named source schemas | `requestId`, `sourceIds`, `traceId`, `operationId` | `AnalyticsIngestionReportingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type AnalyticsIngestionReportingContractField =
  | 'operationId'
  | 'sourceIds'
  | 'traceId';
interface AnalyticsIngestionReportingWorkbenchContractFields {
  source: '39-analytics-ingestion-reporting.md';
  fields: Readonly<Record<AnalyticsIngestionReportingContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `39-analytics-ingestion-reporting.md` | `AnalyticsIngestionReportingWorkbenchContractFields.fields` and `AnalyticsIngestionReportingWorkbenchProps.contractFields` | `ADAPTER_INVALID`, `AUTH_REQUIRED`, `CASE_NOT_FOUND`, `CASE_SCOPE_REQUIRED`, `CASE_STATE_CONFLICT`, `CATALOG_BINDING_CONFLICT`, `CATALOG_NOT_FOUND`, `CONNECTION_CONFLICT`, `CONNECTION_NOT_FOUND`, `CONTRIBUTOR_AUTHORITY_REQUIRED`, `CORROBORATION_REQUIRED`, `CREDIT_AUTHORITY_UNAVAILABLE`, `DEPENDENCY_TIMEOUT`, `DUAL_CONTROL_REQUIRED`, `EVIDENCE_INVALID`, `FIELD_POLICY_INVALID`, `METRIC_NOT_FOUND`, `OWNER_REQUIRED`, `PERIOD_INVALID`, `PREVIEW_MISMATCH`, `PROJECTION_UNAVAILABLE`, `PROOF_REQUEST_INVALID`, `PROOF_VERSION_CONFLICT`, `PROVIDER_SCOPE_INVALID`, `PROVIDER_UNAVAILABLE`, `PURGE_TERMS_FORBIDDEN`, `QUERY_INVALID`, `RATE_LIMITED`, `REDISTRIBUTION_FORBIDDEN`, `RENDER_UNAVAILABLE`, `REVISION_MISMATCH`, `ROLE_AUTHORITY_REQUIRED`, `ROLE_SLICE_INVALID`, `RULE_INVALID`, `RULE_NOT_FOUND`, `SERVICE_AUTH_REQUIRED`, `SNAPSHOT_STALE`, `SOURCE_OBSERVATION_CONFLICT`, `STEP_UP_REQUIRED`, `SUBJECT_AUTHORITY_REQUIRED`, `VALIDATION_FAILED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
