# Door access, box office, reconciliation and ticketing risk: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [36a-door-replicas-scans-age.md](../be/36a-door-replicas-scans-age.md), [36b-boxoffice-counts-drops-walkup-close.md](../be/36b-boxoffice-counts-drops-walkup-close.md), [36c-ticket-refunds-event-changes.md](../be/36c-ticket-refunds-event-changes.md), [36d-external-counts-attestation-reconciliation.md](../be/36d-external-counts-attestation-reconciliation.md), [36e-ticket-limits-transfer-exchange-consent.md](../be/36e-ticket-limits-transfer-exchange-consent.md)
> **IA Source**: [36-box-office-risk.md](../ia/36-box-office-risk.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [36-box-office-risk.md](../ia/36-box-office-risk.md) in full.
- **BE sources**: [36a-door-replicas-scans-age.md](../be/36a-door-replicas-scans-age.md), [36b-boxoffice-counts-drops-walkup-close.md](../be/36b-boxoffice-counts-drops-walkup-close.md), [36c-ticket-refunds-event-changes.md](../be/36c-ticket-refunds-event-changes.md), [36d-external-counts-attestation-reconciliation.md](../be/36d-external-counts-attestation-reconciliation.md), [36e-ticket-limits-transfer-exchange-consent.md](../be/36e-ticket-limits-transfer-exchange-consent.md).
...
|---|---|---|
| Classification and scope | `36-box-office-risk.md`; BE index | Shard boundary and completed BE split group |
...
### `BoxOfficeRiskRoute` (Astro server route)

```ts
interface BoxOfficeRiskRouteProps {
  children?: never;
...
### `DoorReplicasScansAgeWorkbench` (bounded React island)

**BE owner**: `36a-door-replicas-scans-age.md`

```ts
interface DoorReplicasScansAgeWorkbenchProps {
  contractFields: DoorReplicasScansAgeWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly DoorReplicasScansAgeRecord[]>;
  actorId: string;
...
interface DoorReplicasScansAgeRecord {
  id: string;
  version: string;
  state: string;
...
### `BoxofficeCountsDropsWalkupCloseWorkbench` (bounded React island)

**BE owner**: `36b-boxoffice-counts-drops-walkup-close.md`

```ts
interface BoxofficeCountsDropsWalkupCloseWorkbenchProps {
  contractFields: BoxofficeCountsDropsWalkupCloseWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly BoxofficeCountsDropsWalkupCloseRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface BoxofficeCountsDropsWalkupCloseRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TicketRefundsEventChangesWorkbench` (bounded React island)

**BE owner**: `36c-ticket-refunds-event-changes.md`

```ts
interface TicketRefundsEventChangesWorkbenchProps {
  contractFields: TicketRefundsEventChangesWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketRefundsEventChangesRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TicketRefundsEventChangesRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `ExternalCountsAttestationReconciliationWorkbench` (bounded React island)

**BE owner**: `36d-external-counts-attestation-reconciliation.md`

```ts
interface ExternalCountsAttestationReconciliationWorkbenchProps {
  contractFields: ExternalCountsAttestationReconciliationWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly ExternalCountsAttestationReconciliationRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface ExternalCountsAttestationReconciliationRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `TicketLimitsTransferExchangeConsentWorkbench` (bounded React island)

**BE owner**: `36e-ticket-limits-transfer-exchange-consent.md`

```ts
interface TicketLimitsTransferExchangeConsentWorkbenchProps {
  contractFields: TicketLimitsTransferExchangeConsentWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TicketLimitsTransferExchangeConsentRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TicketLimitsTransferExchangeConsentRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/box-office-risk` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/box-office-risk/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `36-box-office-risk.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `36-box-office-risk.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `36-box-office-risk.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `36-box-office-risk.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `36-box-office-risk.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `36-box-office-risk.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `36-box-office-risk.md` Accessibility |
...
|---|---|---|---|---|
| `BoxOfficeRiskRoute` | `BoxOfficeRiskRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `36-box-office-risk.md` user flows/accessibility; design-system page archetypes |
| `DoorReplicasScansAgeWorkbench` | `DoorReplicasScansAgeWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `36a-door-replicas-scans-age.md` request/response fields; `36-box-office-risk.md` interactions/access rules |
| `BoxofficeCountsDropsWalkupCloseWorkbench` | `BoxofficeCountsDropsWalkupCloseWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `36b-boxoffice-counts-drops-walkup-close.md` request/response fields; `36-box-office-risk.md` interactions/access rules |
| `TicketRefundsEventChangesWorkbench` | `TicketRefundsEventChangesWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `36c-ticket-refunds-event-changes.md` request/response fields; `36-box-office-risk.md` interactions/access rules |
| `ExternalCountsAttestationReconciliationWorkbench` | `ExternalCountsAttestationReconciliationWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `36d-external-counts-attestation-reconciliation.md` request/response fields; `36-box-office-risk.md` interactions/access rules |
| `TicketLimitsTransferExchangeConsentWorkbench` | `TicketLimitsTransferExchangeConsentWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `36e-ticket-limits-transfer-exchange-consent.md` request/response fields; `36-box-office-risk.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `36.01` | Provision scanner | `BoxOfficeRiskRoute` | Recognized device/operator and complete event replica | Encrypted replica/device epoch enters ready | Missing/incomplete replica blocks readiness | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.02` | Scan ticket | `BoxOfficeRiskRoute` | Current/offline replica and code/name input | Accept/refuse/override event appends instantly | Stale warns; unknown leaks no cross-event info | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.03` | Coordinate scanners | `BoxOfficeRiskRoute` | Peers reachable or later server sync | Duplicate/conflict detected with causal/order confidence | Disconnected conflict admits and reconciles | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.04` | Reverse/refuse prior scan | `BoxOfficeRiskRoute` | Authorized door actor and reason | Additive linked reversal decrements gate-observed count | Original scan never deleted | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.05` | Verify age/ID | `BoxOfficeRiskRoute` | Ticket restriction and venue policy current | Human records pass/refusal class only | No ID image/data retained | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.06` | Read live count | `BoxOfficeRiskRoute` | Authorized deal/role projection | Consistent sourced/fresh counter snapshot renders | Fan sees only availability boolean | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.07` | Issue immutable drop | `BoxOfficeRiskRoute` | Recipient/scope deal term valid | Snapshot/source/freshness/movement delivers | Later scope change is forward-only | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.08` | Evaluate pacing | `BoxOfficeRiskRoute` | Break-even/trajectory reference available | Actionable deviation alert or silence | No reference means no claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.09` | Sell walk-up | `BoxOfficeRiskRoute` | Window permission, manifest units and all-in price | Cash/card sale admits at birth and records ticket/float | Offline oversell from authorized block is honored then reconciled | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.10` | Close box office | `BoxOfficeRiskRoute` | Sales drained; devices reconciled/written off | Certified counters/float/exceptions and attestations version | Unreconciled device blocks close absent reason | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.11` | Request individual refund/exchange | `BoxOfficeRiskRoute` | Ticket/holder/payer/policy and unscanned state | Automatic/excluded/discretionary result or atomic exchange | One obligation; refund returns original payer method | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.12` | Cancel event | `BoxOfficeRiskRoute` | Step-up actor, blast-radius preview and reason | Event cancellation and per-ticket obligations commit | Rail failure never rolls back cancellation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.13` | Reschedule/postpone | `BoxOfficeRiskRoute` | New date or TBC deadline and policy version | Wallet updates and opt-out window opens | Deadline expiry auto-converts cancellation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.14` | Connect external platform | `BoxOfficeRiskRoute` | Venue credentials and capability profile accepted | Verified data/mapping proposal/freshness state saves | Broken connector shows explicit gap | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.15` | Map/ingest external count | `BoxOfficeRiskRoute` | Operator-confirmed show mapping and schema mapping | Source-currency sourced/fresh/provisional values append | No silent auto-bind/conversion | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.16` | Attest manual count | `BoxOfficeRiskRoute` | Authorized actor at close; source known | Immutable attestation/unknown/reconstructed state appends | Concurrent entries remain competing | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.17` | Reconcile count discrepancy | `BoxOfficeRiskRoute` | Two independent sources or explicit single-source state | Chosen number/decider/reason/counter-acceptance record | Platform does not choose | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.18` | Configure/enforce purchase limit | `BoxOfficeRiskRoute` | Show policy enabled and account identified | Accumulated acquisition count enforces before selection | Circumvention signals review, not auto-block | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.19` | Transfer ticket | `BoxOfficeRiskRoute` | Policy/holder/recipient/epoch valid | Same ticket changes holder; old credential invalidates | Reversible until claim; no allowance reset | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.20` | List/buy face-value exchange | `BoxOfficeRiskRoute` | Valid unscanned ticket and waitlist posture | Waitlist-first resale transfers/refunds seller atomically | Unsold state is explicit | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `36.21` | Capture/withdraw consent | `BoxOfficeRiskRoute` | Named party/purpose/fan and policy valid | Consent version/expiry/withdrawal propagates | Transaction unchanged if declined | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/box-office-risk` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fbox-office-risk` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `BoxOfficeRiskRoute` variant `appPage` | `Door access, box office, reconciliation and ticketing risk | WeJammin` | `Work with door access, box office, reconciliation and ticketing risk using current authority, record state, and provenance.` |
| `/app/box-office-risk/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `BoxOfficeRiskRoute` with the matching workbench detail variant | `Record | Door access, box office, reconciliation and ticketing risk | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `BoxOfficeRiskRoute` variant `publicPage` | `Door access, box office, reconciliation and ticketing risk | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `BoxOfficeRiskRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `BoxOfficeRiskRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `DoorReplicasScansAgeWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `BoxofficeCountsDropsWalkupCloseWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TicketRefundsEventChangesWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `ExternalCountsAttestationReconciliationWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TicketLimitsTransferExchangeConsentWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---:|---|---|---|
| `BoxOfficeRiskRoute` public variant | ≤45 KB initial route JS; zero hydration when static | Hydrate only a visible interaction with `client:visible`; no global router | Astro image pipeline emits width/height, AVIF/WebP plus fallback, responsive `srcset`/`sizes`; below-fold images lazy; hero/record identity eager only when LCP | LCP <2.5 s, INP <200 ms, CLS <0.1 at p75 |
| `BoxOfficeRiskRoute` app/admin variant | ≤90 KB initial route JS including shared shell | Each workbench island ≤35 KB initial; editor/media/chart modules split to ≤80 KB lazy chunk and load on explicit entry/visibility; independent fetches parallel | Same optimized image contract; audio/video metadata preload only until explicit play; waveform data lazy and functional | LCP <2.5 s, INP <200 ms, CLS <0.1; interaction feedback same frame |
| `DoorReplicasScansAgeWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `BoxofficeCountsDropsWalkupCloseWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TicketRefundsEventChangesWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `ExternalCountsAttestationReconciliationWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `TicketLimitsTransferExchangeConsentWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `36a-door-replicas-scans-age.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `36b-boxoffice-counts-drops-walkup-close.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `36c-ticket-refunds-event-changes.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `36d-external-counts-attestation-reconciliation.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `36e-ticket-limits-transfer-exchange-consent.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `36a-door-replicas-scans-age.md` | `36A-DOOR-REPLICAS-SCANS-AGE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `DoorReplicasScansAgeWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `36b-boxoffice-counts-drops-walkup-close.md` | `36B-BOXOFFICE-COUNTS-DROPS-WALKUP-CLOSE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `BoxofficeCountsDropsWalkupCloseWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `36c-ticket-refunds-event-changes.md` | `36C-TICKET-REFUNDS-EVENT-CHANGES-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketRefundsEventChangesWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `36d-external-counts-attestation-reconciliation.md` | `36D-EXTERNAL-COUNTS-ATTESTATION-RECONCILIATION-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `ExternalCountsAttestationReconciliationWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `36e-ticket-limits-transfer-exchange-consent.md` | `36E-TICKET-LIMITS-TRANSFER-EXCHANGE-CONSENT-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TicketLimitsTransferExchangeConsentWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `36a-door-replicas-scans-age.md` | Named source schemas | All named response fields | `DoorReplicasScansAgeWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `36b-boxoffice-counts-drops-walkup-close.md` | Named source schemas | `CountQuery`, `CounterSource`, `visibleFields`, `inventory_block_id`, `ticket_product_id`, `ticket_id` | `BoxofficeCountsDropsWalkupCloseWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `36c-ticket-refunds-event-changes.md` | Named source schemas | `discharged` | `TicketRefundsEventChangesWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `36d-external-counts-attestation-reconciliation.md` | Named source schemas | `Origin` | `ExternalCountsAttestationReconciliationWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `36e-ticket-limits-transfer-exchange-consent.md` | Named source schemas | `sold`, `suppression_job_id`, `recipient_notification_job_id` | `TicketLimitsTransferExchangeConsentWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type DoorReplicasScansAgeContractField =
  never;
interface DoorReplicasScansAgeWorkbenchContractFields {
  source: '36a-door-replicas-scans-age.md';
  fields: Readonly<Record<DoorReplicasScansAgeContractField, unknown>>;
}
```

```ts
type BoxofficeCountsDropsWalkupCloseContractField =
  | 'CountQuery'
  | 'CounterSource'
  | 'inventory_block_id'
  | 'ticket_id'
  | 'ticket_product_id'
  | 'visibleFields';
interface BoxofficeCountsDropsWalkupCloseWorkbenchContractFields {
  source: '36b-boxoffice-counts-drops-walkup-close.md';
  fields: Readonly<Record<BoxofficeCountsDropsWalkupCloseContractField, unknown>>;
}
```

```ts
type TicketRefundsEventChangesContractField =
  | 'discharged';
interface TicketRefundsEventChangesWorkbenchContractFields {
  source: '36c-ticket-refunds-event-changes.md';
  fields: Readonly<Record<TicketRefundsEventChangesContractField, unknown>>;
}
```

```ts
type ExternalCountsAttestationReconciliationContractField =
  | 'Origin';
interface ExternalCountsAttestationReconciliationWorkbenchContractFields {
  source: '36d-external-counts-attestation-reconciliation.md';
  fields: Readonly<Record<ExternalCountsAttestationReconciliationContractField, unknown>>;
}
```

```ts
type TicketLimitsTransferExchangeConsentContractField =
  | 'recipient_notification_job_id'
  | 'sold'
  | 'suppression_job_id';
interface TicketLimitsTransferExchangeConsentWorkbenchContractFields {
  source: '36e-ticket-limits-transfer-exchange-consent.md';
  fields: Readonly<Record<TicketLimitsTransferExchangeConsentContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `36a-door-replicas-scans-age.md` | `DoorReplicasScansAgeWorkbenchContractFields.fields` and `DoorReplicasScansAgeWorkbenchProps.contractFields` | `AGE_CHECK_REQUIRED`, `CAUSAL_BATCH_INVALID`, `CONFLICT`, `CORRECTION_FORBIDDEN`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `NOT_FOUND`, `OUTCOME_CLASS_INVALID`, `PII_POLICY_FAILED`, `RATE_LIMITED`, `REPLICA_EXPIRED`, `REPLICA_MISMATCH`, `STEP_UP_REQUIRED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_MISMATCH` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `36b-boxoffice-counts-drops-walkup-close.md` | `BoxofficeCountsDropsWalkupCloseWorkbenchContractFields.fields` and `BoxofficeCountsDropsWalkupCloseWorkbenchProps.contractFields` | `DROP_SCOPE_FORBIDDEN`, `FLOAT_MISMATCH`, `PREVIEW_STALE`, `PROJECTION_FORBIDDEN`, `REFERENCE_UNAVAILABLE`, `SOURCE_STALE` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `36c-ticket-refunds-event-changes.md` | `TicketRefundsEventChangesWorkbenchContractFields.fields` and `TicketRefundsEventChangesWorkbenchProps.contractFields` | `DATE_INVALID`, `DEADLINE_INVALID`, `EXCHANGE_UNAVAILABLE`, `OPT_OUT_POLICY_INVALID`, `PREVIEW_STALE`, `STALE_VERSION`, `STEP_UP_REQUIRED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `36d-external-counts-attestation-reconciliation.md` | `ExternalCountsAttestationReconciliationWorkbenchContractFields.fields` and `ExternalCountsAttestationReconciliationWorkbenchProps.contractFields` | `ATTESTATION_NOT_FOUND`, `CAPABILITY_PROFILE_MISMATCH`, `CONNECTION_NOT_FOUND`, `COUNTERPARTY_MISMATCH`, `CREDENTIAL_INVALID`, `DEPENDENCY_UNAVAILABLE`, `EGRESS_FORBIDDEN`, `EVENT_NOT_FOUND`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `INDEPENDENT_SOURCE_REQUIRED`, `PROVIDER_PAYLOAD_INVALID`, `RATE_LIMITED`, `SOURCE_FORBIDDEN`, `UNAUTHENTICATED`, `VALIDATION_FAILED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `36e-ticket-limits-transfer-exchange-consent.md` | `TicketLimitsTransferExchangeConsentWorkbenchContractFields.fields` and `TicketLimitsTransferExchangeConsentWorkbenchProps.contractFields` | `ACCESS_ACK_REQUIRED`, `CLAIM_TOKEN_INVALID`, `CONSENT_SCOPE_MISMATCH`, `DEPENDENCY_UNAVAILABLE`, `EVENT_OR_POLICY_NOT_FOUND`, `EXCHANGE_FORBIDDEN`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `PURPOSE_INVALID`, `RATE_LIMITED`, `RESERVATION_CONFLICT`, `TEXT_STALE`, `TICKET_OR_LISTING_NOT_FOUND`, `TICKET_OR_TRANSFER_NOT_FOUND`, `TRANSFER_FORBIDDEN`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `WAITLIST_REQUIRED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
