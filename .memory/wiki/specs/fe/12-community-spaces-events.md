...
> **BE Source**: [12a-scenes-stewardship-seeding.md](../be/12a-scenes-stewardship-seeding.md), [12b-craft-forums-qa.md](../be/12b-craft-forums-qa.md), [12c-contests-submissions-judging.md](../be/12c-contests-submissions-judging.md), [12d-informal-listening-conference-events.md](../be/12d-informal-listening-conference-events.md)
> **IA Source**: [12-community-spaces-events.md](../ia/12-community-spaces-events.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [12-community-spaces-events.md](../ia/12-community-spaces-events.md) in full.
- **BE sources**: [12a-scenes-stewardship-seeding.md](../be/12a-scenes-stewardship-seeding.md), [12b-craft-forums-qa.md](../be/12b-craft-forums-qa.md), [12c-contests-submissions-judging.md](../be/12c-contests-submissions-judging.md), [12d-informal-listening-conference-events.md](../be/12d-informal-listening-conference-events.md).
...
|---|---|---|
| Classification and scope | `12-community-spaces-events.md`; BE index | Shard boundary and completed BE split group |
...
### `CommunitySpacesEventsRoute` (Astro server route)

```ts
interface CommunitySpacesEventsRouteProps {
  children?: never;
...
### `ScenesStewardshipSeedingWorkbench` (bounded React island)

**BE owner**: `12a-scenes-stewardship-seeding.md`

```ts
interface ScenesStewardshipSeedingWorkbenchProps {
  contractFields: ScenesStewardshipSeedingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly ScenesStewardshipSeedingRecord[]>;
  actorId: string;
...
interface ScenesStewardshipSeedingRecord {
  id: string;
  version: string;
  state: string;
...
### `CraftForumsQaWorkbench` (bounded React island)

**BE owner**: `12b-craft-forums-qa.md`

```ts
interface CraftForumsQaWorkbenchProps {
  contractFields: CraftForumsQaWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly CraftForumsQaRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
### `ContestsSubmissionsJudgingWorkbench` (bounded React island)

**BE owner**: `12c-contests-submissions-judging.md`

```ts
interface ContestsSubmissionsJudgingWorkbenchProps {
  contractFields: ContestsSubmissionsJudgingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly ContestsSubmissionsJudgingRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface ContestsSubmissionsJudgingRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `InformalListeningConferenceEventsWorkbench` (bounded React island)

**BE owner**: `12d-informal-listening-conference-events.md`

```ts
interface InformalListeningConferenceEventsWorkbenchProps {
  contractFields: InformalListeningConferenceEventsWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly InformalListeningConferenceEventsRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface InformalListeningConferenceEventsRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/community-spaces-events` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/community-spaces-events/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|---|
| `SPC-01` Discover/join scene | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-02` Leave/dismiss scene | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-03` View scene | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-04` Steward scene | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-05` Seed/claim premises/event | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-06` Ask/answer craft question | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-07` Publish contest brief | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-08` Submit contest entry | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-09` Judge/award contest | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-10` Confirm local jam/open mic | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-11` Join peer listening room | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-12` Enable conference mode | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `SPC-13` Persist event relationship | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `12-community-spaces-events.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `12-community-spaces-events.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `12-community-spaces-events.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `12-community-spaces-events.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `12-community-spaces-events.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `12-community-spaces-events.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `12-community-spaces-events.md` Accessibility |
...
|---|---|---|---|---|
| `CommunitySpacesEventsRoute` | `CommunitySpacesEventsRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `12-community-spaces-events.md` user flows/accessibility; design-system page archetypes |
| `ScenesStewardshipSeedingWorkbench` | `ScenesStewardshipSeedingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `12a-scenes-stewardship-seeding.md` request/response fields; `12-community-spaces-events.md` interactions/access rules |
| `CraftForumsQaWorkbench` | `CraftForumsQaWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `12b-craft-forums-qa.md` request/response fields; `12-community-spaces-events.md` interactions/access rules |
| `ContestsSubmissionsJudgingWorkbench` | `ContestsSubmissionsJudgingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `12c-contests-submissions-judging.md` request/response fields; `12-community-spaces-events.md` interactions/access rules |
| `InformalListeningConferenceEventsWorkbench` | `InformalListeningConferenceEventsWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `12d-informal-listening-conference-events.md` request/response fields; `12-community-spaces-events.md` interactions/access rules |
...
|---|---|---|---|
| `SPC-01` Discover/join scene | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-01` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-02` Leave/dismiss scene | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-02` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-03` View scene | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-03` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-04` Steward scene | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-04` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-05` Seed/claim premises/event | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-05` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-06` Ask/answer craft question | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-06` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-07` Publish contest brief | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-07` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-08` Submit contest entry | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-08` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-09` Judge/award contest | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-09` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-10` Confirm local jam/open mic | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-10` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-11` Join peer listening room | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-11` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-12` Enable conference mode | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-12` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `SPC-13` Persist event relationship | `CommunitySpacesEventsRoute` orchestrates; `ScenesStewardshipSeedingWorkbench`, `CraftForumsQaWorkbench`, `ContestsSubmissionsJudgingWorkbench`, `InformalListeningConferenceEventsWorkbench` renders the relevant BE response and command state | `12-community-spaces-events.md` Interactions row `SPC-13` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
...
|---|---|---|---|---|
| `/app/community-spaces-events` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fcommunity-spaces-events` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `CommunitySpacesEventsRoute` variant `appPage` | `Communities, participatory spaces and events | WeJammin` | `Work with communities, participatory spaces and events using current authority, record state, and provenance.` |
| `/app/community-spaces-events/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `CommunitySpacesEventsRoute` with the matching workbench detail variant | `Record | Communities, participatory spaces and events | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `CommunitySpacesEventsRoute` variant `publicPage` | `Communities, participatory spaces and events | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `CommunitySpacesEventsRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `CommunitySpacesEventsRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
...
| `CraftForumsQaWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `ContestsSubmissionsJudgingWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `InformalListeningConferenceEventsWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
| `CraftForumsQaWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `ContestsSubmissionsJudgingWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `InformalListeningConferenceEventsWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `12a-scenes-stewardship-seeding.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `12b-craft-forums-qa.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `12c-contests-submissions-judging.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `12d-informal-listening-conference-events.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `12a-scenes-stewardship-seeding.md` | `12A-SCENES-STEWARDSHIP-SEEDING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `ScenesStewardshipSeedingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `12b-craft-forums-qa.md` | `12B-CRAFT-FORUMS-QA-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `CraftForumsQaWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `12c-contests-submissions-judging.md` | `12C-CONTESTS-SUBMISSIONS-JUDGING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `ContestsSubmissionsJudgingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `12d-informal-listening-conference-events.md` | `12D-INFORMAL-LISTENING-CONFERENCE-EVENTS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `InformalListeningConferenceEventsWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `12a-scenes-stewardship-seeding.md` | Named source schemas | `scene`, `scene_membership`, `scene_density_projection`, `scene_stewardship_threshold_policy`, `scene_stewardship`, `seed_premises`, `seed_event`, `scene_seed_records`, `SceneViewSuccess`, `anon` | `ScenesStewardshipSeedingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `12b-craft-forums-qa.md` | Named source schemas | `forum_thread`, `craft_threads`, `forum_post`, `craft_content_versions`, `anon`, `open`, `craft_moderation_actions`, `answer`, `edit`, `accept_answer`, `answered`, `withdraw`, `withdrawn`, `moderate`, `retain`, `limit`, `limited`, `remove`, `removed`, `expectedContentVersion` | `CraftForumsQaWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `12c-contests-submissions-judging.md` | Named source schemas | `contest_submission`, `prize_commitment`, `contest_brief_version`, `contest_versions`, `judge_appointment`, `contest_judge_assignments`, `contest_verdict`, `contest_verdicts`, `anon`, `draft`, `published`, `cancelled`, `closesAt`, `judging`, `finalized`, `awarded`, `verified`, `revoked`, `expired`, `submitted`, `eligible`, `ineligible`, `withdrawn`, `finalist`, `winner`, `assigned`, `recused`, `completed`, `corrected`, `contest_scores` | `ContestsSubmissionsJudgingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `12d-informal-listening-conference-events.md` | Named source schemas | `informal_event_listing`, `listening_room_scope`, `conference_attendee_grant`, `space_audit_event`, `anon` | `InformalListeningConferenceEventsWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type ScenesStewardshipSeedingContractField =
  | 'SceneViewSuccess'
  | 'anon'
  | 'scene'
  | 'scene_density_projection'
  | 'scene_membership'
  | 'scene_seed_records'
  | 'scene_stewardship'
  | 'scene_stewardship_threshold_policy'
  | 'seed_event'
  | 'seed_premises';
interface ScenesStewardshipSeedingWorkbenchContractFields {
  source: '12a-scenes-stewardship-seeding.md';
  fields: Readonly<Record<ScenesStewardshipSeedingContractField, unknown>>;
}
```

```ts
type CraftForumsQaContractField =
  | 'accept_answer'
  | 'anon'
  | 'answer'
  | 'answered'
  | 'craft_content_versions'
  | 'craft_moderation_actions'
  | 'craft_threads'
  | 'edit'
  ...
  | 'withdraw'
  | 'withdrawn';
interface CraftForumsQaWorkbenchContractFields {
  source: '12b-craft-forums-qa.md';
  fields: Readonly<Record<CraftForumsQaContractField, unknown>>;
}
```

```ts
type ContestsSubmissionsJudgingContractField =
  | 'anon'
  | 'assigned'
  | 'awarded'
  | 'cancelled'
  | 'closesAt'
  | 'completed'
  | 'contest_brief_version'
  | 'contest_judge_assignments'
  | 'contest_scores'
  | 'contest_submission'
  | 'contest_verdict'
  | 'contest_verdicts'
  ...
  | 'winner'
  | 'withdrawn';
interface ContestsSubmissionsJudgingWorkbenchContractFields {
  source: '12c-contests-submissions-judging.md';
  fields: Readonly<Record<ContestsSubmissionsJudgingContractField, unknown>>;
}
```

```ts
type InformalListeningConferenceEventsContractField =
  | 'anon'
  | 'conference_attendee_grant'
  | 'informal_event_listing'
  | 'listening_room_scope'
  | 'space_audit_event';
interface InformalListeningConferenceEventsWorkbenchContractFields {
  source: '12d-informal-listening-conference-events.md';
  fields: Readonly<Record<InformalListeningConferenceEventsContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `12a-scenes-stewardship-seeding.md` | `ScenesStewardshipSeedingWorkbenchContractFields.fields` and `ScenesStewardshipSeedingWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `EVIDENCE_INVALID`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `MODERATOR_EVIDENCE_INVALID`, `NOT_FOUND`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `12b-craft-forums-qa.md` | `CraftForumsQaWorkbenchContractFields.fields` and `CraftForumsQaWorkbenchProps.contractFields` | `DEPENDENCY_UNAVAILABLE`, `DUPLICATE_CONTENT`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `MODERATION_DECISION_INVALID`, `NOT_FOUND`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `12c-contests-submissions-judging.md` | `ContestsSubmissionsJudgingWorkbenchContractFields.fields` and `ContestsSubmissionsJudgingWorkbenchProps.contractFields` | `CONTEST_CANCELLED`, `DEPENDENCY_UNAVAILABLE`, `DUPLICATE_SCORE`, `DUPLICATE_SUBMISSION`, `DUPLICATE_SUBMISSION_OR_SCORE`, `ELIGIBILITY_OR_RIGHTS_FAILED`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `JUDGE_CONFLICT_OR_QUORUM_FAILED`, `NOT_FOUND`, `PRIZE_EVIDENCE_INVALID`, `RATE_LIMITED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `12d-informal-listening-conference-events.md` | `InformalListeningConferenceEventsWorkbenchContractFields.fields` and `InformalListeningConferenceEventsWorkbenchProps.contractFields` | `BILATERAL_CONSENT_REQUIRED`, `DEPENDENCY_UNAVAILABLE`, `DUPLICATE_OR_CONSENT_CONFLICT`, `FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `NOT_FOUND`, `RATE_LIMITED`, `SOURCE_OR_ELIGIBILITY_INVALID`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_CONFLICT`, `WINDOW_OR_GRANT_EXPIRED`, `WINDOW_OR_ROOM_EXPIRED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
