# Gear transactions, fulfilment and possession models: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [26a-gear-offers-cart-checkout.md](../be/26a-gear-offers-cart-checkout.md), [26b-gear-logistics-order-lifecycle.md](../be/26b-gear-logistics-order-lifecycle.md), [26c-gear-remedies-settlement-transfers.md](../be/26c-gear-remedies-settlement-transfers.md), [26d-gear-pickup-service-warranty.md](../be/26d-gear-pickup-service-warranty.md), [26e-gear-future-commerce-gates.md](../be/26e-gear-future-commerce-gates.md)
> **IA Source**: [26-gear-commerce-fulfilment.md](../ia/26-gear-commerce-fulfilment.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [26-gear-commerce-fulfilment.md](../ia/26-gear-commerce-fulfilment.md) in full.
- **BE sources**: [26a-gear-offers-cart-checkout.md](../be/26a-gear-offers-cart-checkout.md), [26b-gear-logistics-order-lifecycle.md](../be/26b-gear-logistics-order-lifecycle.md), [26c-gear-remedies-settlement-transfers.md](../be/26c-gear-remedies-settlement-transfers.md), [26d-gear-pickup-service-warranty.md](../be/26d-gear-pickup-service-warranty.md), [26e-gear-future-commerce-gates.md](../be/26e-gear-future-commerce-gates.md).
...
|---|---|---|
| Classification and scope | `26-gear-commerce-fulfilment.md`; BE index | Shard boundary and completed BE split group |
...
### `GearCommerceFulfilmentRoute` (Astro server route)

```ts
interface GearCommerceFulfilmentRouteProps {
  children?: never;
...
### `GearOffersCartCheckoutWorkbench` (bounded React island)

**BE owner**: `26a-gear-offers-cart-checkout.md`

```ts
interface GearOffersCartCheckoutWorkbenchProps {
  contractFields: GearOffersCartCheckoutWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearOffersCartCheckoutRecord[]>;
  actorId: string;
...
interface GearOffersCartCheckoutRecord {
  id: string;
  version: string;
  state: string;
...
### `GearLogisticsOrderLifecycleWorkbench` (bounded React island)

**BE owner**: `26b-gear-logistics-order-lifecycle.md`

```ts
interface GearLogisticsOrderLifecycleWorkbenchProps {
  contractFields: GearLogisticsOrderLifecycleWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearLogisticsOrderLifecycleRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearLogisticsOrderLifecycleRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `GearRemediesSettlementTransfersWorkbench` (bounded React island)

**BE owner**: `26c-gear-remedies-settlement-transfers.md`

```ts
interface GearRemediesSettlementTransfersWorkbenchProps {
  contractFields: GearRemediesSettlementTransfersWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearRemediesSettlementTransfersRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearRemediesSettlementTransfersRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `GearPickupServiceWarrantyWorkbench` (bounded React island)

**BE owner**: `26d-gear-pickup-service-warranty.md`

```ts
interface GearPickupServiceWarrantyWorkbenchProps {
  contractFields: GearPickupServiceWarrantyWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearPickupServiceWarrantyRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearPickupServiceWarrantyRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `GearFutureCommerceGatesWorkbench` (bounded React island)

**BE owner**: `26e-gear-future-commerce-gates.md`

```ts
interface GearFutureCommerceGatesWorkbenchProps {
  contractFields: GearFutureCommerceGatesWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearFutureCommerceGatesRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface GearFutureCommerceGatesRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/gear-commerce-fulfilment` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/gear-commerce-fulfilment/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `26-gear-commerce-fulfilment.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `26-gear-commerce-fulfilment.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `26-gear-commerce-fulfilment.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `26-gear-commerce-fulfilment.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `26-gear-commerce-fulfilment.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `26-gear-commerce-fulfilment.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `26-gear-commerce-fulfilment.md` Accessibility |
...
|---|---|---|---|---|
| `GearCommerceFulfilmentRoute` | `GearCommerceFulfilmentRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `26-gear-commerce-fulfilment.md` user flows/accessibility; design-system page archetypes |
| `GearOffersCartCheckoutWorkbench` | `GearOffersCartCheckoutWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `26a-gear-offers-cart-checkout.md` request/response fields; `26-gear-commerce-fulfilment.md` interactions/access rules |
| `GearLogisticsOrderLifecycleWorkbench` | `GearLogisticsOrderLifecycleWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `26b-gear-logistics-order-lifecycle.md` request/response fields; `26-gear-commerce-fulfilment.md` interactions/access rules |
| `GearRemediesSettlementTransfersWorkbench` | `GearRemediesSettlementTransfersWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `26c-gear-remedies-settlement-transfers.md` request/response fields; `26-gear-commerce-fulfilment.md` interactions/access rules |
| `GearPickupServiceWarrantyWorkbench` | `GearPickupServiceWarrantyWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `26d-gear-pickup-service-warranty.md` request/response fields; `26-gear-commerce-fulfilment.md` interactions/access rules |
| `GearFutureCommerceGatesWorkbench` | `GearFutureCommerceGatesWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `26e-gear-future-commerce-gates.md` request/response fields; `26-gear-commerce-fulfilment.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `26.01` | Make/counter offer | `GearCommerceFulfilmentRoute` | Listing allows offers; buyer eligible for destination/item | Private structured thread stores item-price offer, expiry and delivered-cost context | Ineligible route explains why; pending offer never reserves | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.02` | Accept/decline offer | `GearCommerceFulfilmentRoute` | Seller controls listing; offer/version active | Acceptance claims unit atomically and starts checkout deadline; all competing offers void | Lost unit, hold or stale offer returns explicit terminal reason | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.03` | Maintain mixed-intent cart | `GearCommerceFulfilmentRoute` | Authenticated user | Cart stores independent intents and displays required checkout groups | No line is silently dropped; cart never claims inventory | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.04` | Resolve checkout eligibility | `GearCommerceFulfilmentRoute` | Buying party, one destination/group, current facts | Per-shipment price, voltage, fulfilment, policy and screening results itemize | Ineligible lines remain in cart with reasons and remediation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.05` | Claim and authorize checkout | `GearCommerceFulfilmentRoute` | All selected lines eligible; versions current | Claims all group lines, reconfirms actual set, authorizes once, creates independent orders | Partial claim losers are removed before charge; no half-created order | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.06` | Request freight quote | `GearCommerceFulfilmentRoute` | Route classified freight/oversize | Purpose-bound request returns quote/options/requirements with validity | No parcel fallback; request holds no inventory/payment | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.07` | Commit domestic shipment | `GearCommerceFulfilmentRoute` | Current quote/coverage/eligibility and order exist | Label/booking evidence, packing standard and delivery requirements pin to shipment | Coverage or carrier failure blocks dispatch path and offers retry/change | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.08` | Manage order lifecycle | `GearCommerceFulfilmentRoute` | Actor authorized for action/current state | Idempotent transition advances concurrent clocks and preserves offline draft | Stale transition fails loudly with canonical state | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.09` | Amend after purchase | `GearCommerceFulfilmentRoute` | Seller discovers material disclosure change pre-dispatch | Dispatch pauses; buyer accepts, accepts reduction or voids for full refund | No answer resolves toward void under configurable deadline | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.10` | Confirm dispatch/delivery | `GearCommerceFulfilmentRoute` | Packing evidence complete; carrier event or pickup confirmation | Evidence appends; verified delivery starts inspection/settlement clocks | Carrier silence remains silence; misdelivery triggers immediate recall/refund workflow | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.11` | Open damage claim | `GearCommerceFulfilmentRoute` | Buyer files before remedy deadline with evidence | Settlement/title suspend atomically; repair/refund/partial remedies proposed | Tie with auto-settle resolves to claim; later filing becomes post-settlement case | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.12` | Request return | `GearCommerceFulfilmentRoute` | Order/line eligible by law or policy | Reason-specific authorization, return freight, evidence and deadlines compose | Seller cooperation/recovery/duty reclaim never gates buyer remedy | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.13` | Inspect returned unit | `GearCommerceFulfilmentRoute` | Return arrives with custody-boundary evidence | Condition delta determines full/partial/diminished-value refund, never refusal | Conflicting evidence escalates through dispute; clocks remain bounded | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.14` | Settle order | `GearCommerceFulfilmentRoute` | Verified delivery/remedy clocks complete, no active hold | Escrow releases once; order becomes financially closed/evidentially open | Payout failure routes human reconciliation and never reverts settlement | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.15` | Emit ownership transfer | `GearCommerceFulfilmentRoute` | Settlement line resolves canonical gear/party | Exactly-once transfer event records object, buyer party and settlement time | Registry outage retries asynchronously; collision appends contested, never misattaches | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.16` | Reverse ownership event | `GearCommerceFulfilmentRoute` | Return/rescission completed | Compensating event references original transfer without deleting history | Refund does not wait for registry write | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.17` | Arrange platform pickup | `GearCommerceFulfilmentRoute` | Adult buyer/seller, platform-settled listing and safe meetup choice | Protected location release, checklist, confirmations and pickup evidence | No home address publication; unilateral/no-show path preserves funds and safety | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.18` | Complete off-platform pickup | `GearCommerceFulfilmentRoute` | Seller chose disclosed off-platform settlement branch | Platform records arrangement outcome only and offers manual Shard 23 transfer handshake | No automatic ownership, comp, escrow, refund or platform payment claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.19` | Add pre-dispatch service | `GearCommerceFulfilmentRoute` | Eligible domain-14 service and custody/liability path accepted | Separate service engagement sequences before shipment; outcome can amend order | Contradiction gives buyer cancel/reprice/continue election; seller service never called independent | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.20` | Route RMA/warranty | `GearCommerceFulfilmentRoute` | Unit/order/warranty evidence exists | Eligible order creates unit-scoped manufacturer warranty evidence/registration; Shard 14 repair and Shard 23 service history receive scoped handoff | “No warranty” stays explicit; provider submission waits for an admitted adapter; platform offers no own warranty | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.21` | Run future international gate | `GearCommerceFulfilmentRoute` | Feature flag/provider/counsel enabled, destination entered | Current tariff/CITES/landed-cost determination commits or rejects route | Unknown/recompute failure fails closed; no under-declaration or stale permission | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `26.22` | Run future auction/ISO/dealer flows | `GearCommerceFulfilmentRoute` | Consumer launch and separate admission gates complete | Capability-specific state machines reuse claims, custody and evidence | Disabled features have no hidden partial path or substitute persona | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/gear-commerce-fulfilment` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fgear-commerce-fulfilment` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `GearCommerceFulfilmentRoute` variant `appPage` | `Gear transactions, fulfilment and possession models | WeJammin` | `Work with gear transactions, fulfilment and possession models using current authority, record state, and provenance.` |
| `/app/gear-commerce-fulfilment/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `GearCommerceFulfilmentRoute` with the matching workbench detail variant | `Record | Gear transactions, fulfilment and possession models | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `GearCommerceFulfilmentRoute` variant `publicPage` | `Gear transactions, fulfilment and possession models | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `GearCommerceFulfilmentRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `GearCommerceFulfilmentRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
...
| `GearLogisticsOrderLifecycleWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearRemediesSettlementTransfersWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearPickupServiceWarrantyWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `GearFutureCommerceGatesWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
| `GearLogisticsOrderLifecycleWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `GearRemediesSettlementTransfersWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `GearPickupServiceWarrantyWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `GearFutureCommerceGatesWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `26a-gear-offers-cart-checkout.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `26b-gear-logistics-order-lifecycle.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `26c-gear-remedies-settlement-transfers.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `26d-gear-pickup-service-warranty.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `26e-gear-future-commerce-gates.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `26a-gear-offers-cart-checkout.md` | `26A-GEAR-OFFERS-CART-CHECKOUT-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearOffersCartCheckoutWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `26b-gear-logistics-order-lifecycle.md` | `26B-GEAR-LOGISTICS-ORDER-LIFECYCLE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearLogisticsOrderLifecycleWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `26c-gear-remedies-settlement-transfers.md` | `26C-GEAR-REMEDIES-SETTLEMENT-TRANSFERS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearRemediesSettlementTransfersWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `26d-gear-pickup-service-warranty.md` | `26D-GEAR-PICKUP-SERVICE-WARRANTY-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearPickupServiceWarrantyWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `26e-gear-future-commerce-gates.md` | `26E-GEAR-FUTURE-COMMERCE-GATES-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearFutureCommerceGatesWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `26a-gear-offers-cart-checkout.md` | Named source schemas | `aggregateId`, `bodyHash` | `GearOffersCartCheckoutWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `26b-gear-logistics-order-lifecycle.md` | Named source schemas | `providerEventId`, `aggregateId`, `payloadHash` | `GearLogisticsOrderLifecycleWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `26c-gear-remedies-settlement-transfers.md` | Named source schemas | All named response fields | `GearRemediesSettlementTransfersWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `26d-gear-pickup-service-warranty.md` | Named source schemas | All named response fields | `GearPickupServiceWarrantyWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `26e-gear-future-commerce-gates.md` | Named source schemas | All named response fields | `GearFutureCommerceGatesWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type GearOffersCartCheckoutContractField =
  | 'aggregateId'
  | 'bodyHash';
interface GearOffersCartCheckoutWorkbenchContractFields {
  source: '26a-gear-offers-cart-checkout.md';
  fields: Readonly<Record<GearOffersCartCheckoutContractField, unknown>>;
}
```

```ts
type GearLogisticsOrderLifecycleContractField =
  | 'aggregateId'
  | 'payloadHash'
  | 'providerEventId';
interface GearLogisticsOrderLifecycleWorkbenchContractFields {
  source: '26b-gear-logistics-order-lifecycle.md';
  fields: Readonly<Record<GearLogisticsOrderLifecycleContractField, unknown>>;
}
```

```ts
type GearRemediesSettlementTransfersContractField =
  never;
interface GearRemediesSettlementTransfersWorkbenchContractFields {
  source: '26c-gear-remedies-settlement-transfers.md';
  fields: Readonly<Record<GearRemediesSettlementTransfersContractField, unknown>>;
}
```

```ts
type GearPickupServiceWarrantyContractField =
  never;
interface GearPickupServiceWarrantyWorkbenchContractFields {
  source: '26d-gear-pickup-service-warranty.md';
  fields: Readonly<Record<GearPickupServiceWarrantyContractField, unknown>>;
}
```

```ts
type GearFutureCommerceGatesContractField =
  never;
interface GearFutureCommerceGatesWorkbenchContractFields {
  source: '26e-gear-future-commerce-gates.md';
  fields: Readonly<Record<GearFutureCommerceGatesContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `26a-gear-offers-cart-checkout.md` | `GearOffersCartCheckoutWorkbenchContractFields.fields` and `GearOffersCartCheckoutWorkbenchProps.contractFields` | `BUYING_PARTY_FORBIDDEN`, `BUY_FORBIDDEN`, `CART_NOT_FOUND`, `CART_VERSION_CONFLICT`, `CHECKOUT_CLAIM_CONFLICT`, `CHECKOUT_COMMIT_FORBIDDEN`, `CHECKOUT_CONTEXT_NOT_FOUND`, `CHECKOUT_PREPARATION_NOT_FOUND`, `CHECKOUT_PREPARATION_STALE`, `DEPENDENCY_UNAVAILABLE`, `INVALID_INPUT`, `OFFER_NOT_FOUND`, `OFFER_RESPONSE_FORBIDDEN`, `OFFER_TARGET_NOT_FOUND`, `OFFER_VERSION_CONFLICT`, `RATE_LIMITED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `26b-gear-logistics-order-lifecycle.md` | `GearLogisticsOrderLifecycleWorkbenchContractFields.fields` and `GearLogisticsOrderLifecycleWorkbenchProps.contractFields` | `AMENDMENT_FORBIDDEN`, `BUYER_ELECTION_REQUIRED`, `COVERAGE_REQUIRED`, `DELIVERY_CONTEXT_NOT_FOUND`, `DELIVERY_FORBIDDEN`, `DELIVERY_VERIFICATION_REQUIRED`, `DEPENDENCY_UNAVAILABLE`, `FREIGHT_CONTEXT_NOT_FOUND`, `FREIGHT_QUOTE_FORBIDDEN`, `FREIGHT_QUOTE_UNAVAILABLE`, `INVALID_INPUT`, `ORDER_NOT_FOUND`, `ORDER_STATE_CONFLICT`, `ORDER_TRANSITION_FORBIDDEN`, `ORDER_VERSION_CONFLICT`, `PROVIDER_EVENT_DUPLICATE`, `PROVIDER_EVENT_INVALID`, `QUOTE_VERSION_CONFLICT`, `RATE_LIMITED`, `SHIPMENT_COMMIT_FORBIDDEN` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `26c-gear-remedies-settlement-transfers.md` | `GearRemediesSettlementTransfersWorkbenchContractFields.fields` and `GearRemediesSettlementTransfersWorkbenchProps.contractFields` | `DAMAGE_CASE_CONFLICT`, `DAMAGE_CLAIM_FORBIDDEN`, `DEPENDENCY_UNAVAILABLE`, `INSPECTION_DISPOSITION_INVALID`, `ORDER_NOT_FOUND`, `ORDER_VERSION_CONFLICT`, `RATE_LIMITED`, `RETURN_FORBIDDEN`, `RETURN_INSPECTION_FORBIDDEN`, `RETURN_NOT_FOUND`, `RETURN_STATE_CONFLICT`, `REVERSAL_BASIS_CONFLICT`, `REVERSAL_EVIDENCE_REQUIRED`, `SETTLEMENT_FORBIDDEN`, `SETTLEMENT_NOT_FOUND`, `SETTLEMENT_TOTAL_INVALID`, `TRANSFER_BASIS_INVALID`, `TRANSFER_INTENT_FORBIDDEN`, `TRANSFER_INTENT_NOT_FOUND`, `TRANSFER_REVERSAL_FORBIDDEN`, `VALIDATION_FAILED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `26d-gear-pickup-service-warranty.md` | `GearPickupServiceWarrantyWorkbenchContractFields.fields` and `GearPickupServiceWarrantyWorkbenchProps.contractFields` | `BUYER_ELECTION_REQUIRED`, `DEPENDENCY_UNAVAILABLE`, `LOCATION_RELEASE_UNAVAILABLE`, `ORDER_NOT_FOUND`, `ORDER_VERSION_CONFLICT`, `PICKUP_ADULT_REQUIRED`, `PICKUP_ARRANGEMENT_FORBIDDEN`, `PICKUP_CONFIRMATION_CONFLICT`, `PICKUP_CONFIRMATION_FORBIDDEN`, `PICKUP_MODE_CONFLICT`, `PICKUP_NOT_FOUND`, `PICKUP_VENUE_INVALID`, `RATE_LIMITED`, `SERVICE_POLICY_CONFLICT`, `SERVICE_PROVIDER_UNAVAILABLE`, `SERVICE_REQUEST_FORBIDDEN`, `VALIDATION_FAILED`, `WARRANTY_EVIDENCE_REQUIRED`, `WARRANTY_PROVIDER_INVALID`, `WARRANTY_PROVIDER_UNAVAILABLE`, `WARRANTY_ROUTE_FORBIDDEN` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `26e-gear-future-commerce-gates.md` | `GearFutureCommerceGatesWorkbenchContractFields.fields` and `GearFutureCommerceGatesWorkbenchProps.contractFields` | `ADMISSION_PROVIDER_UNAVAILABLE`, `CAPABILITY_TARGET_NOT_FOUND`, `CAPABILITY_VERSION_CONFLICT`, `COMPLIANCE_PROVIDER_UNAVAILABLE`, `DEPENDENCY_UNAVAILABLE`, `DETERMINATION_VERSION_CONFLICT`, `FUTURE_GATE_FORBIDDEN`, `GATE_TARGET_NOT_FOUND`, `INTERNATIONAL_GATE_FORBIDDEN`, `RATE_LIMITED`, `VALIDATION_FAILED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
