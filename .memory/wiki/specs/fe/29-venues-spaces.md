# Venues, studios and spaces: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [29a-place-room-authority-status.md](../be/29a-place-room-authority-status.md), [29b-room-specs-accessibility-conformance.md](../be/29b-room-specs-accessibility-conformance.md), [29c-room-calendars-holds-enquiries.md](../be/29c-room-calendars-holds-enquiries.md), [29d-room-reservations-series-handoff.md](../be/29d-room-reservations-series-handoff.md)
> **IA Source**: [29-venues-spaces.md](../ia/29-venues-spaces.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [29-venues-spaces.md](../ia/29-venues-spaces.md) in full.
- **BE sources**: [29a-place-room-authority-status.md](../be/29a-place-room-authority-status.md), [29b-room-specs-accessibility-conformance.md](../be/29b-room-specs-accessibility-conformance.md), [29c-room-calendars-holds-enquiries.md](../be/29c-room-calendars-holds-enquiries.md), [29d-room-reservations-series-handoff.md](../be/29d-room-reservations-series-handoff.md).
...
|---|---|---|
| Classification and scope | `29-venues-spaces.md`; BE index | Shard boundary and completed BE split group |
...
### `VenuesSpacesRoute` (Astro server route)

```ts
interface VenuesSpacesRouteProps {
  children?: never;
...
### `PlaceRoomAuthorityStatusWorkbench` (bounded React island)

**BE owner**: `29a-place-room-authority-status.md`

```ts
interface PlaceRoomAuthorityStatusWorkbenchProps {
  contractFields: PlaceRoomAuthorityStatusWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly PlaceRoomAuthorityStatusRecord[]>;
  actorId: string;
...
interface PlaceRoomAuthorityStatusRecord {
  id: string;
  version: string;
  state: string;
...
### `RoomSpecsAccessibilityConformanceWorkbench` (bounded React island)

**BE owner**: `29b-room-specs-accessibility-conformance.md`

```ts
interface RoomSpecsAccessibilityConformanceWorkbenchProps {
  contractFields: RoomSpecsAccessibilityConformanceWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly RoomSpecsAccessibilityConformanceRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface RoomSpecsAccessibilityConformanceRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `RoomCalendarsHoldsEnquiriesWorkbench` (bounded React island)

**BE owner**: `29c-room-calendars-holds-enquiries.md`

```ts
interface RoomCalendarsHoldsEnquiriesWorkbenchProps {
  contractFields: RoomCalendarsHoldsEnquiriesWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly RoomCalendarsHoldsEnquiriesRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface RoomCalendarsHoldsEnquiriesRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `RoomReservationsSeriesHandoffWorkbench` (bounded React island)

**BE owner**: `29d-room-reservations-series-handoff.md`

```ts
interface RoomReservationsSeriesHandoffWorkbenchProps {
  contractFields: RoomReservationsSeriesHandoffWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly RoomReservationsSeriesHandoffRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface RoomReservationsSeriesHandoffRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/venues-spaces` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/venues-spaces/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `29-venues-spaces.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `29-venues-spaces.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `29-venues-spaces.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `29-venues-spaces.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `29-venues-spaces.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `29-venues-spaces.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `29-venues-spaces.md` Accessibility |
...
|---|---|---|---|---|
| `VenuesSpacesRoute` | `VenuesSpacesRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `29-venues-spaces.md` user flows/accessibility; design-system page archetypes |
| `PlaceRoomAuthorityStatusWorkbench` | `PlaceRoomAuthorityStatusWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `29a-place-room-authority-status.md` request/response fields; `29-venues-spaces.md` interactions/access rules |
| `RoomSpecsAccessibilityConformanceWorkbench` | `RoomSpecsAccessibilityConformanceWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `29b-room-specs-accessibility-conformance.md` request/response fields; `29-venues-spaces.md` interactions/access rules |
| `RoomCalendarsHoldsEnquiriesWorkbench` | `RoomCalendarsHoldsEnquiriesWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `29c-room-calendars-holds-enquiries.md` request/response fields; `29-venues-spaces.md` interactions/access rules |
| `RoomReservationsSeriesHandoffWorkbench` | `RoomReservationsSeriesHandoffWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `29d-room-reservations-series-handoff.md` request/response fields; `29-venues-spaces.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `29.01` | Create or find place | `VenuesSpacesRoute` | Actor may contribute; normalized location available | Existing candidate is shown or seeded/community record created with source provenance | Race may create candidate duplicate; never silently merge | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.02` | Claim place or room | `VenuesSpacesRoute` | Shard 01 acting party; eligible proof route | Provisional/full authority and scoped capabilities recorded | Failed proof reveals no private anchor; review/appeal retained | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.03` | Add or change room | `VenuesSpacesRoute` | Authorized operator; place type compatible | Room, operating party, relationships and effective version commit | Type auto-extends on add; removal with live room blocks | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.04` | Retire, supersede or outage room | `VenuesSpacesRoute` | No unsafe future transition | History remains; availability closes from effective time | Confirmed reservations block retire/supersede or require explicit migration | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.05` | Publish room specification | `VenuesSpacesRoute` | Authorized operator/contributor by field class | Typed field revision, source, age and completeness tier publish | Contradictions flag; malformed or impossible typed values reject | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.06` | Publish gear provision | `VenuesSpacesRoute` | Room references authorized Shard-24 register view | Provision posture and available quantity/identity projection publish | Missing authority or composed-exposure risk suppresses detail | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.07` | Contribute photo/evidence | `VenuesSpacesRoute` | Actor has first-hand or operator capability | Evidence attaches to checklist slot and field with provenance | Safety/privacy removal allowed; mere unflattering accuracy becomes contest, not erasure | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.08` | Read accessibility | `VenuesSpacesRoute` | Public room exists | Separate audience and performer routes show caveats, source and temporary overrides | Unknown is announced; no binary “accessible” claim is inferred | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.09` | Suggest factual correction | `VenuesSpacesRoute` | Field class permits contribution | Immutable suggestion/contest enters review or configured safe auto-apply | Commercial terms and statutory declarations never community-overwrite | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.10` | Change place status/at-risk | `VenuesSpacesRoute` | Authorized operator or qualified evidence | Effective status/signal propagates to projections and calendars | Public at-risk claim requires moderation/corroboration; abuse routes Shard 06 | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.11` | Configure availability | `VenuesSpacesRoute` | Authorized room operator; version current | Native slots, buffers, holds, exceptions and external blocks form one room calendar | Cycles/overlaps reject with exact resource/member | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.12` | Sync external calendar | `VenuesSpacesRoute` | Provider grant valid; room mapping explicit | Busy-only mirror updates watermark and staleness state | Ambiguous or stale sync blocks uncertain slots and disables instant book | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.13` | Place/challenge hold | `VenuesSpacesRoute` | Slot and resource graph available | Ranked expiring hold claims all required resources atomically | Losing challenge names higher claim; expiry releases all resources | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.14` | Request room-hire enquiry | `VenuesSpacesRoute` | Posture permits use type; requester eligible | Routed enquiry carries identity, credits, requirements and lapse time | Broken assignee falls back to org; lapse is explicit, never decline | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.15` | Instant-book room hire | `VenuesSpacesRoute` | Full claim, valid rate card/payout, fresh calendar | Quote, policy, resources, payment authorization and reservation commit idempotently | Any leg fails all-or-release-all; payment compensation visible | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.16` | Reserve compound resources | `VenuesSpacesRoute` | Every room/person/asset dependency available | One compound identity owns member reservations and holds | Member failure releases all and identifies failed dependency | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.17` | Cancel, reduce or reschedule | `VenuesSpacesRoute` | Reservation/policy snapshot active | Versioned ladder applies to released delta; one allowed move is atomic | Stale request returns current terms/state; no partial mutation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.18` | Complete/no-show reservation | `VenuesSpacesRoute` | End plus configured provisional window reached | Completion inferred, then evidence harvest becomes eligible | Timely no-show voids provisional harvest; money dispute remains separate | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.19` | Join/receive waitlist offer | `VenuesSpacesRoute` | Eligible demand and expiring entry | Ranked, lead-time-aware sequential offer receives bounded acceptance window | Expiry advances queue; repeated-loss notification budget enforced | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.20` | Create recurring series | `VenuesSpacesRoute` | Every instance and review horizon valid | Series identity, exception instances and future review commit | Conflicting instance is named; user may trim/split, never partial-create silently | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.21` | Compare rider to room | `VenuesSpacesRoute` | Comparable rider/spec snapshots available | Per-field match/unknown/conflict and confidence projection returns | Stale/unsupported fields become unknown, not false conflict | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `29.22` | Hand off performance bill slot | `VenuesSpacesRoute` | Declared use type means venue pays/splits with act | Shard 30 receives immutable room/spec/availability snapshot references | Room-hire fields are rejected at seam; no dual lifecycle | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/venues-spaces` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fvenues-spaces` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `VenuesSpacesRoute` variant `appPage` | `Venues, studios and spaces | WeJammin` | `Work with venues, studios and spaces using current authority, record state, and provenance.` |
| `/app/venues-spaces/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `VenuesSpacesRoute` with the matching workbench detail variant | `Record | Venues, studios and spaces | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `VenuesSpacesRoute` variant `publicPage` | `Venues, studios and spaces | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `VenuesSpacesRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `VenuesSpacesRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
...
| `RoomSpecsAccessibilityConformanceWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `RoomCalendarsHoldsEnquiriesWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `RoomReservationsSeriesHandoffWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
| `RoomSpecsAccessibilityConformanceWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `RoomCalendarsHoldsEnquiriesWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
| `RoomReservationsSeriesHandoffWorkbench` | ≤35 KB hydrated entry | Detail/editor/media/export modules dynamic-import on selection; list over 100 virtualizes; no barrel import | Preserve intrinsic dimensions; thumbnails use bounded variants; originals never download for list rows | Long task off main thread or chunked; no task >50 ms during input |
...
|---|---|---|---|---|
| `29a-place-room-authority-status.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `29b-room-specs-accessibility-conformance.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
| `29d-room-reservations-series-handoff.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `29a-place-room-authority-status.md` | `29A-PLACE-ROOM-AUTHORITY-STATUS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `PlaceRoomAuthorityStatusWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `29b-room-specs-accessibility-conformance.md` | `29B-ROOM-SPECS-ACCESSIBILITY-CONFORMANCE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `RoomSpecsAccessibilityConformanceWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `29c-room-calendars-holds-enquiries.md` | `29C-ROOM-CALENDARS-HOLDS-ENQUIRIES-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `RoomCalendarsHoldsEnquiriesWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `29d-room-reservations-series-handoff.md` | `29D-ROOM-RESERVATIONS-SERIES-HANDOFF-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `RoomReservationsSeriesHandoffWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `29a-place-room-authority-status.md` | Named source schemas | `CreatePlace`, `ClaimPlaceScope`, `UpsertRoomVersion`, `DeclareRoomStatus`, `Place`, `Room`, `RoomRelationship`, `V29_01_RESOLVE_PLACE`, `V29_02_CLAIM_SCOPE`, `V29_03_UPSERT_ROOM`, `V29_04_TRANSITION_ROOM`, `V29_10_DECLARE_STATUS`, `ClaimPlaceScopeResult`, `UpsertRoomVersionResult`, `RoomLifecycleRequest`, `RoomLifecycleResult`, `DeclareRoomStatusResult`, `excludes`, `AuthoredRelationship`, `venue_private`, `owner_id`, `place`, `search_path`, `review_required`, `event_id` | `PlaceRoomAuthorityStatusWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `29b-room-specs-accessibility-conformance.md` | Named source schemas | `ReviseSpecField`, `EvaluateConformance`, `V29_05_REVISE_SPEC_FIELD`, `SpecFieldRevision`, `AccessibilityProfile`, `StatutoryDeclaration`, `V29_06_PUBLISH_GEAR_PROVISION`, `V29_07_ATTACH_MEDIA_EVIDENCE`, `MediaEvidence`, `V29_08_READ_ACCESSIBILITY`, `V29_09_CONTEST_FIELD`, `FieldContest`, `V29_21_EVALUATE_CONFORMANCE`, `ConformanceRun`, `roomId`, `route`, `includeNotStated`, `at`, `segments`, `ReviseSpecFieldResult`, `PublishGearProvisionRequest`, `PublishGearProvisionResult`, `AttachMediaEvidenceRequest`, `AttachMediaEvidenceResult`, `AccessibilityPath`, `ReadAccessibilityQuery`, `ReadAccessibilityResult`, `ContestFieldRequest`, `FieldContestResult`, `EvaluateConformanceResult`, `commercial`, `statutory`, `not_stated`, `owner_id`, `supersedes_revision_id`, `room_id`, `Room`, `search_path`, `event_id` | `RoomSpecsAccessibilityConformanceWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `29c-room-calendars-holds-enquiries.md` | Named source schemas | `Calendar`, `ResourceHold`, `WaitlistEntry`, `ConfigureCalendar`, `RecordExternalBusyDelta`, `PlaceResourceHold`, `V29_11_CONFIGURE_CALENDAR`, `V29_12_RECORD_EXTERNAL_BUSY_DELTA`, `V29_13_PLACE_OR_CHALLENGE_HOLD`, `V29_14_REQUEST_ENQUIRY`, `V29_19_WAITLIST_ACTION`, `roomId`, `ConfigureCalendarResult`, `manage_calendar`, `RecordExternalBusyDeltaResult`, `ResourceHoldResult`, `RequestRoomHireEnquiry`, `RoomHireEnquiryResult`, `WaitlistActionRequest`, `WaitlistActionResult`, `blocked`, `venue_private`, `owner_id`, `room`, `search_path`, `healthy`, `disconnected`, `queued_organization_fallback`, `event_id` | `RoomCalendarsHoldsEnquiriesWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `29d-room-reservations-series-handoff.md` | Named source schemas | `RateCardVersion`, `QuoteSnapshot`, `Reservation`, `CompoundReservation`, `RecurringSeries`, `QuoteRoomHire`, `CommitRoomReservation`, `MutateReservation`, `V29_15_INSTANT_BOOK`, `V29_16_COMMIT_COMPOUND`, `V29_17_MUTATE_RESERVATION`, `V29_18_FINALIZE_ATTENDANCE`, `V29_20_CREATE_SERIES`, `V29_22_HANDOFF_BILL_SLOT`, `InstantBookRoomHire`, `InstantBookResult`, `CommitCompoundReservation`, `CompoundReservationResult`, `MutateReservationResult`, `RecordAttendanceOutcome`, `AttendanceOutcomeResult`, `CreateRecurringSeries`, `RecurringSeriesResult`, `PerformanceBillSlotHandoff`, `PerformanceSlotHandoffResult`, `handoff_performance_slot`, `bigint`, `venue_private`, `manage_rates`, `search_path`, `anon`, `authenticated`, `completed_provisional`, `no_show`, `voided`, `pending_authorization`, `in_progress`, `event_id` | `RoomReservationsSeriesHandoffWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type PlaceRoomAuthorityStatusContractField =
  | 'AuthoredRelationship'
  | 'ClaimPlaceScope'
  | 'ClaimPlaceScopeResult'
  | 'CreatePlace'
  | 'DeclareRoomStatus'
  | 'DeclareRoomStatusResult'
  | 'Place'
  | 'Room'
  | 'RoomLifecycleRequest'
  | 'RoomLifecycleResult'
  | 'RoomRelationship'
  | 'UpsertRoomVersion'
  | 'UpsertRoomVersionResult'
  | 'V29_01_RESOLVE_PLACE'
  | 'V29_02_CLAIM_SCOPE'
  | 'V29_03_UPSERT_ROOM'
  | 'V29_04_TRANSITION_ROOM'
  | 'V29_10_DECLARE_STATUS'
  | 'event_id'
  | 'excludes'
  | 'owner_id'
  | 'place'
  ...
  | 'search_path'
  | 'venue_private';
interface PlaceRoomAuthorityStatusWorkbenchContractFields {
  source: '29a-place-room-authority-status.md';
  fields: Readonly<Record<PlaceRoomAuthorityStatusContractField, unknown>>;
}
```

```ts
type RoomSpecsAccessibilityConformanceContractField =
  | 'AccessibilityPath'
  | 'AccessibilityProfile'
  | 'ApiError'
  | 'AttachMediaEvidenceRequest'
  | 'AttachMediaEvidenceResult'
  | 'ConformanceRun'
  | 'ContestFieldRequest'
  | 'EvaluateConformance'
  | 'EvaluateConformanceResult'
  | 'FieldContest'
  | 'FieldContestResult'
  | 'MediaEvidence'
  | 'PublishGearProvisionRequest'
  | 'PublishGearProvisionResult'
  | 'ReadAccessibilityQuery'
  | 'ReadAccessibilityResult'
  | 'ReviseSpecField'
  | 'ReviseSpecFieldResult'
  | 'Room'
  | 'SpecFieldRevision'
  | 'StatutoryDeclaration'
  | 'V29_05_REVISE_SPEC_FIELD'
  | 'V29_06_PUBLISH_GEAR_PROVISION'
  | 'V29_07_ATTACH_MEDIA_EVIDENCE'
  | 'V29_08_READ_ACCESSIBILITY'
  | 'V29_09_CONTEST_FIELD'
  | 'V29_21_EVALUATE_CONFORMANCE'
  | 'commercial'
  | 'event_id'
  | 'includeNotStated'
  | 'not_stated'
...
interface RoomSpecsAccessibilityConformanceWorkbenchContractFields {
  source: '29b-room-specs-accessibility-conformance.md';
  fields: Readonly<Record<RoomSpecsAccessibilityConformanceContractField, unknown>>;
}
```

```ts
type RoomCalendarsHoldsEnquiriesContractField =
  | 'ApiError'
  | 'Calendar'
  | 'ConfigureCalendar'
  | 'ConfigureCalendarResult'
  | 'PlaceResourceHold'
  | 'RecordExternalBusyDelta'
  | 'RecordExternalBusyDeltaResult'
  | 'RequestRoomHireEnquiry'
  | 'ResourceHold'
  | 'ResourceHoldResult'
  | 'RoomHireEnquiryResult'
  | 'V29_11_CONFIGURE_CALENDAR'
  | 'V29_12_RECORD_EXTERNAL_BUSY_DELTA'
  | 'V29_13_PLACE_OR_CHALLENGE_HOLD'
  | 'V29_14_REQUEST_ENQUIRY'
  | 'V29_19_WAITLIST_ACTION'
  ...
  | 'manage_calendar'
  | 'owner_id'
  | 'queued_organization_fallback'
  | 'room'
  | 'roomId'
  | 'search_path'
  | 'venue_private';
interface RoomCalendarsHoldsEnquiriesWorkbenchContractFields {
  source: '29c-room-calendars-holds-enquiries.md';
  fields: Readonly<Record<RoomCalendarsHoldsEnquiriesContractField, unknown>>;
}
```

```ts
type RoomReservationsSeriesHandoffContractField =
  | 'ApiError'
  | 'AttendanceOutcomeResult'
  | 'CommitCompoundReservation'
  | 'CommitRoomReservation'
  | 'CompoundReservation'
  | 'CompoundReservationResult'
  | 'CreateRecurringSeries'
  | 'InstantBookResult'
  | 'InstantBookRoomHire'
  | 'MutateReservation'
  | 'MutateReservationResult'
  | 'PerformanceBillSlotHandoff'
  | 'PerformanceSlotHandoffResult'
  | 'QuoteRoomHire'
  | 'QuoteSnapshot'
  | 'RateCardVersion'
  ...
  | 'Reservation'
  | 'V29_15_INSTANT_BOOK'
  | 'V29_16_COMMIT_COMPOUND'
  | 'V29_17_MUTATE_RESERVATION'
  | 'V29_18_FINALIZE_ATTENDANCE'
  | 'V29_20_CREATE_SERIES'
  ...
  | 'venue_private'
  | 'voided';
interface RoomReservationsSeriesHandoffWorkbenchContractFields {
  source: '29d-room-reservations-series-handoff.md';
  fields: Readonly<Record<RoomReservationsSeriesHandoffContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `29a-place-room-authority-status.md` | `PlaceRoomAuthorityStatusWorkbenchContractFields.fields` and `PlaceRoomAuthorityStatusWorkbenchProps.contractFields` | `AUTHORITY_REQUIRED`, `CLAIM_CONFLICT`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `FUTURE_BOOKING_CONFLICT`, `IDEMPOTENCY_CONFLICT`, `INVALID_REQUEST`, `LIVE_RESERVATION_CONFLICT`, `LOCATION_INVALID`, `NOT_FOUND`, `PROOF_FAILED`, `RATE_LIMITED`, `REVIEW_REQUIRED`, `SOURCE_FORBIDDEN`, `STALE_VERSION`, `TRANSITION_INVALID`, `TYPE_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `29b-room-specs-accessibility-conformance.md` | `RoomSpecsAccessibilityConformanceWorkbenchContractFields.fields` and `RoomSpecsAccessibilityConformanceWorkbenchProps.contractFields` | `ASSET_AUTHORITY_REQUIRED`, `CONTEST_FORBIDDEN`, `IDEMPOTENCY_CONFLICT`, `SOURCE_FORBIDDEN`, `STALE_VERSION`, `VALUE_INVALID` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `29c-room-calendars-holds-enquiries.md` | `RoomCalendarsHoldsEnquiriesWorkbenchContractFields.fields` and `RoomCalendarsHoldsEnquiriesWorkbenchProps.contractFields` | `ASSIGNEE_UNAVAILABLE`, `CONNECTOR_REVOKED`, `HOLD_EXPIRED`, `IDEMPOTENCY_CONFLICT`, `OFFER_EXPIRED`, `OFFER_TOKEN_INVALID`, `POSTURE_BLOCKED`, `RESOURCE_UNAVAILABLE`, `STALE_VERSION`, `UNAUTHENTICATED`, `WINDOW_INVALID` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `29d-room-reservations-series-handoff.md` | `RoomReservationsSeriesHandoffWorkbenchContractFields.fields` and `RoomReservationsSeriesHandoffWorkbenchProps.contractFields` | `COMPOUND_MEMBER_FAILED`, `HOLD_EXPIRED`, `IDEMPOTENCY_CONFLICT`, `POLICY_BLOCKED`, `QUOTE_EXPIRED`, `QUOTE_POLICY_UNAVAILABLE`, `RATE_UNAVAILABLE`, `RESOURCE_UNAVAILABLE`, `REVIEW_HORIZON_INVALID`, `ROOM_HIRE_FIELDS_FORBIDDEN`, `SERIES_CONFLICT`, `STALE_VERSION` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
