# Gear identity, provenance and recovery: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [23a-gear-identity-claims-transfers.md](../be/23a-gear-identity-claims-transfers.md), [23b-theft-screening-recovery.md](../be/23b-theft-screening-recovery.md), [23c-service-component-history.md](../be/23c-service-component-history.md), [23d-valuation-insurance-discography.md](../be/23d-valuation-insurance-discography.md)
> **IA Source**: [23-gear-provenance-registry.md](../ia/23-gear-provenance-registry.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [23-gear-provenance-registry.md](../ia/23-gear-provenance-registry.md) in full.
- **BE sources**: [23a-gear-identity-claims-transfers.md](../be/23a-gear-identity-claims-transfers.md), [23b-theft-screening-recovery.md](../be/23b-theft-screening-recovery.md), [23c-service-component-history.md](../be/23c-service-component-history.md), [23d-valuation-insurance-discography.md](../be/23d-valuation-insurance-discography.md).
...
|---|---|---|
| Classification and scope | `23-gear-provenance-registry.md`; BE index | Shard boundary and completed BE split group |
...
### `GearProvenanceRegistryRoute` (Astro server route)

```ts
interface GearProvenanceRegistryRouteProps {
  children?: never;
...
### `GearIdentityClaimsTransfersWorkbench` (bounded React island)

**BE owner**: `23a-gear-identity-claims-transfers.md`

```ts
interface GearIdentityClaimsTransfersWorkbenchProps {
  contractFields: GearIdentityClaimsTransfersWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly GearIdentityClaimsTransfersRecord[]>;
  actorId: string;
...
interface GearIdentityClaimsTransfersRecord {
  id: string;
  version: string;
  state: string;
...
### `TheftScreeningRecoveryWorkbench` (bounded React island)

**BE owner**: `23b-theft-screening-recovery.md`

```ts
interface TheftScreeningRecoveryWorkbenchProps {
  contractFields: TheftScreeningRecoveryWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly TheftScreeningRecoveryRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface TheftScreeningRecoveryRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `ServiceComponentHistoryWorkbench` (bounded React island)

**BE owner**: `23c-service-component-history.md`

```ts
interface ServiceComponentHistoryWorkbenchProps {
  contractFields: ServiceComponentHistoryWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly ServiceComponentHistoryRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface ServiceComponentHistoryRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `ValuationInsuranceDiscographyWorkbench` (bounded React island)

**BE owner**: `23d-valuation-insurance-discography.md`

```ts
interface ValuationInsuranceDiscographyWorkbenchProps {
  contractFields: ValuationInsuranceDiscographyWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly ValuationInsuranceDiscographyRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface ValuationInsuranceDiscographyRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/gear-provenance-registry` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/gear-provenance-registry/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|---|
| `GPR-01` User mints gear record | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-02` User corrects/adds identifier | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-03` Person claims ownership | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-04` Buyer/seller transfers ownership | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-05` User views provenance | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-06` Parties resolve duplicate records | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-07` Owner reports theft | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-08` Buyer/marketplace screens serial | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-09` Person reports sighting | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-10` Claimant contests false flag | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-11` Service provider records work | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-12` Owner records manual modification | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-13` Owner requests valuation | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-14` Appraiser issues appraisal | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-15` Owner builds insurance claim pack | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `GPR-16` Producer attests gear use | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `23-gear-provenance-registry.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `23-gear-provenance-registry.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `23-gear-provenance-registry.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `23-gear-provenance-registry.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `23-gear-provenance-registry.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `23-gear-provenance-registry.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `23-gear-provenance-registry.md` Accessibility |
...
|---|---|---|---|---|
| `GearProvenanceRegistryRoute` | `GearProvenanceRegistryRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `23-gear-provenance-registry.md` user flows/accessibility; design-system page archetypes |
| `GearIdentityClaimsTransfersWorkbench` | `GearIdentityClaimsTransfersWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `23a-gear-identity-claims-transfers.md` request/response fields; `23-gear-provenance-registry.md` interactions/access rules |
| `TheftScreeningRecoveryWorkbench` | `TheftScreeningRecoveryWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `23b-theft-screening-recovery.md` request/response fields; `23-gear-provenance-registry.md` interactions/access rules |
| `ServiceComponentHistoryWorkbench` | `ServiceComponentHistoryWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `23c-service-component-history.md` request/response fields; `23-gear-provenance-registry.md` interactions/access rules |
| `ValuationInsuranceDiscographyWorkbench` | `ValuationInsuranceDiscographyWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `23d-valuation-insurance-discography.md` request/response fields; `23-gear-provenance-registry.md` interactions/access rules |
...
|---|---|---|---|
| `GPR-01` User mints gear record | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-01` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-02` User corrects/adds identifier | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-02` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-03` Person claims ownership | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-03` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-04` Buyer/seller transfers ownership | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-04` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-05` User views provenance | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-05` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-06` Parties resolve duplicate records | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-06` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-07` Owner reports theft | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-07` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-08` Buyer/marketplace screens serial | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-08` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-09` Person reports sighting | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-09` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-10` Claimant contests false flag | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-10` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-11` Service provider records work | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-11` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-12` Owner records manual modification | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-12` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-13` Owner requests valuation | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-13` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-14` Appraiser issues appraisal | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-14` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-15` Owner builds insurance claim pack | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-15` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `GPR-16` Producer attests gear use | `GearProvenanceRegistryRoute` orchestrates; `GearIdentityClaimsTransfersWorkbench`, `TheftScreeningRecoveryWorkbench`, `ServiceComponentHistoryWorkbench`, `ValuationInsuranceDiscographyWorkbench` renders the relevant BE response and command state | `23-gear-provenance-registry.md` Interactions row `GPR-16` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
...
|---|---|---|---|---|
| `/app/gear-provenance-registry` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fgear-provenance-registry` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `GearProvenanceRegistryRoute` variant `appPage` | `Gear identity, provenance and recovery | WeJammin` | `Work with gear identity, provenance and recovery using current authority, record state, and provenance.` |
| `/app/gear-provenance-registry/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `GearProvenanceRegistryRoute` with the matching workbench detail variant | `Record | Gear identity, provenance and recovery | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `GearProvenanceRegistryRoute` variant `publicPage` | `Gear identity, provenance and recovery | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `GearProvenanceRegistryRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `GearProvenanceRegistryRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `GearIdentityClaimsTransfersWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `TheftScreeningRecoveryWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `23a-gear-identity-claims-transfers.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `23b-theft-screening-recovery.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `23c-service-component-history.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `23d-valuation-insurance-discography.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `23a-gear-identity-claims-transfers.md` | `23A-GEAR-IDENTITY-CLAIMS-TRANSFERS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `GearIdentityClaimsTransfersWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `23b-theft-screening-recovery.md` | `23B-THEFT-SCREENING-RECOVERY-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `TheftScreeningRecoveryWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `23c-service-component-history.md` | `23C-SERVICE-COMPONENT-HISTORY-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `ServiceComponentHistoryWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `23d-valuation-insurance-discography.md` | `23D-VALUATION-INSURANCE-DISCOGRAPHY-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `ValuationInsuranceDiscographyWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `23a-gear-identity-claims-transfers.md` | Named source schemas | All named response fields | `GearIdentityClaimsTransfersWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `23b-theft-screening-recovery.md` | Named source schemas | All named response fields | `TheftScreeningRecoveryWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `23c-service-component-history.md` | Named source schemas | All named response fields | `ServiceComponentHistoryWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `23d-valuation-insurance-discography.md` | Named source schemas | All named response fields | `ValuationInsuranceDiscographyWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type GearIdentityClaimsTransfersContractField =
  never;
interface GearIdentityClaimsTransfersWorkbenchContractFields {
  source: '23a-gear-identity-claims-transfers.md';
  fields: Readonly<Record<GearIdentityClaimsTransfersContractField, unknown>>;
}
```

```ts
type TheftScreeningRecoveryContractField =
  never;
interface TheftScreeningRecoveryWorkbenchContractFields {
  source: '23b-theft-screening-recovery.md';
  fields: Readonly<Record<TheftScreeningRecoveryContractField, unknown>>;
}
```

```ts
type ServiceComponentHistoryContractField =
  never;
interface ServiceComponentHistoryWorkbenchContractFields {
  source: '23c-service-component-history.md';
  fields: Readonly<Record<ServiceComponentHistoryContractField, unknown>>;
}
```

```ts
type ValuationInsuranceDiscographyContractField =
  never;
interface ValuationInsuranceDiscographyWorkbenchContractFields {
  source: '23d-valuation-insurance-discography.md';
  fields: Readonly<Record<ValuationInsuranceDiscographyContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `23a-gear-identity-claims-transfers.md` | `GearIdentityClaimsTransfersWorkbenchContractFields.fields` and `GearIdentityClaimsTransfersWorkbenchProps.contractFields` | `CLAIM_EVIDENCE_REQUIRED`, `CONFLICT`, `DEPENDENCY_UNAVAILABLE`, `ENTITY_CONTEXT_REQUIRED`, `FORBIDDEN`, `IDENTITY_KEY_CONFLICT`, `INVALID_ARGUMENT`, `MERGE_CONSENT_REQUIRED`, `NOT_FOUND`, `PROJECTION_UNAVAILABLE`, `RATE_LIMITED`, `SCREENING_UNAVAILABLE`, `TRANSFER_SCREEN_REQUIRED`, `UNAUTHENTICATED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `23b-theft-screening-recovery.md` | `TheftScreeningRecoveryWorkbenchContractFields.fields` and `TheftScreeningRecoveryWorkbenchProps.contractFields` | `CLAIM_CASE_REQUIRED`, `CONFLICT`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `INVALID_ARGUMENT`, `LOSS_FACTS_REQUIRED`, `MODERATION_REQUIRED`, `NOT_FOUND`, `RATE_LIMITED`, `SCREENING_UNAVAILABLE`, `SIGHTING_SCOPE_INVALID`, `THEFT_SCOPE_REQUIRED`, `UNAUTHENTICATED` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `23c-service-component-history.md` | `ServiceComponentHistoryWorkbenchContractFields.fields` and `ServiceComponentHistoryWorkbenchProps.contractFields` | `COMPONENT_PROVENANCE_REQUIRED`, `CONFLICT`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `IDENTITY_KEY_CONFLICT`, `INVALID_REQUEST`, `INVALID_TRANSITION`, `NOT_FOUND`, `OWNER_APPROVAL_REQUIRED`, `RATE_LIMITED`, `STEP_UP_REQUIRED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_MISMATCH` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `23d-valuation-insurance-discography.md` | `ValuationInsuranceDiscographyWorkbenchContractFields.fields` and `ValuationInsuranceDiscographyWorkbenchProps.contractFields` | `CONFLICT`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `INVALID_TRANSITION`, `NOT_FOUND`, `RATE_LIMITED`, `STEP_UP_REQUIRED`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VERSION_MISMATCH` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
