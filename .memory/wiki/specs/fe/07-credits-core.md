...
> **BE Source**: [07a-credit-assertions-visibility.md](../be/07a-credit-assertions-visibility.md), [07b-session-capture-offline.md](../be/07b-session-capture-offline.md), [07c-claims-attestations-confidence-taxonomy.md](../be/07c-claims-attestations-confidence-taxonomy.md)
> **IA Source**: [07-credits-core.md](../ia/07-credits-core.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [07-credits-core.md](../ia/07-credits-core.md) in full.
- **BE sources**: [07a-credit-assertions-visibility.md](../be/07a-credit-assertions-visibility.md), [07b-session-capture-offline.md](../be/07b-session-capture-offline.md), [07c-claims-attestations-confidence-taxonomy.md](../be/07c-claims-attestations-confidence-taxonomy.md).
...
|---|---|---|
| Classification and scope | `07-credits-core.md`; BE index | Shard boundary and completed BE split group |
...
### `CreditsCoreRoute` (Astro server route)

```ts
interface CreditsCoreRouteProps {
  children?: never;
...
### `CreditAssertionsVisibilityWorkbench` (bounded React island)

**BE owner**: `07a-credit-assertions-visibility.md`

```ts
interface CreditAssertionsVisibilityWorkbenchProps {
  contractFields: CreditAssertionsVisibilityWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly CreditAssertionsVisibilityRecord[]>;
  actorId: string;
...
interface CreditAssertionsVisibilityRecord {
  id: string;
  version: string;
  state: string;
...
### `SessionCaptureOfflineWorkbench` (bounded React island)

**BE owner**: `07b-session-capture-offline.md`

```ts
interface SessionCaptureOfflineWorkbenchProps {
  contractFields: SessionCaptureOfflineWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly SessionCaptureOfflineRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface SessionCaptureOfflineRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
### `ClaimsAttestationsConfidenceTaxonomyWorkbench` (bounded React island)

**BE owner**: `07c-claims-attestations-confidence-taxonomy.md`

```ts
interface ClaimsAttestationsConfidenceTaxonomyWorkbenchProps {
  contractFields: ClaimsAttestationsConfidenceTaxonomyWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly ClaimsAttestationsConfidenceTaxonomyRecord[]>;
  actorId: string;
  actingPartyId: string;
  access: AccessVariant;
  query: Readonly<Record<string, string>>;
...
interface ClaimsAttestationsConfidenceTaxonomyRecord {
  id: string;
  version: string;
  state: string;
  provenance: ReadonlyArray<{ source: string; evidence: string; at: string; visibility: string }>;
...
|---|---|---|---|
| `/app/credits-core` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/credits-core/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|---|
| `CRD-01` Assert credit | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-02` View work ledger | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-03` View public discography | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-04` Traverse graph | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-05` Merge duplicate party shell | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-06` Set/lift embargo | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-07` Propose correction | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-08` Run session roll | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-09` Log contribution | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-10` Close session | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-11` Capture attendance evidence | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-12` Import external credit | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-13` Claim unclaimed credit | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-14` Request attestation | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-15` Answer/retract attestation | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-16` Derive provenance | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-17` Contest credit | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-18` Resolve pending taxonomy alias | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
| `CRD-19` Curate discography page | Native link/button/form; focus stays until navigation or named result heading | Server-derived actor/context/capability, valid Zod input, required ETag/idempotency | Render authoritative response/version/provenance/next action; announce status | Map exact `ApiError`; retain input; focus summary/field; reconcile unknown mutation before retry | URL for navigation/filter; scoped draft before commit; server after success |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `07-credits-core.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `07-credits-core.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `07-credits-core.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `07-credits-core.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `07-credits-core.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `07-credits-core.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `07-credits-core.md` Accessibility |
...
|---|---|---|---|---|
| `CreditsCoreRoute` | `CreditsCoreRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `07-credits-core.md` user flows/accessibility; design-system page archetypes |
| `CreditAssertionsVisibilityWorkbench` | `CreditAssertionsVisibilityWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `07a-credit-assertions-visibility.md` request/response fields; `07-credits-core.md` interactions/access rules |
| `SessionCaptureOfflineWorkbench` | `SessionCaptureOfflineWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `07b-session-capture-offline.md` request/response fields; `07-credits-core.md` interactions/access rules |
| `ClaimsAttestationsConfidenceTaxonomyWorkbench` | `ClaimsAttestationsConfidenceTaxonomyWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `07c-claims-attestations-confidence-taxonomy.md` request/response fields; `07-credits-core.md` interactions/access rules |
...
|---|---|---|---|
| `CRD-01` Assert credit | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-01` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-02` View work ledger | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-02` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-03` View public discography | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-03` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-04` Traverse graph | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-04` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-05` Merge duplicate party shell | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-05` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-06` Set/lift embargo | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-06` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-07` Propose correction | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-07` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-08` Run session roll | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-08` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-09` Log contribution | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-09` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-10` Close session | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-10` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-11` Capture attendance evidence | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-11` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-12` Import external credit | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-12` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-13` Claim unclaimed credit | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-13` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-14` Request attestation | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-14` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-15` Answer/retract attestation | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-15` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-16` Derive provenance | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-16` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-17` Contest credit | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-17` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-18` Resolve pending taxonomy alias | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-18` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
| `CRD-19` Curate discography page | `CreditsCoreRoute` orchestrates; `CreditAssertionsVisibilityWorkbench`, `SessionCaptureOfflineWorkbench`, `ClaimsAttestationsConfidenceTaxonomyWorkbench` renders the relevant BE response and command state | `07-credits-core.md` Interactions row `CRD-19` | Pressed/pending state in the same animation frame; inline progress after 250 ms; result or typed error text plus polite announcement within 100 ms of parsed response; 150–220 ms visual transition |
...
|---|---|---|---|---|
| `/app/credits-core` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fcredits-core` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `CreditsCoreRoute` variant `appPage` | `Credit graph, capture and confidence | WeJammin` | `Work with credit graph, capture and confidence using current authority, record state, and provenance.` |
| `/app/credits-core/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `CreditsCoreRoute` with the matching workbench detail variant | `Record | Credit graph, capture and confidence | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `CreditsCoreRoute` variant `publicPage` | `Credit graph, capture and confidence | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `CreditsCoreRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `CreditsCoreRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `CreditAssertionsVisibilityWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `SessionCaptureOfflineWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
| `ClaimsAttestationsConfidenceTaxonomyWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `07a-credit-assertions-visibility.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `07b-session-capture-offline.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
| `07c-claims-attestations-confidence-taxonomy.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `07a-credit-assertions-visibility.md` | `07A-CREDIT-ASSERTIONS-VISIBILITY-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `CreditAssertionsVisibilityWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `07b-session-capture-offline.md` | `07B-SESSION-CAPTURE-OFFLINE-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `SessionCaptureOfflineWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |
| `07c-claims-attestations-confidence-taxonomy.md` | `07C-CLAIMS-ATTESTATIONS-CONFIDENCE-TAXONOMY-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `ClaimsAttestationsConfidenceTaxonomyWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `07a-credit-assertions-visibility.md` | Named source schemas | `includeQualified`, `roleFamily`, `startKind`, `tier`, `maxHops`, `densityFloor`, `edgeKinds` | `CreditAssertionsVisibilityWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `07b-session-capture-offline.md` | Named source schemas | All named response fields | `SessionCaptureOfflineWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
| `07c-claims-attestations-confidence-taxonomy.md` | Named source schemas | All named response fields | `ClaimsAttestationsConfidenceTaxonomyWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type CreditAssertionsVisibilityContractField =
  | 'densityFloor'
  | 'edgeKinds'
  | 'includeQualified'
  | 'maxHops'
  ...
  | 'startKind'
  | 'tier';
interface CreditAssertionsVisibilityWorkbenchContractFields {
  source: '07a-credit-assertions-visibility.md';
  fields: Readonly<Record<CreditAssertionsVisibilityContractField, unknown>>;
}
```

```ts
type SessionCaptureOfflineContractField =
  never;
interface SessionCaptureOfflineWorkbenchContractFields {
  source: '07b-session-capture-offline.md';
  fields: Readonly<Record<SessionCaptureOfflineContractField, unknown>>;
}
```

```ts
type ClaimsAttestationsConfidenceTaxonomyContractField =
  never;
interface ClaimsAttestationsConfidenceTaxonomyWorkbenchContractFields {
  source: '07c-claims-attestations-confidence-taxonomy.md';
  fields: Readonly<Record<ClaimsAttestationsConfidenceTaxonomyContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `07a-credit-assertions-visibility.md` | `CreditAssertionsVisibilityWorkbenchContractFields.fields` and `CreditAssertionsVisibilityWorkbenchProps.contractFields` | `ACTING_CONTEXT_STALE`, `CANDIDATE_NOT_FOUND`, `CREDIT_NOT_FOUND`, `DISPUTE_REQUIRED`, `EMBARGOED_NOT_FOUND`, `FORBIDDEN`, `HUMAN_CONFIRMATION_REQUIRED`, `IDEMPOTENCY_MISMATCH`, `INVALID_REQUEST`, `LIFT_GROUND_REQUIRED`, `PARTY_NOT_FOUND`, `PARTY_PAGE_NOT_FOUND`, `PROJECTION_UNAVAILABLE`, `RATE_LIMITED`, `ROLE_OR_SCOPE_INVALID`, `START_NOT_FOUND`, `UNAUTHENTICATED`, `VERSION_CONFLICT`, `WORK_NOT_FOUND`, `WORK_OR_PARTY_NOT_FOUND` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `07b-session-capture-offline.md` | `SessionCaptureOfflineWorkbenchContractFields.fields` and `SessionCaptureOfflineWorkbenchProps.contractFields` | `CONSENT_REQUIRED`, `CONSENT_REVOKED`, `FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `INVALID_PART`, `INVALID_REQUEST`, `PROVIDER_UNAVAILABLE`, `SESSION_NOT_FOUND`, `SIGNAL_INVALID`, `UNAUTHENTICATED`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
| `07c-claims-attestations-confidence-taxonomy.md` | `ClaimsAttestationsConfidenceTaxonomyWorkbenchContractFields.fields` and `ClaimsAttestationsConfidenceTaxonomyWorkbenchProps.contractFields` | `ALIAS_NOT_FOUND`, `ANSWER_INVALID`, `CLAIM_CONFLICT`, `CONTEST_INVALID`, `CREDIT_NOT_FOUND`, `DERIVATION_UNAVAILABLE`, `DISPUTE_REQUIRED`, `EVIDENCE_SET_INVALID`, `FORBIDDEN`, `FUZZY_MATCH_FORBIDDEN`, `IDEMPOTENCY_MISMATCH`, `IDENTITY_EVIDENCE_INVALID`, `INVALID_REQUEST`, `LICENSE_REQUIRED`, `PROVIDER_UNAVAILABLE`, `RATE_LIMITED`, `REQUEST_NOT_FOUND`, `RETRACTION_REASON_REQUIRED`, `ROLE_MAPPING_INVALID`, `SOURCE_HASH_INVALID`, `SOURCE_NOT_FOUND`, `UNAUTHENTICATED`, `VAULT_CLASS_INVALID`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
