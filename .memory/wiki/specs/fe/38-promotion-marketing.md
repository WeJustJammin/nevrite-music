# Promotion and marketing: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [38-promotion-marketing.md](../be/38-promotion-marketing.md)
> **IA Source**: [38-promotion-marketing.md](../ia/38-promotion-marketing.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [38-promotion-marketing.md](../ia/38-promotion-marketing.md) in full.
- **BE sources**: [38-promotion-marketing.md](../be/38-promotion-marketing.md).
...
|---|---|---|
| Classification and scope | `38-promotion-marketing.md`; BE index | Shard boundary and completed BE split group |
...
### `PromotionMarketingRoute` (Astro server route)

```ts
interface PromotionMarketingRouteProps {
  children?: never;
...
### `PromotionMarketingWorkbench` (bounded React island)

**BE owner**: `38-promotion-marketing.md`

```ts
interface PromotionMarketingWorkbenchProps {
  contractFields: PromotionMarketingWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly PromotionMarketingRecord[]>;
  actorId: string;
...
interface PromotionMarketingRecord {
  id: string;
  version: string;
  state: string;
...
|---|---|---|---|
| `/app/promotion-marketing` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/promotion-marketing/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `38-promotion-marketing.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `38-promotion-marketing.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `38-promotion-marketing.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `38-promotion-marketing.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `38-promotion-marketing.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `38-promotion-marketing.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `38-promotion-marketing.md` Accessibility |
...
|---|---|---|---|---|
| `PromotionMarketingRoute` | `PromotionMarketingRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `38-promotion-marketing.md` user flows/accessibility; design-system page archetypes |
| `PromotionMarketingWorkbench` | `PromotionMarketingWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `38-promotion-marketing.md` request/response fields; `38-promotion-marketing.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `38.01` | Create campaign | `PromotionMarketingRoute` | Authorized entity and valid release/tour/event anchor | Typed campaign and rule-pack version create derived grid | Missing/stale rule pack blocks derived deadlines | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.02` | Inspect readiness | `PromotionMarketingRoute` | Campaign dependencies and source projections available | Each requirement shows state, verifier and provenance | Unknown source remains unknown; attestation never renders validated | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.03` | Plan content beats | `PromotionMarketingRoute` | Editable campaign revision | Offset/pinned beat saves with position and target channel | Concurrent stale edit returns conflict; no silent merge | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.04` | Preview date cascade | `PromotionMarketingRoute` | Proposed anchor date and campaign revision | Movable, pinned, stranded, elapsed and embargo-conflict items preview | Preview mutates nothing and identifies stale source data | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.05` | Commit anchor change | `PromotionMarketingRoute` | Source-owner authority, step-up and accepted preview version | Anchor and movable beats change atomically; escaped work becomes tasks | Revision drift forces new preview; no external auto-send | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.06` | Assemble DSP pitch | `PromotionMarketingRoute` | Eligible release/track and current provider rule pack | Verified credits/metadata prefill, blockers and external submission steps render | Missing provider facts block readiness; no submission claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.07` | Compose direct pitch | `PromotionMarketingRoute` | Authorized actor, target, ask and campaign object | Durable pitch revision and channel-specific brief save | Mass personalization, prohibited offer or missing disclosure blocks | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.08` | Reserve pitch budget | `PromotionMarketingRoute` | Confirmed target opt-out state and supported channel | Atomic target/sender budget lease issues before send/export | Unknown opt-out/cap race blocks while preserving draft | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.09` | Send or export pitch | `PromotionMarketingRoute` | Verified entity sender or export-only mode | Platform send records provider receipt, or export records user-controlled handoff | Shared sender unavailable by design; delivery uncertainty reconciles | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.10` | Record pitch outcome | `PromotionMarketingRoute` | Existing pitch and entity actor | One-click accepted/rejected/silence/bounce/coverage outcome appends | Bounce updates contact confidence, never rejection | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.11` | Manage target reference | `PromotionMarketingRoute` | Approved directory steward/source | Provenance, interests, confidence and staleness update | Unverified contribution quarantines; personal/private data rejected | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.12` | Export private CRM | `PromotionMarketingRoute` | Entity mandate and step-up | Structured contacts/history export asynchronously in open format | Directory-only facts and B5-gated notes excluded | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.13` | Build smart link | `PromotionMarketingRoute` | Published/upcoming release or first-party event | Canonical destinations and durable slug publish | Missing destinations show explicit unavailable state | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.14` | Follow smart-link destination | `PromotionMarketingRoute` | Public visitor selects destination | Remembered preference routes and first-party click records minimally | Dead destination blocks/returns to page; no audience capture | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.15` | Authorize pre-save | `PromotionMarketingRoute` | Fan chooses DSP and sees exact narrow scope | Revocable expiring grant binds one release/provider | Provider denial/expiry is explicit; no marketing consent created | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.16` | Execute pre-save | `PromotionMarketingRoute` | Release becomes provider-available and grant valid | Library-write attempt records provider outcome once | Rate limit/token failure queues within validity then reports failure | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.17` | Retire link | `PromotionMarketingRoute` | Authorized entity actor selects behavior | Keep, canonical redirect or retired page publishes atomically | Hard delete refused; ongoing traffic prompts review | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.18` | Build paid-promotion plan | `PromotionMarketingRoute` | Campaign goals, budget ceiling and target market | Advisory channel/budget/creative plan saves with assumptions | No provider spend, ad account access or fan-data export | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.19` | Create seeding brief | `PromotionMarketingRoute` | Campaign and creator engagement reference | Non-removable disclosure and explicit amplification terms bind brief | Prohibited placement promise or missing rights blocks activation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.20` | Classify promotion offer | `PromotionMarketingRoute` | Offer intent, value exchange, channel and current terms | Unpaid-allowed, paid-review-only or prohibited verdict explains itself | Unknown/evaluator outage fails closed; paid-review-only cannot activate brokerage | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.21` | Schedule social post | `PromotionMarketingRoute` | Connected supported provider and channel rendering | Post job binds immutable beat/revision and provider capability version | Disconnection or lost capability blocks before due time | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.22` | Reconcile social post | `PromotionMarketingRoute` | Provider receipt/read-back or actor attestation | Calendar shows published, failed, unknown or manual outcome | No success inferred from timeout; retry reuses idempotency key | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.23` | Record coverage | `PromotionMarketingRoute` | URL/artifact and entity actor | Claimed item appends; permitted verifier may strengthen it | Monitoring suggestion never auto-inserts; mutable artifact remains timestamped | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.24` | Plan tour/event announce | `PromotionMarketingRoute` | First-party shows, venue notice and staggered on-sale facts | Per-date readiness, embargo notices and tagged links generate | Readiness reveals but does not claim to bind venue contract | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.25` | Diagnose soft date | `PromotionMarketingRoute` | Authorized artist/venue and sufficiently fresh sales facts | Evidence, confidence and possible actions show to both parties | Sparse/stale data withholds diagnosis; never auto-spends | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.26` | Attribute ticket sale | `PromotionMarketingRoute` | First-party tagged click/session and settled ticket order | Observed last eligible first-party click conversion records, net refunds | Off-platform ticketing remains click-only; no multi-touch credit claim | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.27` | Publish/share EPK | `PromotionMarketingRoute` | Authorized actor, versioned assets/credits and share policy | Immutable EPK version and recipient-scoped revocable link publish | Missing photo/audio authority blocks affected asset; claimed credit stays labeled | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `38.28` | Manage embargo/exclusive | `PromotionMarketingRoute` | Authorized campaign actor, asset/scope, counterparty and lift time | Constraint validates, propagates least-detail notice and blocks conflict | Second overlapping exclusive refuses; wall-clock lift cannot roll back | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/promotion-marketing` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fpromotion-marketing` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `PromotionMarketingRoute` variant `appPage` | `Promotion and marketing | WeJammin` | `Work with promotion and marketing using current authority, record state, and provenance.` |
| `/app/promotion-marketing/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `PromotionMarketingRoute` with the matching workbench detail variant | `Record | Promotion and marketing | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `PromotionMarketingRoute` variant `publicPage` | `Promotion and marketing | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `PromotionMarketingRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `PromotionMarketingRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `PromotionMarketingWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `38-promotion-marketing.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `38-promotion-marketing.md` | `38-PROMOTION-MARKETING-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `PromotionMarketingWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `38-promotion-marketing.md` | Named source schemas | `items`, `blockers`, `warnings`, `signals`, `missingSources`, `array`, `nonempty`, `overlaps`, `jsonb_typeof`, `cardinality`, `requestId`, `traceId`, `operationId` | `PromotionMarketingWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type PromotionMarketingContractField =
  | 'array'
  | 'blockers'
  | 'cardinality'
  | 'items'
  ...
  | 'traceId'
  | 'warnings';
interface PromotionMarketingWorkbenchContractFields {
  source: '38-promotion-marketing.md';
  fields: Readonly<Record<PromotionMarketingContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `38-promotion-marketing.md` | `PromotionMarketingWorkbenchContractFields.fields` and `PromotionMarketingWorkbenchProps.contractFields` | `ANCHOR_INVALID`, `ARTIFACT_INVALID`, `ASSET_OR_CREDIT_AUTHORITY_REQUIRED`, `ATTEMPT_CONFLICT`, `ATTRIBUTION_INPUT_INVALID`, `AUTH_REQUIRED`, `BEAT_INVALID`, `BRIEF_INVALID`, `CAMPAIGN_NOT_FOUND`, `CAPABILITY_REQUIRED`, `CHANNEL_RULE_FAILED`, `CONNECTION_CAPABILITY_REQUIRED`, `COUNTERPART_AUTHORITY_REQUIRED`, `CRM_CAPABILITY_REQUIRED`, `DEPENDENCY_TIMEOUT`, `DESTINATION_POLICY_FAILED`, `DIRECT_PITCH_INVALID`, `DISCLOSURE_VERSION_CONFLICT`, `DISPATCH_CAPABILITY_REQUIRED`, `DSP_PITCH_INVALID`, `DSP_RULE_FAILED`, `EMBARGO_INVALID`, `ENGAGEMENT_AUTHORITY_REQUIRED`, `EPK_INVALID`, `EVENT_AUTHORITY_REQUIRED`, `EVENT_PLAN_INVALID`, `EXPORT_POLICY_INVALID`, `GRANT_REVOKED`, `IDEMPOTENCY_CONFLICT`, `LEASE_EXPIRED`, `LINK_INVALID`, `LINK_NOT_FOUND`, `MERGE_CONFLICT`, `OAUTH_SCOPE_INVALID`, `OFFER_BLOCKED`, `OFFER_INVALID`, `OUTCOME_INVALID`, `PLAN_INVALID`, `PLAN_NOT_FOUND`, `PREVIEW_EXPIRED`, `PROVIDER_SPEND_FORBIDDEN`, `PROVIDER_UNAVAILABLE`, `PURPOSE_REQUIRED`, `RATE_LIMITED`, `READINESS_BLOCKED`, `RECEIPT_DIGEST_CONFLICT`, `RELEASE_UNAVAILABLE`, `RENDERING_INVALID`, `RENDER_UNAVAILABLE`, `RESOLVER_UNAVAILABLE`, `REVIEW_REQUIRED`, `REVISION_MISMATCH`, `RULE_PACK_EXPIRED`, `SCHEDULE_CONFLICT`, `SERVICE_AUTH_REQUIRED`, `SOURCE_AUTHORITY_REQUIRED`, `SOURCE_EVENT_CONFLICT`, `SOURCE_REVISION_CONFLICT`, `SOURCE_UNAVAILABLE`, `STEP_UP_REQUIRED`, `STEWARD_REQUIRED`, `TARGET_FACT_INVALID`, `TARGET_NOT_FOUND`, `TARGET_RATE_LIMITED`, `TERMS_VERSION_STALE`, `VERSION_CONFLICT` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
