# Market intelligence, fraud and scouting signals: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [40-market-intelligence-signals.md](../be/40-market-intelligence-signals.md)
> **IA Source**: [40-market-intelligence-signals.md](../ia/40-market-intelligence-signals.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [40-market-intelligence-signals.md](../ia/40-market-intelligence-signals.md) in full.
- **BE sources**: [40-market-intelligence-signals.md](../be/40-market-intelligence-signals.md).
...
|---|---|---|
| Classification and scope | `40-market-intelligence-signals.md`; BE index | Shard boundary and completed BE split group |
...
### `MarketIntelligenceSignalsRoute` (Astro server route)

```ts
interface MarketIntelligenceSignalsRouteProps {
  children?: never;
...
### `MarketIntelligenceSignalsWorkbench` (bounded React island)

**BE owner**: `40-market-intelligence-signals.md`

```ts
interface MarketIntelligenceSignalsWorkbenchProps {
  contractFields: MarketIntelligenceSignalsWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly MarketIntelligenceSignalsRecord[]>;
  actorId: string;
...
interface MarketIntelligenceSignalsRecord {
  id: string;
  version: string;
  state: string;
...
|---|---|---|---|
| `/app/market-intelligence-signals` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/market-intelligence-signals/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `40-market-intelligence-signals.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `40-market-intelligence-signals.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `40-market-intelligence-signals.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `40-market-intelligence-signals.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `40-market-intelligence-signals.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `40-market-intelligence-signals.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `40-market-intelligence-signals.md` Accessibility |
...
|---|---|---|---|---|
| `MarketIntelligenceSignalsRoute` | `MarketIntelligenceSignalsRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `40-market-intelligence-signals.md` user flows/accessibility; design-system page archetypes |
| `MarketIntelligenceSignalsWorkbench` | `MarketIntelligenceSignalsWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `40-market-intelligence-signals.md` request/response fields; `40-market-intelligence-signals.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `40.01` | Record playlist transition | `MarketIntelligenceSignalsRoute` | Integrity-qualified provider observation | Added/moved/removed event appends with event-time reach snapshot | Duplicate/source restatement reconciles additively | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.02` | Render placement alert | `MarketIntelligenceSignalsRoute` | Current placement event and quality assessment | Neutral/positive/risk-first alert uses current policy | Unknown/risky quality suppresses celebration and states uncertainty | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.03` | Record chart event | `MarketIntelligenceSignalsRoute` | Named chart source and chart period | Source-specific position appends at period date | Conflicting source remains side-by-side, not merged | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.04` | Inspect curator evidence | `MarketIntelligenceSignalsRoute` | Public curator/institution and sufficient sample | Range, n, freshness and fraud context render | Below floor/B2 gate suppresses shared result | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.05` | View audience geography | `MarketIntelligenceSignalsRoute` | Authorized artist context and source integrity | Separate owned/rented layers plus engagement depth render | Sources never average; sparse cells coarsen/suppress by policy | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.06` | Request routing shortlist | `MarketIntelligenceSignalsRoute` | Authorized act/tour context and available history | Candidates with reasons, unknowns and confidence return | Insufficient facts returns no ranking, never invented route | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.07` | Evaluate show impact | `MarketIntelligenceSignalsRoute` | First-party booked show and usable before/after windows | Range, confidence, null result and confounders publish privately | Confounded/sparse case declines measurement | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.08` | Detect anomaly | `MarketIntelligenceSignalsRoute` | Integrity-qualified series crosses descriptive rule | Hedged observation and evidence case append early | Missing/claimed data lowers confidence or withholds, never accuses | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.09` | Export anomaly dossier | `MarketIntelligenceSignalsRoute` | Artist authority and evidence case | Human-readable source/provenance/timeline dossier exports | No automatic provider report or fraud verdict | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.10` | Evaluate vendor history | `MarketIntelligenceSignalsRoute` | Linked promotion campaigns and anomaly outcomes | Coincidence range, n and retractions show privately | Below floor/no linkage yields insufficient evidence | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.11` | Add private watch | `MarketIntelligenceSignalsRoute` | Scout purpose mandate and subject consent at read time | Reference watch entry saves without subject snapshot | No consent returns invariant no-result; follow remains separate | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.12` | Fire momentum signal | `MarketIntelligenceSignalsRoute` | Watch active, purpose consent current and absolute/relative floors met | Descriptive signal records and notifies within budget | Revoked consent is indistinguishable from quiet | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.13` | Search scouting discovery | `MarketIntelligenceSignalsRoute` | Purpose, mandate, query and current consent scope | Capped credit-native results return | Empty/denied/suppressed cases render identically | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `40.14` | Revoke scouting visibility | `MarketIntelligenceSignalsRoute` | Subject changes one purpose | Index, watches and pending signals lose access atomically | Tombstone tells scout access ended without reason/data | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/market-intelligence-signals` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fmarket-intelligence-signals` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `MarketIntelligenceSignalsRoute` variant `appPage` | `Market intelligence, fraud and scouting signals | WeJammin` | `Work with market intelligence, fraud and scouting signals using current authority, record state, and provenance.` |
| `/app/market-intelligence-signals/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `MarketIntelligenceSignalsRoute` with the matching workbench detail variant | `Record | Market intelligence, fraud and scouting signals | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `MarketIntelligenceSignalsRoute` variant `publicPage` | `Market intelligence, fraud and scouting signals | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `MarketIntelligenceSignalsRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `MarketIntelligenceSignalsRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `MarketIntelligenceSignalsWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `40-market-intelligence-signals.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `40-market-intelligence-signals.md` | `40-MARKET-INTELLIGENCE-SIGNALS-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `MarketIntelligenceSignalsWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `40-market-intelligence-signals.md` | Named source schemas | `MetricObservationV1`, `SeriesIntegrityV1`, `AnalyticsSeriesV1`, `PlaylistPlacementEventV1`, `PlacementQualityV1`, `ChartObservationV1`, `CuratorEvidenceV1`, `AudienceGeoLayerV1`, `RoutingCandidateV1`, `ShowImpactEstimateV1`, `AnomalyObservationV1`, `VendorCoincidenceV1`, `ScoutingConsentDecisionV1`, `WatchReferenceV1`, `MomentumObservationV1`, `expectedVersion`, `limit`, `operationId`, `RecordPlaylistTransitionRequest`, `RenderPlacementAlertRequest`, `PlacementAlertResponse`, `RecordChartObservationRequest`, `CuratorEvidenceRequest`, `AudienceGeographyRequest`, `AudienceGeographyResponse`, `RequestRoutingShortlistRequest`, `RoutingShortlistResponse`, `EvaluateShowImpactRequest`, `DetectAnomalyRequest`, `ExportAnomalyDossierRequest`, `DossierExportAccepted`, `VendorCoincidenceRequest`, `AddPrivateWatchRequest`, `WatchCreateResponse`, `FireMomentumSignalRequest`, `MomentumSignalReceipt`, `ScoutingDiscoveryRequest`, `ScoutingDiscoveryResponse`, `RevokeScoutingVisibilityRequest`, `sourceRefs`, `layers`, `sourceLayer`, `pageSize`, `subject_opaque_id`, `purpose`, `creditQuery`, `roleCodes`, `regionCode`, `consentPolicyVersion`, `cursor`, `offset`, `page`, `sort`, `B2_DISABLED`, `insufficient`, `no_result`, `IdempotencyRecord`, `AuditEvent`, `OutboxEvent`, `Job`, `ObjectRecord`, `playlist_placement_event`, `placement_quality_projection`, `chart_observation`, `curator_public_reference`, `audience_geo_observation`, `routing_candidate_projection`, `show_impact_analysis`, `anomaly_case`, `anomaly_observation`, `vendor_campaign_link`, `vendor_coincidence_projection`, `scouting_visibility_instruction`, `scout_watch`, `watch_tombstone`, `momentum_observation`, `discovery_query_audit`, `playlist_placement_events`, `placement_quality_projections`, `chart_observations` | `MarketIntelligenceSignalsWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type MarketIntelligenceSignalsContractField =
  | 'AddPrivateWatchRequest'
  | 'AnalyticsSeriesV1'
  | 'AnomalyObservationV1'
  | 'AudienceGeoLayerV1'
  | 'AudienceGeographyRequest'
  | 'AudienceGeographyResponse'
  | 'AuditEvent'
  | 'B2_DISABLED'
  | 'ChartObservationV1'
  | 'CuratorEvidenceRequest'
  | 'CuratorEvidenceV1'
  | 'DetectAnomalyRequest'
  | 'DossierExportAccepted'
  | 'EvaluateShowImpactRequest'
  | 'ExportAnomalyDossierRequest'
  | 'FireMomentumSignalRequest'
  | 'GetAnalyticsSeriesV1'
  | 'GetGeoLayersV1'
  | 'IdempotencyRecord'
  | 'Job'
  | 'MetricObservationV1'
  | 'MomentumObservationV1'
  | 'MomentumSignalReceipt'
  | 'ObjectRecord'
  | 'OutboxEvent'
  | 'PlacementAlertResponse'
  | 'PlacementQualityV1'
  | 'PlaylistPlacementEventV1'
  | 'RecordChartObservationRequest'
  | 'RecordPlaylistTransitionRequest'
  | 'RenderPlacementAlertRequest'
  | 'RequestRoutingShortlistRequest'
  | 'RevokeScoutingVisibilityRequest'
  | 'RoutingCandidateV1'
  | 'RoutingShortlistResponse'
  | 'ScoutingConsentDecisionV1'
  | 'ScoutingDiscoveryRequest'
  | 'ScoutingDiscoveryResponse'
  | 'SeriesIntegrityV1'
  | 'ShowImpactEstimateV1'
  | 'VendorCoincidenceRequest'
  | 'VendorCoincidenceV1'
  | 'WatchCreateResponse'
  | 'WatchReferenceV1'
  | 'allow'
  ...
  | 'anon'
  | 'audience_geo_observation'
  | 'audience_geo_observations'
  | 'authenticated'
  | 'chart_observation'
  | 'chart_observations'
  | 'consentPolicyVersion'
  | 'creditQuery'
  | 'curator_public_reference'
  | 'curator_public_references'
  | 'cursor'
  | 'declined'
  | 'discovery_query_audit'
  | 'discovery_query_audits'
  | 'eventType'
  | 'expectedVersion'
  | 'insufficient'
  | 'intelligence_api'
  | 'intelligence_consent_worker'
  | 'intelligence_detector'
  | 'intelligence_exporter'
  | 'intelligence_retention'
  | 'intelligence_source_worker'
  | 'layers'
  | 'limit'
  | 'measured'
  ...
  | 'page'
  | 'pageSize'
  | 'placement_quality_projection'
  | 'placement_quality_projections'
  | 'playlist_placement_event'
  | 'playlist_placement_events'
  | 'prior_watch_id'
  | 'privacyClass'
  | 'purpose'
  | 'recorded'
  ...
  | 'retracted'
  | 'roleCodes'
  | 'routing_candidate_projection'
  | 'routing_candidate_projections'
  | 'scout_watch'
  | 'scout_watches'
  | 'scouting_visibility_instruction'
  | 'scouting_visibility_instructions'
  | 'search_path'
  | 'show_impact_analyses'
  ...
  | 'subject_opaque_id'
  | 'suppressed'
  | 'vendor_campaign_link'
  | 'vendor_campaign_links'
  | 'vendor_coincidence_projection'
  | 'vendor_coincidence_projections'
  | 'watch_tombstone'
  | 'watch_tombstones'
  | 'withdraw';
interface MarketIntelligenceSignalsWorkbenchContractFields {
  source: '40-market-intelligence-signals.md';
  fields: Readonly<Record<MarketIntelligenceSignalsContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `40-market-intelligence-signals.md` | `MarketIntelligenceSignalsWorkbenchContractFields.fields` and `MarketIntelligenceSignalsWorkbenchProps.contractFields` | `CASE_NOT_FOUND`, `CURATOR_NOT_FOUND`, `DEPENDENCY_UNAVAILABLE`, `EXPORT_FORBIDDEN`, `FORBIDDEN`, `FORBIDDEN_FIELD`, `IDEMPOTENCY_CONFLICT`, `INVALID_REQUEST`, `MANDATE_REQUIRED`, `OWNER_FORBIDDEN`, `PERIOD_INVALID`, `PLACEMENT_NOT_FOUND`, `POLICY_VERSION_STALE`, `POSITION_INVALID`, `PRIVACY_POLICY_STALE`, `PURPOSE_INVALID`, `RATE_LIMITED`, `RULE_VERSION_STALE`, `SHOW_NOT_FOUND`, `SOURCE_REVISION_STALE`, `STEP_UP_REQUIRED`, `SUBJECT_FORBIDDEN`, `SUBJECT_NOT_FOUND`, `TOUR_CONTEXT_INVALID`, `TOUR_CONTEXT_NOT_FOUND`, `TRANSITION_INVALID`, `UNAUTHENTICATED`, `VALIDATION_FAILED`, `VENDOR_NOT_FOUND`, `VERSION_MISMATCH`, `WINDOW_INVALID` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
