# Career planning, insurance and sustainability: Frontend Specification

> **Classification**: Feature specification
> **BE Source**: [42-career-planning-risk.md](../be/42-career-planning-risk.md)
> **IA Source**: [42-career-planning-risk.md](../ia/42-career-planning-risk.md)
> **Surface**: Responsive Astro hybrid web/PWA with bounded React islands
> **Status**: Complete

## Referenced Material Inventory

- **Primary IA**: [42-career-planning-risk.md](../ia/42-career-planning-risk.md) in full.
- **BE sources**: [42-career-planning-risk.md](../be/42-career-planning-risk.md).
...
|---|---|---|
| Classification and scope | `42-career-planning-risk.md`; BE index | Shard boundary and completed BE split group |
...
### `CareerPlanningRiskRoute` (Astro server route)

```ts
interface CareerPlanningRiskRouteProps {
  children?: never;
...
### `CareerPlanningRiskWorkbench` (bounded React island)

**BE owner**: `42-career-planning-risk.md`

```ts
interface CareerPlanningRiskWorkbenchProps {
  contractFields: CareerPlanningRiskWorkbenchContractFields;
  children?: never;
  variant: DomainVariant;
  initial: AsyncState<readonly CareerPlanningRiskRecord[]>;
  actorId: string;
...
interface CareerPlanningRiskRecord {
  id: string;
  version: string;
  state: string;
...
|---|---|---|---|
| `/app/career-planning-risk` | Astro SSR or cache-safe prerender by route registry | Public exposes public projection; protected verifies session/acting context; admin requires explicit capability and named step-up | Query, cursor, selected record, tab are URL state; invalid values normalize with `replaceState`; Back restores selection/scroll |
| `/app/career-planning-risk/:recordId` | Server-first detail with bounded islands | Concealed returns disclosure-safe 404; visible forbidden uses `<CapabilityGate>`; expired session preserves safe return target | Bookmark resolves current canonical version; stale/deleted target shows exact state and safe parent |
...
|---|---|---|---|---|
| Route shell/navigation | 1.3.1, 2.4.1–2.4.3, 2.4.7/2.4.11 | Skip link; logical DOM; route focuses `h1` | Named landmarks, one main, `aria-current=page`, unique title | `42-career-planning-risk.md` Accessibility/User Flows |
| Workbench selection | 1.3.1, 2.1.1, 2.4.3, 4.1.2 | Native controls; Enter opens; Escape closes bounded inspector; focus returns | Named list/detail; selected state; state/provenance text | `42-career-planning-risk.md` Interactions/Access Control |
| Forms/validation | 1.3.1, 3.3.1–3.3.4, 4.1.3 | Persistent labels; linked summary focuses first invalid field; no trap | `aria-invalid`, `aria-describedby`, error links, polite status | `42-career-planning-risk.md` Acceptance Criteria/Edge Cases |
| Async/refetch/conflict | 2.2.1, 2.4.3, 4.1.3 | Refresh never steals focus; Retry native; conflict begins at heading | Polite atomic update; stale/pending/failed text; request ID | `42-career-planning-risk.md` Interactions/BE failures |
| Tables/filters | 1.3.1, 1.4.10, 2.1.1, 2.5.8 | Header buttons; Apply/Reset; 24 CSS px minimum, 44 preferred | Caption, headers, sort, count, active-filter summary | `42-career-planning-risk.md` User Flows/responsive |
| High-risk confirmation | 2.1.2, 2.4.3, 3.3.4, 4.1.2 | Inline first; dialog heading focus, Tab containment, Escape before commit, return focus | Consequence, scope, version, context, step-up, irreversible effect | `42-career-planning-risk.md` Access Control/Edge Cases |
| Motion/media | 1.2.x where applicable, 2.2.2, 2.3.3 | Media keyboard controls; pause/stop; no essential timed gesture | Captions/transcript/metadata; reduced motion; waveform never sole content | `42-career-planning-risk.md` Accessibility |
...
|---|---|---|---|---|
| `CareerPlanningRiskRoute` | `CareerPlanningRiskRouteProps` | `never` | `publicPage`, `appPage`, `adminPage`, `authPage`, `degradedPage` | `42-career-planning-risk.md` user flows/accessibility; design-system page archetypes |
| `CareerPlanningRiskWorkbench` | `CareerPlanningRiskWorkbenchProps` | `never` | `publicRead`, `entitledRead`, `ownerFull`, `guardianMandate`, `juniorRestricted`, `businessMandate`, `staffCaseScoped`, `adminStepUp`, `forbiddenHidden`, `disabledPrerequisite` | `42-career-planning-risk.md` request/response fields; `42-career-planning-risk.md` interactions/access rules |
...
|---|---|---|---|---|---|---|
| `42.01` | Add goal template | `CareerPlanningRiskRoute` | Authorized person/entity and supported derivations | Composable goals instantiate with targets, cadence and visibility | Missing source/query disables goal with explanation | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.02` | View goal progress | `CareerPlanningRiskRoute` | Current source projection and integrity | Progress/unknown state derives with source/freshness, segregated by trust tier with verified and declared reported separately and never blended | Stale/incomplete source never renders achieved | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.03` | Derive milestone | `CareerPlanningRiskRoute` | Qualifying canonical fact and rule version | One immutable milestone appends with safe visibility/wording | Duplicate derivation idempotent; revoked fact invalidates visibly | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.04` | Share milestone | `CareerPlanningRiskRoute` | Milestone class permits sharing and actor authorized | Public/entity-scoped projection uses bounded wording | Financial/hard-private class refuses public visibility | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.05` | View peer distribution | `CareerPlanningRiskRoute` | B2 gate open, role/context and cohort meet floor | Pull-only distribution, n, position and caveats render | Below floor/denied/no data suppresses without widening | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.06` | Detect insurance need | `CareerPlanningRiskRoute` | Named underlying action and structured requirement/policy facts | Non-blocking possible-gap prompt appears once per policy | Unknown facts state uncertainty; no coverage verdict | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.07` | Request insurance referral | `CareerPlanningRiskRoute` | User affirmatively selects provider/path and disclosure | Minimum consented package transmits with provider terms | Decline/suppress leaves underlying action unchanged | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.08` | Record provider response | `CareerPlanningRiskRoute` | Provider/actor supplies reference/outcome | Referral status records without underwriting claim | Ambiguous/failed response remains external/unknown | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
| `42.09` | Request wellbeing/sustainability score | `CareerPlanningRiskRoute` | Any actor/context | Stable capability-not-offered explanation and runway/workload source routes | No sensitive input is solicited or stored | Same-frame pressed or disabled state; loading feedback after 250 ms; parsed success or error announcement within 100 ms. |
...
|---|---|---|---|---|
| `/app/career-planning-risk` | Server validates Supabase token, expiry, acting context, and route capability. Missing/expired token redirects 303 to `/auth/sign-in?returnTo=%2Fapp%2Fcareer-planning-risk` after allowlist normalization. Valid but concealed target returns 404; visible forbidden target renders `CapabilityGate`. | `CareerPlanningRiskRoute` variant `appPage` | `Career planning, insurance and sustainability | WeJammin` | `Work with career planning, insurance and sustainability using current authority, record state, and provenance.` |
| `/app/career-planning-risk/:recordId` | Same token/expiry/context check; malformed ID returns 400, concealed/unreadable returns 404, expired session uses the same safe sign-in redirect. | `CareerPlanningRiskRoute` with the matching workbench detail variant | `Record | Career planning, insurance and sustainability | WeJammin` | `Review the current record, provenance, history, and permitted actions.` |
| Public projection when a BE route declares one | No session accepted as authority; public projection only. Unsafe or non-public record returns disclosure-safe 404, never app-shell redirect. | `CareerPlanningRiskRoute` variant `publicPage` | `Career planning, insurance and sustainability | WeJammin` | `View the public, provenance-labelled record.` |
| System/degraded boundary | Preserves verified shell only; Retry stays on canonical URL; unsafe cached data is removed. | `CareerPlanningRiskRoute` variant `degradedPage` | `Service status | WeJammin` | `Review affected scope, last verified time, request ID, and recovery action.` |

### Per-component responsive contract

| Component | Mobile ≤768 px | Tablet 769–1024 px | Desktop ≥1025 px |
|---|---|---|---|
| `CareerPlanningRiskRoute` | Four-column shell, 16 px gutter/margins, compact tabs, stack navigation, Back before detail, no horizontal page scroll at 320 CSS px | Eight-column shell, 20 px gutter, 24 px margins, collapsible sidebar, list/inspector when container permits | Twelve-column shell, 24 px gutter, max 1440 px, persistent sidebar/top bar, stable route heading/action region |
| `CareerPlanningRiskWorkbench` | List then detail stack; priority facts remain; action bar avoids virtual keyboard; controls at least 44 by 44 px | List plus inline inspector or stack by container; two-column fields only when independent; lower-priority columns move into row details | List/detail workbench; compact semantic table; virtualize over 100 rows; detail/action rail shows acting context and version |
...
|---|---|---|---|---|
| `42-career-planning-risk.md` | Exact named Zod request schema: type, optionality, default, min/max/enum/format, cross-field rule; safe local checks on blur and full authoritative check on submit | Linked summary plus exact field errors; preserve all valid and server-accepted input; no raw upstream copy | One idempotent commit with expected version; stable pending label; parsed authoritative response replaces draft; result heading receives focus | CSRF/origin rule by credential mode, unknown-key rejection, allowlist sanitization, framework output encoding, secret/PII redaction |
...
|---|---|---|---|---|
| `42-career-planning-risk.md` | `42-CAREER-PLANNING-RISK-REGISTRY` | `REGISTERED See route registry` | `2xx` parsed into `CareerPlanningRiskWorkbench`; update only after validation | BE00 typed envelope to inline, capability, conflict, rate-wait, or degraded state |

### Response field ownership

| BE source | Contract schemas | Parsed field set | UI ownership |
|---|---|---|---|
| `42-career-planning-risk.md` | Named source schemas | `GoalDefinitionV1`, `GoalProgressV1`, `DerivedMilestoneV1`, `PeerDistributionV1`, `InsuranceNeedObservationV1`, `InsuranceReferralV1`, `idempotencyKey`, `expectedVersion`, `operationId`, `AddGoalTemplateRequest`, `AddGoalTemplateResponse`, `GoalProgressReadRequest`, `DeriveMilestoneRequest`, `ShareMilestoneRequest`, `MilestoneShareResponse`, `PeerDistributionRequest`, `DetectInsuranceNeedRequest`, `RequestInsuranceReferralRequest`, `RecordProviderResponseRequest`, `SustainabilityCapabilityRequest`, `SustainabilityCapabilityResponse`, `entries`, `verified`, `declared`, `not_applicable`, `caveats`, `B2_DISABLED`, `possibleGap`, `provider_pending`, `unknown_reconciling`, `not_offered`, `financial_hard_private`, `search_path`, `IdempotencyRecord`, `AuditEvent`, `OutboxEvent`, `ProviderOperation`, `WebhookReceipt`, `goal_template`, `goal_definition`, `goal_progress_projection`, `derived_milestone`, `cohort_definition`, `peer_distribution`, `insurance_need_observation`, `insurance_referral`, `insurance_prompt_suppression`, `TargetPredicate`, `DeclaredPolicyAttributes`, `underlying_action_type`, `underlying_action_id`, `booking`, `venue_requirement`, `gear_purchase`, `finance_action`, `goal_templates`, `anon`, `authenticated`, `goal_definitions`, `goal_progress_projections`, `derived_milestones`, `cohort_definitions`, `peer_distributions`, `insurance_need_observations`, `insurance_referrals`, `external_reference_digest`, `insurance_prompt_suppressions`, `career_api`, `career_worker`, `career_provider_reconciler`, `career_retention`, `retired`, `unavailable`, `public_first_recorded_on_wejammin`, `GetGoalFactsV1`, `ComputePeerDistributionV1`, `eventType`, `privacyClass`, `retryAfterSeconds` | `CareerPlanningRiskWorkbench`: identity/version/state in `<RecordHeader>`; provenance in `<ProvenanceFact>`; command/status in `<ActionBar>`/`<StateLabel>`; remaining authorized facts in detail rows; security-only fields never serialize |
...
```ts
type CareerPlanningRiskContractField =
  | 'AddGoalTemplateRequest'
  | 'AddGoalTemplateResponse'
  | 'AuditEvent'
  | 'B2_DISABLED'
  | 'ComputePeerDistributionV1'
  | 'DeclaredPolicyAttributes'
  | 'DeriveMilestoneRequest'
  | 'DerivedMilestoneV1'
  | 'DetectInsuranceNeedRequest'
  | 'GetGoalFactsV1'
  | 'GoalDefinitionV1'
  | 'GoalProgressReadRequest'
  | 'GoalProgressV1'
  | 'IdempotencyRecord'
  | 'InsuranceNeedObservationV1'
  | 'InsuranceReferralV1'
  | 'MilestoneShareResponse'
  | 'OutboxEvent'
  | 'PeerDistributionRequest'
  | 'PeerDistributionV1'
  | 'ProviderOperation'
  | 'RecordProviderResponseRequest'
  | 'RequestInsuranceReferralRequest'
  | 'ShareMilestoneRequest'
  | 'SustainabilityCapabilityRequest'
  | 'SustainabilityCapabilityResponse'
  | 'TargetPredicate'
  | 'WebhookReceipt'
  | 'anon'
  | 'authenticated'
  ...
  | 'finance_action'
  | 'financial_hard_private'
  | 'gear_purchase'
  | 'goal_definition'
  | 'goal_definitions'
  | 'goal_progress_projection'
  | 'goal_progress_projections'
  | 'goal_template'
  | 'goal_templates'
  | 'idempotencyKey'
  | 'insurance_need_observation'
  | 'insurance_need_observations'
  | 'insurance_prompt_suppression'
  | 'insurance_prompt_suppressions'
  | 'insurance_referral'
  | 'insurance_referrals'
  | 'not_applicable'
  | 'not_offered'
  ...
  | 'privacyClass'
  | 'provider_pending'
  | 'public_first_recorded_on_wejammin'
  | 'retired'
  | 'retryAfterSeconds'
  | 'search_path'
  | 'unavailable'
  ...
  | 'venue_requirement'
  | 'verified';
interface CareerPlanningRiskWorkbenchContractFields {
  source: '42-career-planning-risk.md';
  fields: Readonly<Record<CareerPlanningRiskContractField, unknown>>;
}
```

| BE source | Owning component/prop | Every discovered application error code | UI state owner |
|---|---|---|---|
| `42-career-planning-risk.md` | `CareerPlanningRiskWorkbenchContractFields.fields` and `CareerPlanningRiskWorkbenchProps.contractFields` | `ACTION_NOT_FOUND`, `COHORT_QUERY_INVALID`, `COHORT_UNAVAILABLE`, `CONSENT_REQUIRED`, `DEPENDENCY_UNAVAILABLE`, `FORBIDDEN`, `GOAL_NOT_FOUND`, `IDEMPOTENCY_CONFLICT`, `INVALID_REQUEST`, `MILESTONE_NOT_FOUND`, `NEED_NOT_FOUND`, `OWNER_FORBIDDEN`, `POLICY_FACTS_INVALID`, `PROVIDER_MISMATCH`, `PROVIDER_SIGNATURE_INVALID`, `RATE_LIMITED`, `REFERRAL_NOT_FOUND`, `RESPONSE_STALE`, `SENSITIVE_INPUT_FORBIDDEN`, `SOURCE_REVISION_STALE`, `TEMPLATE_NOT_FOUND`, `TERMS_VERSION_STALE`, `UNAUTHENTICATED`, `VERSION_MISMATCH`, `VISIBILITY_FORBIDDEN` | validation/input → linked summary; auth/permission → auth or capability gate; not-found → disclosure-safe route/row; conflict/stale/mismatch/duplicate → sync conflict; rate → retry wait; dependency/timeout/unavailable → degraded; blocked/failed/cancelled/revoked → exact terminal state and legitimate recovery |
