...
| Promotions | A promotion version freezes each exact member's positive standalone selling price in one settlement currency and allocates full-bundle consideration proportionally. One largest-remainder pass on immutable `bundle_member_id` makes the vector exact. Ownership adjustment deducts frozen shares for already-owned members; acquired entitlements pin their shares and basis versions. Missing basis fails closed. Multi-vendor participants approve the computed immutable vector before sale. |
...
| Cent rounding | Consumes Shard 18 `RoundPayableAggregate`; this shard never authors a second rounding rule. Two different aggregate boundaries each invoke it once. Promotion admission converts the proportional full-bundle vector to minor units once, using immutable `bundle_member_id` as row/tie key so member shares sum exactly to bundle consideration. Contributor accrual remains exact decimal at >=9 dp and rounds once at period close on per-payee aggregates, using Shard-10 key `(pool, party-id, role, contribution-basis)` and DEC-011 unsigned UTF-8 `party-id` comparison. List/insertion/retrieval order and `entered-by` are prohibited; neither boundary may retain residue as platform float or revenue. |
...
- **AC-28.14 — Create promotion/bundle:** Given The vendor controls every member it contributes; each exact member listing/model has a positive independently executable standalone-price version in the promotion settlement currency; full-bundle consideration and effective window are supplied; and every participating vendor has accepted the computed promotion version, when the actor invokes this flow, then the system MUST (1) validate all member, currency, price and authority inputs, (2) authenticate and resolve acting context, (3) authorize each vendor contribution/acceptance, (4) enforce expected promotion/member-price revisions and idempotency, (5) freeze `basis_kind=standalone_selling_price` plus listing/price versions, compute `B × s_i / Σs` for every member, invoke Shard 18 largest remainder once with immutable `bundle_member_id` tie keys, and persist an allocation vector summing exactly to `B`, and (6) return an immutable promotion version/allocation hash usable for ownership-aware quotes. At cart and before capture, a verified ownership snapshot deducts the frozen shares of owned members; only unowned shares are charged/minted and each entitlement pins its member allocation evidence. Missing/zero/negative/different-currency/non-independent price returns `PROMOTION_ALLOCATION_BASIS_MISSING` or `PROMOTION_CURRENCY_MISMATCH` and publishes nothing; incomplete vendor acceptance returns `PROMOTION_SPLIT_UNACCEPTED`; ownership failure returns `OWNERSHIP_UNVERIFIED` and holds the cart; stale member/promotion state returns `PROMOTION_VERSION_STALE` and requires a visible successor quote. No path substitutes list price, equal division, vendor weights or runtime FX.
...
|---|---|---|
| `CommitDigitalPurchase` | buyer/holder, product/tier, listing/price/terms/artifact versions, payment proof, promotion/allocation/ownership-quote versions when applicable | Entitlement issuance and paid order idempotent; no stale terms consent; bundle lines equal the pinned unowned-member allocations exactly and never recompute basis after capture |
...
| `SavePromotion` | vendor products, full-bundle consideration/currency, exact listing/model and standalone-price versions, effective window, expected versions | Only `standalone_selling_price` basis. Every `ssp_minor > 0` in settlement currency. Compute raw proportional shares, invoke Shard 18 largest remainder once on immutable `bundle_member_id`, persist exact full vector/hash and vendor acceptances. Missing/invalid basis, runtime FX, list/equal/vendor weights and unaccepted multi-vendor vectors fail typed and publish nothing. |
| `QuotePromotionForHolder` | immutable promotion/allocation version, holder, verified ownership snapshot/version, quote expiry/idempotency | Payable equals sum of unowned frozen member shares; owned shares are itemized deductions and mint/earn nothing. All-owned returns zero/no purchase. Recheck before capture; changed ownership produces a visible successor quote, uncertainty returns `OWNERSHIP_UNVERIFIED` and holds cart. |
...
| `PromotionAllocation` | immutable promotion version, `standalone_selling_price` basis, settlement currency/full consideration, member/listing/price versions, positive SSP values, raw shares, minor-unit allocations/remainders, stable member tie keys, allocation hash and vendor acceptances | Full vector is computed before ownership adjustment and sums exactly to bundle consideration. Missing basis/currency or unaccepted vendor blocks admission. Each acquired entitlement cites one member row; no later price change or refund re-apportions it. |
| `PromotionPriceQuote` | holder/promotion/allocation/ownership-snapshot versions, owned and unowned member sets, itemized deductions, payable minor/currency, expiry/state/version | Payable equals sum of unowned frozen shares; all-owned means zero/no capture. Ownership change creates a visible successor; unverifiable ownership never assumes unowned. |
...
- **`PromotionAllocation`:** required fields `id: uuid`, `promotion_version_id: uuid`, `basis_kind: standalone_selling_price`, `settlement_currency: char(3)`, `full_bundle_consideration_minor: bigint`, `sum_ssp_minor: bigint`, `allocation_hash: text`, `state: draft|accepted|retired`, `version: bigint`, `created_at: timestamptz`; 1:N immutable member rows with `bundle_member_id: uuid`, `product_id: uuid`, `listing_version_id: uuid`, `monetisation_model: closed enum`, `standalone_price_version_id: uuid`, `ssp_minor: bigint > 0`, `raw_share_numerator: bigint`, `allocated_minor: bigint >= 0`, `fractional_remainder: decimal`, `tie_key: uuid = bundle_member_id`, `vendor_party_id: uuid` and vendor-acceptance ref/version. Member allocations sum exactly to full consideration; one admitted vector per promotion version.
- **`PromotionPriceQuote`:** required fields `id: uuid`, `promotion_allocation_id: uuid`, `holder_party_id: uuid`, `ownership_snapshot_id: uuid`, `ownership_snapshot_version: bigint`, `owned_member_ids: uuid[]`, `unowned_member_ids: uuid[]`, `deducted_minor: bigint`, `payable_minor: bigint`, `currency: char(3)`, `allocation_hash: text`, `expires_at: timestamptz`, `state: current|superseded|captured|expired`, `version: bigint`, `created_at: timestamptz`; payable plus deducted equals full consideration, capture requires current ownership recheck, and no buyer-specific price enters the allocation basis.
...
| Create/accept bundle allocation | no | own contributed members; accept exact immutable proportional vector | no | no manual weighting | validate SSP basis/currency, compute exact vector, hold on missing acceptance |
...
| `digital_promotion.changed.v1` | promotion, basis kind, settlement currency, allocation hash, product/listing/standalone-price versions, allocated minor values, vendor-acceptance state, effective window, version | catalog/cart/entitlement/revenue |
...
| Bundle includes already-owned product | Deduct that member's frozen allocation explicitly; it mints no entitlement and earns no payout in this acquisition. Ownership lookup failure holds cart. |
| Bundle member lacks positive standalone price in settlement currency | `PROMOTION_ALLOCATION_BASIS_MISSING` or `PROMOTION_CURRENCY_MISMATCH`; do not admit promotion and never substitute list/equal/vendor weights or runtime FX. |
| Standalone member price changes after promotion admission | Existing promotion/allocation, carts, entitlements, refunds and accrual evidence remain pinned. New selling terms require a successor promotion version and fresh vendor acceptance. |
| Holder owns every bundle member | Quote is zero with no capture and no new entitlements; show already owned rather than manufacturing a free order. |
...
- RESOLVED 2026-08-27 (F09): ideation `14.07.04` D-04 authors frozen standalone-selling-price proportional apportionment; `14.02.01` D-07 pins each acquired entitlement's allocation evidence. This shard owns the full-vector calculation, ownership quote and exact Shard 18 residue pass.
...
| 28.14 Create promotion/bundle | Same key returns the same promotion/allocation hash or holder quote. Member-price/promotion/ownership expected versions serialize one vector/quote; capture pins it and never recomputes after payment. | Vendor authority, positive same-currency SSP basis and every multi-vendor acceptance validate before admission. Missing/stale basis or unverifiable ownership publishes/charges nothing; no alternative weighting fallback. | Member/price removal retires offerability through a successor version but preserves old allocation, entitlement/refund/accrual evidence; ownership change supersedes the quote before capture without rewriting the full vector. |
...
- 2026-08-27: F09 resolved by `/resolve-ambiguity` — bundle consideration now uses immutable standalone-selling-price proportional allocation, one Shard 18 largest-remainder pass on stable member keys, ownership-adjusted quotes, missing-basis failure, vendor acceptance, and entitlement/refund/accrual evidence pins.
