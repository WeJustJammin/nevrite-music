...
- **AC-IDA-15 — Record external identifier:** Given Validate namespace format/capacity, record provenance, and attempt configured registry verification., when the actor invokes this flow, then the system MUST (1) validate inputs, (2) authenticate and resolve acting context, (3) authorize, (4) enforce revision and idempotency, (5) Record external identifier, and (6) return Identifier is labelled `self_asserted; if the flow cannot complete, verified.
