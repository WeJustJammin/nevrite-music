...
| Confirmation and announce | Confirmation requires one accepted offer version, both approval chains and artist approval; executed paper is not required. Announcement is separate and requires hard preconditions plus contract-defined deposit state. |
...
| 30.14 | Authorize announcement | Confirmation and hard prerequisite snapshot pass | Durable announce authorization emitted to downstream owner | Deposit/lineup/embargo gap returns exact blocker |
...
| `AuthorizeAnnouncement` | deal, prerequisite evidence/version | authorization | `DEPOSIT_UNSATISFIED`, `LINEUP_UNRESOLVED`, `EMBARGO_ACTIVE`, `HARD_GATE_FAILED` |
...
- **Depended on by:** [[specs/ia/31-live-settlement|Shard 31]], [[specs/ia/32-event-operations|Shard 32]], [[specs/ia/35-discovery-recommendations|Shard 35]]
...
- **Shard 31:** consume [Shard 31 Contracts](31-live-settlement.md#contracts) into this shard `§ Contracts`; publish this shard `§ Event Schemas` to [Shard 31 Event Schemas](31-live-settlement.md#event-schemas). Canonical ownership stays with the producer; typed failure/unknown states cross the same boundary.
