...
- **AC-DLV-13 — Apply/release delivery hold:** Given a valid request with current identity, authority, source state and required inputs, when the actor invokes this flow, then the system MUST (1) validate inputs, (2) authenticate and resolve acting context, (3) authorize, (4) enforce revision and idempotency, (5) Accept a protected inbound command from Shard 06, 10 or 20 carrying the caller's own case reference, subject scope and effective time; write or update `TakedownCaseLink` and apply or release the hold, and (6) return Hold state and case link commit under one version; affected references re-evaluate and an urgent purge is queued on apply; if the flow cannot complete, invalid input, stale authority, revision conflict or dependency failure returns a typed refusal and no contradictory canonical mutation.
- **AC-DLV-14 — Revoke delivery eligibility:** Given a valid request with current identity, authority, source state and required inputs, when the actor invokes this flow, then the system MUST (1) validate inputs, (2) authenticate and resolve acting context, (3) authorize, (4) enforce revision and idempotency, (5) Accept a protected inbound command from Shard 06, 10 or 20 that sets `AssetRight.state` to `restricted`, `disputed` or `revoked` against the caller's own case reference, and (6) return `AssetRight.state` commits with caller attribution; ineligible uses are removed and purge intent is written in the same transaction; if the flow cannot complete, invalid input, stale authority, revision conflict or dependency failure returns a typed refusal and no contradictory canonical mutation.
...
| DLV-13 | Apply/release delivery hold | Shard 06, 10 or 20 calls this protected command with its own case reference, subject scope and effective time; this shard writes or updates `TakedownCaseLink` and applies or releases the hold. This shard records the caller's adjudication and never performs one. | Hold state and case link commit under one version; affected references re-evaluate and an urgent purge is queued on apply. |
| DLV-14 | Revoke delivery eligibility | Shard 06, 10 or 20 calls this protected command to set `AssetRight.state` to `restricted`, `disputed` or `revoked` against its own case reference; this shard executes the delivery consequence only. | `AssetRight.state` commits with caller attribution; ineligible uses are removed and purge intent is written in the same transaction. |
