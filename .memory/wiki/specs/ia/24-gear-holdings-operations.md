...
- Supplies confirmed custody and scoped grants to gear commerce — Shard 26 for checkout, settlement and transfer sequencing; commerce may not infer rights from possession, and this shard declares no dependency on Shard 26.
...
- **Shard 26:** publish this shard `§ Contracts` — specifically `ProposeCustody` / `RespondCustody` plus the `CustodyInterval` and `CustodyGrant` models in `§ Data Models` — to [Shard 26 Contracts](26-gear-commerce-fulfilment.md#contracts), whose Cross-Domain Contracts name this shard as the supplier of confirmed custody and scoped `sell` authority for `PrepareCheckoutGroup`, `CommitCheckoutGroup` and `RecordTransferFromSettlement`; consume [Shard 26 Event Schemas](26-gear-commerce-fulfilment.md#event-schemas) — `gear_order.settled.v1` and `gear_order.return_changed.v1` — into this shard `§ Event Schemas` as the source event that marks a sold member unresolved via `gear.rig_member_unresolved.v1` and prompts case and custody reconciliation. Shard 26 owns order, settlement and remedy truth and may not infer listing, sale or possession authority from a custody record; this shard never auto-closes a custody interval or restates title from a commerce event, and ownership remains canonical in Shard 23. Canonical ownership stays with the producer and typed failure/unknown states cross the same boundary.
...
- 2026-08-05: F2 — cross-shard contract reciprocity: mirrored the inbound Shard 26 edge (confirmed custody and scoped `sell` authority out; settlement/return events in as unresolved-member and reconciliation triggers) in the Cross-Shard Section Contract Map and named Shard 26 as the downstream consumer in Dependency References (`/resolve-ambiguity`).
