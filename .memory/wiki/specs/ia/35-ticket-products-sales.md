...
| Announcement | Shard 30 owns the AcceptedDeal lifecycle, AnnounceAuthorization, canonical `confirmed_unannounced`/exactly-once `announced` transition; this shard owns the operational venue-local `OnSaleSchedule` and fan-invisible derived `Embargoed` projection. Schedule registration precedes authorization and the venue-local timer can commit only through Shard 30 with exact versions. Tour-wide on-sale is a batch template over per-show schedules, not shared mutable entity. |
...
- **AC-35.04 — Register and schedule announce/on-sale:** Given a locked manifest and accepted-deal reference, when the actor invokes this flow, then the system MUST (1) validate inputs, (2) authenticate and resolve acting context, (3) authorize schedule ownership, (4) enforce revision and idempotency, (5) persist the versioned venue-local `OnSaleSchedule`, (6) call Shard-30 `RecordOnSaleSchedule` before authorization/timer creation, and (7) return the schedule version plus the resulting authorization binding or a typed pending state; if the flow cannot complete, DST gap, stale schedule, missing binding or authorization failure keeps the fan-invisible derived `Embargoed` projection and arms no announcing timer.
...
- **AC-35.22 — Execute scheduled announce:** Given an active venue-local timer and exact current schedule, authorization and accepted-deal lifecycle versions, when the system worker invokes this flow, then the system MUST (1) validate the callback and idempotency key, (2) authenticate the worker context, (3) authorize the protected Shard-30 boundary, (4) re-read and pin the exact versions, (5) call Shard-30 `CommitScheduledAnnouncement`, and (6) publish/update only the resulting schedule projection; if the flow cannot complete, missing, stale, revoked, superseded or unknown versions fail closed, preserve `Embargoed`, and never publish or retry with weakened evidence.
...
| 35.04 | Register and schedule announce/on-sale | Locked manifest and accepted-deal reference; Shard-30 boundary available | Versioned venue-local `OnSaleSchedule` is recorded in Shard 35, Shard 30 binds authorization to the exact schedule, and only then is a durable timer eligible | DST gap, stale edit, missing binding or authorization failure leaves fan-invisible `Embargoed` and arms no announce timer |
...
| `ScheduleOnSale` | accepted-deal reference, locked manifest, venue-local datetimes/timezone, idempotency key | versioned `OnSaleSchedule`, Shard-30 schedule-registration result and authorization binding/pending state | `MANIFEST_UNLOCKED`, `DST_NONEXISTENT`, `STALE_VERSION`, `AUTHORIZATION_NOT_BOUND` |
| `ExecuteScheduledAnnouncement` | schedule id/version, authorization id/version, accepted-deal lifecycle version, due-at and idempotency key | timer result and derived schedule projection | `SCHEDULE_VERSION_STALE`, `AUTHORIZATION_MISSING_OR_STALE`, `DEAL_LIFECYCLE_UNKNOWN`, `ANNOUNCE_ALREADY_COMMITTED` |
...
- Shard 30 owns AcceptedDeal lifecycle, AnnounceAuthorization and the canonical `confirmed_unannounced` → exactly-once `announced` transition. Shard 35 owns fan-facing product/schedule execution, venue-local timers and the fan-invisible derived `Embargoed` projection.
- `ScheduleOnSale` persists the schedule first and calls Shard-30 `RecordOnSaleSchedule`; no timer can announce until that boundary returns an authorization bound to the exact schedule version. `ExecuteScheduledAnnouncement` calls Shard-30 `CommitScheduledAnnouncement` with exact current schedule, authorization and lifecycle versions.
- Shard 35 never stores or mutates a second AcceptedDeal/lifecycle truth. Missing, stale, revoked, superseded or unknown upstream facts fail closed; schedule and lifecycle event consumers dedupe by event ID and reject out-of-order aggregate versions.
...
| `OnSaleSchedule` | Canonical Shard-35 schedule with venue-local/UTC announce/public/presale windows, accepted-deal and authorization IDs/versions, source lifecycle version, schedule/job versions and fan-invisible derived `Embargoed` projection |
...
- `OnSaleSchedule` is the only canonical schedule aggregate in Shard 35. `Embargoed` is derived from schedule state plus the current Shard-30 authorization/lifecycle projection, is never fan-visible, and cannot mutate AcceptedDeal or AnnounceAuthorization.
...
| Operator/box office | Configure show inventory/on-sale/fees, guest funders, resolve delivery; create and revise the venue-local `OnSaleSchedule` | Mint/revoke `AnnounceAuthorization`, mutate `AcceptedDeal` lifecycle, bypass Shard-30 commit/evidence gates, Artist-only allocation spend beyond role; disability proof collection |
...
| System worker | Register schedule callbacks, execute venue-local timers, queue, expire/restore units, deliver/update passes; call Shard-30 announce commands with exact pinned versions | Direct AcceptedDeal/AnnounceAuthorization mutation, queue priority, accessible release, hidden repricing or involuntary unwind |
...
| `ticketing.schedule.changed.v1` | show, schedule, schedule version, accepted-deal reference, authorization id/version when bound, source lifecycle version, local/UTC times, window/job states, derived `Embargoed` state and event ID | discovery, notifications, Shard-35 timer worker |
...
Shard-30 lifecycle and authorization inputs are consumed as versioned `booking.deal.lifecycle_changed.v1` and `booking.announce.authorization_changed.v1` events. Events carry opaque refs, not code/barcode/claim secrets, guest contact data or disability information. Consumers dedupe by event ID and order by aggregate version; duplicate or out-of-order schedule/lifecycle events cannot clear `Embargoed` or arm a timer. No event consumer creates a second canonical AcceptedDeal or authorization state.
...
| Schedule registration precedes authorization | Persist the versioned `OnSaleSchedule`, call Shard-30 `RecordOnSaleSchedule`, and keep `Embargoed` fan-invisible until authorization binds that exact schedule; no timer is armed early |
| Missing or stale authorization/lifecycle on timer callback | Preserve `Embargoed`, cancel or pause the job, return a typed boundary error, and require a fresh exact-version callback; never announce on time alone |
| Authorization revoked or deal superseded after scheduling | Shard-30 versioned event invalidates the pending job and derived projection; schedule remains operational history but cannot publish |
| Duplicate or out-of-order schedule/lifecycle event or timer callback | Dedupe by event/idempotency key and reject lower versions; Shard-30 owns the single exactly-once canonical transition, while Shard 35 only projects the result |
| Operator attempts lifecycle or authorization mutation | Deny with typed authority error; only Shard-30 capability/evidence gates can change those canonical records |
...
| 35.04 Register and schedule announce/on-sale | Same idempotency key returns the same schedule-registration result; competing schedule revisions serialize one winner, invalidate prior timer versions and cannot create duplicate authorization bindings. | Schema, authority, manifest, DST and Shard-30 boundary validation fail before timer creation; missing/unknown authorization remains `Embargoed` without existence leakage. | Schedule deletion/revocation preserves required immutable schedule/evidence history, invalidates derived projection/jobs, and never rewrites Shard-30 lifecycle state. |
...
| 35.22 Execute scheduled announce | Duplicate timer callbacks with the same pinned versions return the same terminal result; callbacks with lower schedule/authorization/lifecycle versions lose without side effects. | Worker validates all exact versions and Shard-30 capability; missing, stale, revoked, superseded or unknown facts fail closed and keep `Embargoed`. | Revocation/supersession cancels or pauses pending jobs; Shard-30 retains canonical lifecycle evidence and exactly-once `announced` history, while Shard 35 removes only its derived operational projection. |
...
- **Shard 30:** Shard 35 owns and versions `OnSaleSchedule`, registers it before authorization through Shard-30 `RecordOnSaleSchedule`, and consumes versioned `AnnounceAuthorization` plus `AcceptedDeal` lifecycle events. Its venue-local timer calls Shard-30 `CommitScheduledAnnouncement` with exact schedule, authorization and lifecycle versions. Shard 30 alone owns AcceptedDeal lifecycle, AnnounceAuthorization and canonical `confirmed_unannounced`/exactly-once `announced`; Shard 35 owns only the fan-invisible derived `Embargoed` projection. No duplicate canonical state or reconciliation loop; Operator cannot mutate the Shard-30 records. See [Shard 30 Contracts](30-booking-contracts.md#contracts) and [Shard 30 Event Schemas](30-booking-contracts.md#event-schemas).
...
| 2026-08-28 | F10 Q05 — made Shard 30 canonical owner of AcceptedDeal/authorization/announce lifecycle; Shard 35 now registers and versions venue-local schedules, executes exact-version timers through Shard-30 commands, and keeps `Embargoed` as a fan-invisible projection with versioned dedupe/fail-closed rules | `/resolve-ambiguity` | Architecture Decisions, AC/Interactions 35.04/35.22, Contracts, Data Models, Access Control, Event Schemas, Edge Cases, Coverage Matrix, Cross-Shard Dependencies |
