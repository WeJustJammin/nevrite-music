...
const workflow = readFileSync(
  new URL(
    '../.github/workflows/attest-ac265-hosted-runner-mapping.yml',
    import.meta.url,
  ),
  'utf8',
);
...
const attestationStep = workflow.match(
  /\n\s{6}- name: Create protected canonical runner-mapping attestation[\s\S]*?(?=\n\s{6}- name: Upload)/u,
);
const uploadStep = workflow.match(
  /\n\s{6}- name: Upload protected mapping source artifacts[\s\S]*$/u,
);

describe('AC265 protected runner-mapping source workflow contract', () => {
  it('is manual-only, names the protected source foundation, and accepts exactly the two required strings', () => {
    expect(workflow).toMatch(
      /^name:\s*AC265 protected source foundation \(runner mapping attestation\)\s*$/mu,
    );
    ...
    expect(dispatchInputs).toEqual(['authorization_ref', 'mapping_id']);
  ...
  it('passes only protected runtime inputs to the attestation entrypoint', () => {
    expect(attestationStep).toBeDefined();
    const step = attestationStep?.[0] ?? '';
    expect(
      step.match(
        /node --experimental-strip-types infra\/workflows\/attest-ac265-approved-runner-mapping\.ts/g,
      ),
    ).toHaveLength(1);

    for (const expected of [
      'AC265_AUTHORIZATION_REF: ${{ inputs.authorization_ref }}',
      'AC265_MAPPING_ID: ${{ inputs.mapping_id }}',
      'AC265_RUNNER_MAPPING_OUTPUT_DIR: ${{ runner.temp }}/ac265-runner-mapping',
      'SUPABASE_URL: ${{ vars.SUPABASE_URL }}',
      'SUPABASE_PROJECT_REF: ${{ vars.SUPABASE_PROJECT_REF }}',
      'SUPABASE_SECRET_KEY: ${{ secrets.SUPABASE_SECRET_KEY }}',
      'AC265_RUNNER_MAPPING_SIGNING_PRIVATE_KEY_PEM: ${{ secrets.AC265_RUNNER_MAPPING_SIGNING_PRIVATE_KEY_PEM }}',
      'AC265_RUNNER_MAPPING_SIGNING_KEY_ID: ${{ vars.AC265_RUNNER_MAPPING_SIGNING_KEY_ID }}',
    ])
      expect(step).toContain(expected);
  });

  it('uploads exactly the canonical mapping and attestation JSON for one short-lived run artifact', () => {
    ...
    expect(upload).toContain(
      'name: ac265-runner-mapping-${{ github.run_id }}-${{ github.run_attempt }}',
    );
    expect(upload).toContain(
      '${{ runner.temp }}/ac265-runner-mapping/canonical-runner-mapping.json',
    );
    expect(upload).toContain(
      '${{ runner.temp }}/ac265-runner-mapping/runner-mapping-attestation.json',
    );
    ...
    expect(upload).not.toMatch(
      /path:\s*\|[\s\S]*\$\{\{ runner\.temp \}\}\/ac265-runner-mapping\s*$/mu,
    );
    expect(upload).toContain('if-no-files-found: error');
    expect(upload).toContain('retention-days: 1');
    expect(upload).not.toMatch(
      /\b(?:SUPABASE_SECRET_KEY|AC265_RUNNER_MAPPING_SIGNING_PRIVATE_KEY_PEM)\b/u,
    );
