...
const RANGE_PATTERN =
  /(?:P2-S06-)?AC-(\d{3})\s*(?:\.\.|…|–|—|-|\bthrough\b|\bto\b)\s*(?:(?:P2-S06-)?AC-)?(\d{3})/gu;
const SINGLE_ID_PATTERN = /(?<![A-Z0-9-])(?:P2-S06-)?AC-(\d{3})(?!\d)/gu;
...
describe('Phase 2 Slice 06 acceptance depth audit', () => {
  it('enumerates exactly the locked Slice 06 acceptance criteria', () => {
    expect(ACCEPTANCE_IDS).toHaveLength(ACCEPTANCE_COUNT);
    expect(new Set(ACCEPTANCE_IDS)).toHaveLength(ACCEPTANCE_COUNT);
    expect(ACCEPTANCE_IDS[0]).toBe('P2-S06-AC-001');
    expect(ACCEPTANCE_IDS.at(-1)).toBe('P2-S06-AC-121');
    ACCEPTANCE_IDS.forEach((id, index) => {
      expect(id).toBe(acceptanceId(index + 1));
    });
  });

  it('binds every criterion to executable Slice 06 evidence', () => {
    expect(
      missingIds,
      `Missing concrete Slice 06 evidence (${missingIds.length}): ${missingIds.join(', ')}`,
    ).toEqual([]);
  });
  ...
  it(`keeps every Slice 06 test at or below ${MAX_TEST_LINES} lines`, () => {
