...
describe('production AC209 queue-to-email exercise orchestration', () => {
  it('binds one real retry, provider delivery, database receipt, and exact cleanup', async () => {
    ...
    expect(deps.verifyEmailSettings).toHaveBeenCalledWith({
      zoneId: ZONE_ID,
      token: 'email-analytics-token-that-is-never-reported',
    });
    ...
    // The settings gate and the zero-row-tolerant events probe are independent
    // capabilities: both must pass before the step records cleanup_required.
    expect(deps.verifyEmailCapability.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  it('leaves the zero-row-tolerant events probe intact alongside the settings gate', async () => {
    const deps = dependencies();
    // The provider-reported events dataset is empty, which the events probe
    // accepts as proof of read access, and which never satisfied the settings
    // gate that AC209's runbook requires.
    deps.verifyEmailCapability.mockImplementationOnce(async () => undefined);

    const report = await exerciseProductionAc209(await baseInput(), deps);

    expect(report.queue).toStrictEqual(queueReport);
    expect(deps.verifyEmailCapability).toHaveBeenCalledOnce();
    expect(deps.verifyEmailSettings).toHaveBeenCalledOnce();
    expect(deps.beforeQueueAccess).toHaveBeenCalledExactlyOnceWith();

    // The events probe keeps accepting an empty provider window; that tolerance
    // is exactly why it cannot stand in for the settings gate.
    expect(deps.verifyEmailCapability).toHaveBeenCalledWith({
      zoneId: ZONE_ID,
      token: 'email-analytics-token-that-is-never-reported',
    });
