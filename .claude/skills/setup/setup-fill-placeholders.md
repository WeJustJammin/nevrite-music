...
This skill surgically fills `{{PLACEHOLDER}}` values across the Claude Code kit. It is idempotent — already-filled placeholders are not overwritten, and new values are only applied where explicitly provided.
...
| `CONTRACT_LIBRARY` (also fills `.claude/rules/tdd-contract-first.md`) | Rule templates |
| `VALIDATION_COMMAND` | `.claude/instructions/commands.md`, root configs |
| `FRAMEWORK_PATTERNS` | `.claude/instructions/patterns.md` |
| `PROJECT_STRUCTURE`, `ARCHITECTURE_TABLE` | `.claude/instructions/structure.md` |
...
**Target**: `.claude/instructions/tech-stack.md`
...
**Target**: `.claude/instructions/tech-stack.md` → Cross-Cutting Skills table
...
**Target**: `.claude/instructions/tech-stack.md` → Global Settings table
...
Scan `.claude/rules/*.md` for `{{PLACEHOLDER}}` values:
...
Scan all `.claude/skills/*/SKILL.md` for `{{PLACEHOLDER}}` values and fill matches:
