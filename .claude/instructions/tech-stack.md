...
### How Claude Code Workflows Use This Map
...
4. Resolve each skill through the 4-tier resolution chain (see `.claude/skills/utilities/resolve-skill.md`)
...
Each cell is a comma-separated list of skill directory names from `.claude/skills/`. Use `—` for "not applicable."
...
Verification gates in `workflow-plan-phase` and `workflow-implement-slice` check these conditions. See `.claude/skills/setup/setup-verify.md` for the full verification procedure.
