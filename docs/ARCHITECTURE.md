...
**Implementation scope:** Phase 1 operational foundation plus Phase 2 Slices 01–09 identity access, authority, organizations, profile ownership, public profile/portfolio delivery, governed settings and admin workspaces, the content-schema/block registry, its production operational-alert boundary, protected AC211 evidence collection, the AC265 per-tab acting-context bridge, protected candidate-enrollment/GitHub-OIDC runner-authorization foundation, staging-only one-use outage-lease control plane, the promoted staging CP-02 approved safe-resource/runner-mapping registry foundation, the promoted CP-03 signed runner-mapping attestation application foundation, the promoted CP-04a approved outage-target read/attestation foundation, and the unpromoted CP-04b approved outage-target registration foundation, plus audited idempotency TTL retention  
**Deployment status (2026-09-21):** The current promoted baseline is PR #87 at exact-main SHA `2b83b9abe0f4de3992b6514e9d2f2ffcb83ca770`; exact-main CI `35601260266` passed, and staging workflow `35602077901` / deployment `6568798373` succeeded at `https://staging.wejamm.in`. CP-01 through CP-04a are deployed foundations. CP-04b is present only as an unpromoted local contract/client/migration/RPC/test foundation; it is not part of this deployed baseline. AC265 preflight `34824500796` passed against an earlier application candidate, while authorization foundation run `34824651793` failed closed at `staging_prepare`; no hosted browser case ran. No live dependency/route policy, target, signing-key configuration, retained artifact, hosted matrix, or independently authenticated receipt exists. Read-only AC209 verifier `34813947512` failed before any effect with `provider_graphql_error`; AC211 collection `35560241699` found zero qualifying command/RPC/acceptance/queue samples and produced no artifact. Production remains on `c8f0cbd52cb6140ee1a756f106fa329f8c23b0e2` / deployment `6417116181`. Slice 09 remains 279/282 active (283 authored), and Slice 10 remains locked on AC209, AC211, and AC265.  
**CP-04a promotion status (2026-09-21):** PR #87 is the current promoted baseline carrying the previously promoted approved outage-target read/attestation foundation. Exact-main CI `35601260266` and staging workflow `35602077901` / deployment `6568798373` are green. Focused CP-04a verification remains a local trust-boundary result; the deployed foundation does not supply live signing-key configuration, seeded rows, retained artifacts, protected attestation-workflow execution, a hosted matrix, a receipt, or AC265 acceptance. A protected orchestrator remains required.  
**CP-04b local foundation status (2026-09-21):** The unpromoted CP-04b change adds a strict `ac265-hosted-approved-outage-target-registration-v1` contract and bounded service-role RPC client, plus migration `20260921040000_ac265_approved_outage_target_registration.sql`, immutable policy/registration ledgers, and registration/concurrency tests. The server accepts only the criterion, schema version, authorization reference, exact policy reference, and idempotency reference; it derives the target, candidate identity, project, route, timestamps, and expiry from the active authorization, verified candidate, and policy. The policy target validity is exactly 120 seconds, reserving a bounded 60-second acquisition window before CP-01's exact 60-second, one-request lease; both the active policy and authorization must cover the full 120-second target window. Direct SQL integration registers a target, then acquires the CP-01 lease and proves `state=acquired`, the registered target reference, `leaseDurationSeconds=60`, and `requestLimit=1`. Future-dated and too-short policy windows return only the generic conflict sentinel. Canonical target JSON is hashed with the tested digest `ecd573c63eff6b11b4ff044a70a8180901948e88a9c1db036f51f4a27e1f3d7e`; forced RLS, service-role-only execution, immutable triggers, generic conflicts, and lock-proven concurrency tests keep registration fail-closed. Current local CP-04b evidence is 35 registration pgTAP assertions, 2 concurrency assertions, and 15 RPC-client tests covering redirects, status failures, duplicate keys, malformed/oversized lengths and streams, cancellation, reader failures, timeout, invalid UTF-8, and secret non-disclosure. The migration seeds no policy or registration rows: no live dependency/route policy, target, signing key, retained artifact, hosted matrix, or receipt exists. CP-04b therefore does not alter the deployed baseline or close AC265; Slice 10 remains locked.  
...
**Phase 2 completion policy (DEC-101, 2026-09-20):** The canonical plan retains 2000 authored criteria, including 283 authored contiguous Slice 09 IDs. Phase 2 implementation completion uses a 1999-criterion denominator and Slice 09 uses a 282-criterion implementation denominator. AC266 remains unchecked while deferred and is a mandatory post-Phase 2 production-readiness/release gate; it must not be marked passed, waived, or simulated and requires genuine protected macOS/Safari/VoiceOver and Windows/Firefox/NVDA evidence. Slice 10 implementation unlocks only after AC209, AC211, and AC265 pass.

> This document maps implemented behavior, not the larger product architecture described in `.memory/wiki/specs/2026-08-02-architecture-design.md`. Generated output (`dist/`, `.astro/`, coverage), dependency trees (`node_modules/`), and generated Cloudflare ambient declarations are excluded from the tree. Slice 07 adds governed settings; Slice 08 adds the private admin authority and workspace; Slice 09 adds eight `CMS-03A` routes, twelve private content-schema/block tables, a server-first modeling workbench, a fenced activation/migration consumer, a fail-closed scheduled operational-alert pipeline, and a manual protected AC211 evidence collector. The collector exists on disk, but no eligible production UTC-day report has been retained. Three environment-owned implementation gates keep Slice 09 at 279/282 within its 282-criterion implementation denominator; all 283 authored Slice 09 IDs remain in the plan. Slice 10 remains locked until AC209, AC211, and AC265 pass. AC266 is not an implementation-completion gate: it remains authored and unchecked as the mandatory post-Phase 2 production-readiness/release gate (`.memory/pipeline/progress/slices/phase-02-slice-09.md`; `.memory/pipeline/progress/verification/2026-09-14-ac209-capability-and-ac265-authorization-foundation.md`). `CFG-05A-01` through `CFG-05A-04`, `CFG-05B-01`, `CFG-05B-04`, and the `read_audit` branch of `CFG-05B-05` are active. `CFG-05B-02`, `CFG-05B-03`, and `run_diagnostic` remain deferred/unmounted (`.memory/pipeline/progress/slices/phase-02-slice-08.md`; `.memory/wiki/operations/runbooks/platform-configuration.md`).
...
`2b83b9abe0f4de3992b6514e9d2f2ffcb83ca770` and staging deployment
`6568798373`. Corrected preflight `34824500796` passed against PR #80
...
remain absent. The current promoted documentation baseline is PR #87 at exact-main
SHA `2b83b9abe0f4de3992b6514e9d2f2ffcb83ca770`, exact-main CI `35601260266`,
staging workflow `35602077901`, and deployment `6568798373`
...
the implementation denominators are 1999 for Phase 2 and 282 for Slice 09.
AC209, AC211, and AC265 are the mandatory external gates for Slice 09
implementation completion and Slice 10 unlock. AC266 remains authored,
...
and combined verification can close it.
...
- Slice 09 is locally QA-GREEN. Current promoted baseline PR #87 at exact main SHA `2b83b9abe0f4de3992b6514e9d2f2ffcb83ca770` passed exact-main CI `35601260266`; staging workflow `35602077901` / deployment `6568798373` succeeded at `https://staging.wejamm.in`; production remains on `c8f0cbd52cb6140ee1a756f106fa329f8c23b0e2` through run `34734646414` / deployment `6417116181`. Slice 09 remains implementation-blocked at 279/282 within its 282-criterion denominator: AC209 verifier `34813947512` failed before any effect with `provider_graphql_error`, so Zone Analytics Read remains unproven and AC209 still needs a successful exact-zone exercise plus real Gmail receipt; AC211 run `35560241699` passed preflight but failed closed with zero eligible samples and no artifact, so it still needs a complete eligible UTC-day report with natural sample floors; AC265 preflight `34824500796` passed against PR #80 SHA `918f598525de772c82b0a0bcd82348ea8f5d523d`, but authorization foundation run `34824651793` failed at `staging_prepare`, so no hosted 9-role/10-scenario Auth/RLS/IdP matrix ran. CP-01 through CP-04a are promoted foundations. CP-04b is an unpromoted local registration foundation: its policy table is empty outside disposable tests, with no live dependency/route policy, target, signing-key configuration, retained artifact, hosted matrix, or independently authenticated receipt. None closes AC265. Slice 10 remains locked until AC209, AC211, and AC265 pass. AC266 remains authored and unchecked as the mandatory post-Phase 2 production-readiness/release gate; it needs both real manual platform reports plus protected combined verification and cannot be advanced by waiver or simulation. Any later promotion must continue to verify exact source SHA, migration, contracts, artifact identity, protected review, and retained evidence before activation.
...
Phase 1 validation is closed and Phase 2 Slices 01–09 are implemented on disk. Slice 09 — **content schemas, relations, activation, and block registry** — is locally QA-GREEN but remains blocked at 279/282 within its 282-criterion implementation denominator; all 283 authored Slice 09 IDs remain retained. Promoted baseline PR #87 at exact main SHA `2b83b9abe0f4de3992b6514e9d2f2ffcb83ca770` passed exact-main CI `35601260266`; staging workflow `35602077901` / deployment `6568798373` succeeded at `https://staging.wejamm.in`; production remains on the preceding verified release. AC209 must prove Zone Analytics Read before a successful protected exercise and real Gmail receipt; AC211 still needs an eligible complete UTC-day report with natural sample floors; and AC265 needs its remaining protected population source, session broker, receipt resolver/issuer, hosted integration, and approved 9-role/10-scenario Auth/RLS/IdP matrix. CP-02, CP-03, and CP-04a are promoted foundations. CP-04b is unpromoted and only provides strict server-derived target registration with an empty policy table outside disposable tests; it has no live dependency/route policy, target, signing key/configuration, retained artifact, hosted browser matrix, or receipt and does not establish hosted acceptance. Slice 10 implementation unlocks only after AC209, AC211, and AC265 pass. AC266 remains unchecked as the mandatory post-Phase 2 production-readiness/release gate and still needs both real manual platform reports plus protected combined verification; Linux, simulated, waived, or synthetic evidence cannot substitute. The next step remains genuine Slice 09 external evidence and exact-candidate hardening, not Slice 10 implementation. Slices 10–17 remain dependency-locked by their individual gates (`.memory/pipeline/progress/phases/phase-02.md`; `.memory/pipeline/progress/slices/phase-02-slice-09.md`).
