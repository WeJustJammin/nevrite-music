...
7. The manual protected AC209 exercise first recollects exact-version alert configuration, verifies the database cooldown, and requires two independent capability preflights before it records `cleanup_required`: a zero-row-tolerant events probe that proves read access to the exact zone's dataset, and a Settings-node gate that fails closed unless `emailSendingAdaptive` is enabled, every field the event query selects is available to the observability token, and the requester limits cover the 50-row bound and all seven selections. It then requires empty exact production source/DLQ peeks. It pushes one opaque malformed envelope, proves real retry exhaustion, and keeps that exact message in the DLQ while a bounded Email Sending analytics query discovers its opaque provider-owned identifier (visible ASCII, at most 512 bytes, not a caller-authored RFC `Message-ID`). A service-only Supabase RPC hashes that identifier and binds it to exactly one delivered row for the release whose claim and delivery are both at or after `notBefore = exercise.startedAt`; the internal UUID receipt remains separately hashed and private. Provider bodies are bounded while streaming and fail closed on timeout, invalid UTF-8, or malformed envelopes. Migration `20260910030000` is the expand phase that preserves the deployed five-field completion caller; a separate forward-only migration may require `providerMessageId` only after the new Worker is verified live. The main path and an idempotent `always()` safety step purge only correlated peek refs; retained output contains digests and allowlisted identities, explicitly leaving Gmail inbox verification pending (`.github/workflows/exercise-production-ac209.yml`; `infra/workflows/ac209-{queue,email-sending,delivery,production-exercise}*.ts`; `supabase/migrations/20260910030000_ac209_operational_alert_verification.sql`).
...
Phase 1 validation is closed and Phase 2 Slices 01–09 are implemented on disk. Slice 09 — **content schemas, relations, activation, and block registry** — is locally QA-GREEN but remains blocked at 279/282 within its 282-criterion implementation denominator; all 283 authored Slice 09 IDs remain retained. Promoted baseline PR #88 at implementation main SHA `52b66272e61331827c59ac1e169868474a2c09c8` passed exact-main CI `35612415141`; staging workflow `35613284966` / deployment `6570861931` succeeded at `https://staging.wejamm.in`; production remains on the preceding verified release. AC209 read-only verifier `35612514031` passed exact-main preflight/protection/workspace checks but failed closed at the capability query with `provider_graphql_error`, producing no effect or receipt; AC209 still needs a successful exact-zone exercise and real Gmail receipt. AC211 still needs an eligible complete UTC-day report with natural sample floors; and AC265 needs its remaining protected population source, session broker, receipt resolver/issuer, hosted integration, and approved 9-role/10-scenario Auth/RLS/IdP matrix. CP-02, CP-03, CP-04a, and CP-04b are promoted foundations. CP-04b's policy table remains empty outside disposable tests; it has no live dependency/route policy, target, signing key/configuration, retained artifact, hosted browser matrix, or receipt and does not establish hosted acceptance. Slice 10 implementation unlocks only after AC209, AC211, and AC265 pass. AC266 remains unchecked as the mandatory post-Phase 2 production-readiness/release gate and still needs both real manual platform reports plus protected combined verification; the automated axe digest from PR #88 cannot substitute, and Linux, simulated, waived, or synthetic evidence cannot substitute. The next step remains genuine Slice 09 external evidence and exact-candidate hardening, not Slice 10 implementation. Slices 10–17 remain dependency-locked by their individual gates (`.memory/pipeline/progress/phases/phase-02.md`; `.memory/pipeline/progress/slices/phase-02-slice-09.md`).
