---
name: setup-cfsa
description: Main setup workflow for Codex CFSA pipeline — gather project info, fill placeholders, provision skills, verify installation
parameters:
  - name: project_name
    type: string
    required: true
...
The setup-cfsa skill is the main entry point for configuring the CFSA pipeline in Codex. It gathers project information, fills global placeholders, provisions skills from the skill library, and verifies the installation is ready for pipeline execution.

This is the Codex equivalent of Antigravity's `/bootstrap-agents` workflow.

## Prerequisites

1. Codex is installed and project is open
2. `.codex/` directory structure exists (created by `cfsa-antigravity init --agent codex`)
...
**What gets filled:**
- Per-Surface Skills table in `.codex/instructions/tech-stack.md`
...
2. For each skill name in every cell:
   ...
   - Install matching skills from `.codex/skill-library/`
...
```
✅ CFSA Pipeline is ready for Codex!
...
Documentation:
- .codex/README.md — Codex version guide
...
| Surface Stack Map can't be read | Fail with clear error — run `cfsa-antigravity init --agent codex` |
...
- `.codex/skills/setup/setup-fill-placeholders.md` — Surface Stack Map filling
- `.codex/skills/setup/setup-provision-skills.md` — Skill provisioning
- `.codex/skills/setup/setup-verify.md` — Installation verification
- `.codex/skills/utilities/resolve-skill.md` — 4-tier resolution chain
