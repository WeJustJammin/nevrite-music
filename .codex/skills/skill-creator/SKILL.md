...
- Extending the `.codex/skills/` or `.codex/skill-library/` directories
...
Every skill lives in a directory under `.codex/skills/` (installed, always available) or `.codex/skill-library/` (catalog, provisioned per-project):
...
| Is this installed or library? | `.codex/skills/` (always active) vs `.codex/skill-library/` (provisioned) |

**Decision rules:**
- Cross-cutting concerns (planning, debugging, testing) → `.codex/skills/`
- Tech-specific (React, PostgreSQL, Docker) → `.codex/skill-library/stack/`
- Surface-specific (web perf, mobile UX, API design) → `.codex/skill-library/surface/`
- Meta/tooling (git, MCP builders, tmux) → `.codex/skill-library/meta/`
...
If the skill goes in `.codex/skill-library/`, add it to `MANIFEST.md`:
...
If the skill goes in `.codex/skills/`, no registration needed — it's always active.
...
### `.codex/skill-library/` (Library Skills)
