...
Read `.codex/skills/bootstrap-agents/SKILL.md` and execute with the provided key(s).
...
|----------|----------------|---------------|
| Language/Framework/DB/ORM | Surface stack map in `.codex/instructions/tech-stack.md` | The cell for the provided key's surface row is no longer empty |
| CI/CD, Hosting, Auth, Security | Cross-cutting section of surface stack map | The cell is no longer empty |
| Design direction | `.codex/skills/brand-guidelines/SKILL.md` | `{{DESIGN_DIRECTION}}` no longer appears as literal text |
| Project structure | `.codex/instructions/structure.md` | `{{PROJECT_STRUCTURE}}` no longer appears as literal text |
| Dev tooling | `.codex/instructions/commands.md` | Template values are replaced with real commands |
| New dependency skill | `.codex/skills/[skill-name]/SKILL.md` | The skill directory exists and `SKILL.md` is readable |
