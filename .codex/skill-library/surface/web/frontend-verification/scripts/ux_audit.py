...
8. COLOR SYSTEM (6 sections):
    ...
    Note: Purple Ban is handled by design-anti-cliche skill, not this scanner.
...
Total: 75+ checks across all design principles (Purple Ban removed — handled by design-anti-cliche skill)
"""
...
class UXAuditor:
    def __init__(self, skip_categories=None):
        self.issues = []
        self.warnings = []
        self.passed_count = 0
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        if 'psychology' not in self.skip_categories:
            # Hick's Law
            ...
            nav_items = 0  # Default for downstream checks
        ...
        # --- 4. COLOR SYSTEM ---
        # Note: Purple Ban removed — handled by design-anti-cliche core skill, not code scanner
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='UX Audit - Frontend Design Quality Scanner')
    parser.add_argument('path', help='File or directory to audit')
    parser.add_argument('--json', action='store_true', help='Output as JSON')
    parser.add_argument('--skip-psychology', action='store_true', help='Skip psychology law checks')
    parser.add_argument('--skip-typography', action='store_true', help='Skip typography checks')
    parser.add_argument('--skip-visual', action='store_true', help='Skip visual effects checks')
    parser.add_argument('--skip-color', action='store_true', help='Skip color system checks')
    parser.add_argument('--skip-animation', action='store_true', help='Skip animation checks')
    parser.add_argument('--skip-motion', action='store_true', help='Skip motion graphics checks')
    parser.add_argument('--skip-accessibility', action='store_true', help='Skip accessibility checks')
    args = parser.parse_args()

    skip = set()
    if args.skip_psychology: skip.add('psychology')
    if args.skip_typography: skip.add('typography')
    if args.skip_visual: skip.add('visual')
    if args.skip_color: skip.add('color')
    if args.skip_animation: skip.add('animation')
    if args.skip_motion: skip.add('motion')
    if args.skip_accessibility: skip.add('accessibility')

    auditor = UXAuditor(skip_categories=skip)
    if os.path.isfile(args.path): auditor.audit_file(args.path)
    else: auditor.audit_directory(args.path)
    ...
    # Exit codes: 0 = pass, 1 = issues found, 2 = warnings only
    if report['issues']:
        sys.exit(1)
