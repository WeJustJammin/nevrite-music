...
## Components
- **CSS architecture** — CSS custom properties plus named cascade layers; Astro scoped styles for Astro components and CSS Modules for React islands. Tailwind and runtime CSS-in-JS are prohibited.
...
- **Error format** — Exactly `{ code: string, message: string, requestId: UUID string, details: Readonly<Record<string, JsonValue>> }`; HTTP status remains on the response line and each code allowlists detail fields
...
- **Pagination** — `cursor?: string` and `limit?: 1..50` (default 25); return `{ items, nextCursor, hasMore }`; authenticated cursors bind route/filter/sort/audience/acting context and expire within 24 hours
...
## Observability → skill: `logging-best-practices`
- **Runtime logger** — Import the typed logger port from `@wejammin/observability`; domain/application code never calls ambient `console.*`
- **Sink** — Emit one newline-delimited JSON event to the Cloudflare Workers console sink after schema validation and scrubbing
