...
1. Read `.claude/instructions/tech-stack.md`
2. Read `.claude/skill-library/MANIFEST.md`
...
1. **Tier 1 (exact):** `.claude/skills/{name}/SKILL.md` exists or exact manifest mapping exists
...
Use `.claude/skills/utilities/resolve-skill.md` for resolution behavior.

### 3) Provision

For each resolved library path:

1. Copy library directory into `.claude/skills/{installed-name}/`
...
- `.claude/instructions/tech-stack.md` (`{{INSTALLED_SKILLS}}`)
