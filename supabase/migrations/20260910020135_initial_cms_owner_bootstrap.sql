-- Operator-only initial-owner setup. This is not a user session, step-up proof,
-- or a public bootstrap endpoint. Normal request-time authorization is unchanged.
create table platform_private.cms_owner_initialization (
  singleton boolean primary key default true check (singleton),
  auth_user_id uuid not null references auth.users(id),
  person_id uuid not null references platform_private.person_party(party_id),
  alias_id uuid not null references platform_private.alias_party(party_id),
  organization_id uuid not null references identity_private.organization_party(party_id),
  authorization_ref uuid not null,
  operator_role name not null,
  grant_ends_at timestamptz not null,
  created_at timestamptz not null default clock_timestamp()
);
alter table platform_private.cms_owner_initialization enable row level security;
alter table platform_private.cms_owner_initialization force row level security;
revoke all on platform_private.cms_owner_initialization from public, anon, authenticated, service_role;

create function platform_private.initialize_cms_owner(
  p_auth_user_id uuid,
  p_person_id uuid,
...
begin
  ...
  foreach setting_name in array array['app.auth_user_id','request.jwt.claim.sub',
    'app.actor_auth_user_id','app.actor_person_id','app.acting_party_id',
  ...
  perform set_config('app.acting_party_id',p_person_id::text,true);
